...
contains
  ...
  subroutine A_cll(A,Auv,m02,rmax,Aerr,id_in)
    ...
    double complex :: Adduv(0:rmax1_DD), Add(0:rmax1_DD)
    double precision :: Aerraux(0:rmax),Adiff(0:rmax/2)
    ...
    logical :: mflag,eflag
    integer, parameter :: noutCritPointsAmax=50
    ...
    select case (mode_cll)
      ...
      case (2)
        ...
        if (rmax.gt.rmax1_DD) then
          call setErrFlag_cll(-10)
          call ErrOut_cll('A_cll','subroutine call with rank higher than rmax1_DD',eflag)
          if(eflag) then
            write(nerrout_cll,*) 'A_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'A_cll: 1-point functions in DD library'
            write(nerrout_cll,*) 'A_cll: --> increase value of rmax1_DD'
          end if
        end if
    ...
    if (Monitoring) then
      ...
      if(maxval(Aacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntA_cll.le.noutCritPointsAmax ) then
          call CritPointsOut_cll('A_cll',0,maxval(Aacc), CritPointsCntA_cll)
          if( CritPointsCntA_cll.eq.noutCritPointsAmax) then
  ...
  subroutine B_main_cll(B,Buv,p10,m02,m12,rmax,Berr,id_in)
    ...
    select case (mode_cll)
      ...
      case (2)
        ...
        if (rmax.gt.rmax2_DD) then
          call setErrFlag_cll(-10)
          call ErrOut_cll('B_cll','subroutine call with rank higher than rmax2_DD',eflag)
          if(eflag) then
            write(nerrout_cll,*) 'B_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'B_cll: 2-point functions in DD library'
            write(nerrout_cll,*) 'B_cll: --> increase value of rmax2_DD'
          end if
        end if
        ...
        rank = min(rmax,rmax2_DD)
        rank_aux = rank
        call B_dd(Bdd,Bdduv,q10,mm02,mm12,rank_aux,id)
        do n0=0,rank/2
          B(n0,0:rank) = Bdd(2*n0,0:rank)
          Buv(n0,0:rank) = Bdduv(2*n0,0:rank)
        end do
      ...
      case (3)
        ...
        rank = min(rmax,rmax2_DD)
        rank_aux = rank
        call B_dd(Bdd,Bdduv,q10,mm02,mm12,rank_aux,0)
        do n0=0,rank/2
          B2(n0,0:rmax) = Bdd(2*n0,0:rmax)
          B2uv(n0,0:rmax) = Bdduv(2*n0,0:rmax)
        end do
        call DDgetacc(accrelDD,accabsDD,accrel2DD,accabs2DD,NDD,rankDD,accflagDD,errflagDD,id)
    ...
    if (Monitoring) then
      ...
      if(maxval(Bacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntB_cll.le.noutCritPointsBmax ) then
          call CritPointsOut_cll('B_cll',0,maxval(Bacc), CritPointsCntB_cll)
          if( CritPointsCntB_cll.eq.noutCritPointsBmax) then
  ...
  subroutine C_main_cll(C,Cuv,p10,p21,p20,m02,m12,m22,rmax,Cerr,id_in,Cerr2)
    ...
    double complex :: Cdduv(0:2*rmax3_DD,0:rmax3_DD,0:rmax3_DD) 
    double complex :: Cdd(0:2*rmax3_DD,0:rmax3_DD,0:rmax3_DD)
    double precision :: Cerraux(0:rmax),Cerr2aux(0:rmax)
    double complex :: elimcminf2
    double complex args(6)
    integer :: n0,rank,rank_aux,errflag,id
    double precision :: accrelDD(0:rmax_DD),accabsDD(0:rmax_DD)
    double precision :: accrel2DD(0:rmax_DD),accabs2DD(0:rmax_DD)
    ...
    logical :: mflag,eflag
    integer :: r,n1,n2
    integer, parameter :: noutCritPointsCmax=50
    ...
    select case (mode_cll)
      ...
      case (2)
        ...
        if (rmax.gt.rmax3_DD) then
          call setErrFlag_cll(-10)
          call ErrOut_cll('C_cll','subroutine call with rank higher than rmax3_DD',eflag)
          if(eflag) then
            write(nerrout_cll,*) 'C_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'C_cll: 3-point functions in DD library'
            write(nerrout_cll,*) 'C_cll: --> increase value of rmax3_DD'
          end if
        end if
        ...
        rank = min(rmax,rmax3_DD)
        rank_aux = rank
        call C_dd(Cdd,Cdduv,q10,q21,q20,mm02,mm12,mm22,rank_aux,id)
        do n0=0,rank/2
          C(n0,0:rank,0:rank) = Cdd(2*n0,0:rank,0:rank)
          Cuv(n0,0:rank,0:rank) = Cdduv(2*n0,0:rank,0:rank)
        end do
      ...
      case (3)
        ...
        call C_dd(Cdd,Cdduv,q10,q21,q20,mm02,mm12,mm22,rank_aux,id)
        do n0=0,rank/2
          C2(n0,0:rank,0:rank) = Cdd(2*n0,0:rank,0:rank)
          C2uv(n0,0:rank,0:rank) = Cdduv(2*n0,0:rank,0:rank)
        end do
    ...
    if (Monitoring) then
      ...
      if(maxval(Cacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntC_cll.le.noutCritPointsCmax ) then
          call CritPointsOut_cll('C_cll',0,maxval(Cacc), CritPointsCntC_cll)
          if( CritPointsCntC_cll.eq.noutCritPointsCmax) then
...
!      if (mode_cll.ne.2) then      
      ...
      if(maxval(Cacc2).gt.critacc_cll) then
        ...
        if ( CritPointsCntC2_cll.le.noutCritPointsCmax ) then
          call CritPointsOut2_cll('C_cll',0,maxval(Cacc2), CritPointsCntC2_cll)
          if( CritPointsCntC2_cll.eq.noutCritPointsCmax) then
...
!      end if
  ...
  subroutine D_main_cll(D,Duv,p10,p21,p32,p30,p20,p31,  &
    ...
    double complex :: Ddduv(0:2*rmax4_DD,0:rmax4_DD,0:rmax4_DD,0:rmax4_DD) 
    double complex :: Ddd(0:2*rmax4_DD,0:rmax4_DD,0:rmax4_DD,0:rmax4_DD)
    ...
    logical :: mflag,eflag
    integer :: r,n1,n2,n3
    integer, parameter :: noutCritPointsDmax=50
    ...
    select case (mode_cll)
      ...
      case (2)
        ...
        if (rmax.gt.rmax4_DD) then
          call setErrFlag_cll(-10)
          call ErrOut_cll('D_cll','subroutine call with rank higher than rmax4_DD',eflag)
          if(eflag) then
            write(nerrout_cll,*) 'D_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'D_cll: 4-point functions in DD library'
            write(nerrout_cll,*) 'D_cll: --> increase value of rmax4_DD'
          end if
        end if
...
!        write(*,*) mm22,mm32
        call D_dd(Ddd,Ddduv,q10,q21,q32,q30,q20,q31,  &
                  mm02,mm12,mm22,mm32,rank_aux,id)   
        do n0=0,rank/2
          D(n0,0:rank,0:rank,0:rank) = Ddd(2*n0,0:rank,0:rank,0:rank)
          Duv(n0,0:rank,0:rank,0:rank) = Ddduv(2*n0,0:rank,0:rank,0:rank)
        end do
      ...
      case (3)
        ...
        call D_dd(Ddd,Ddduv,q10,q21,q32,q30,q20,q31,  &
                  mm02,mm12,mm22,mm32,rank_aux,id)   
        do n0=0,rank/2
          D2(n0,0:rank,0:rank,0:rank) = Ddd(2*n0,0:rank,0:rank,0:rank)
          D2uv(n0,0:rank,0:rank,0:rank) = Ddduv(2*n0,0:rank,0:rank,0:rank)
        end do
        call DDgetacc(accrelDD,accabsDD,accrel2DD,accabs2DD,NDD,rankDD,accflagDD,errflagDD,id)
    ...
    if (Monitoring) then
      ...
      if(maxval(Dacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntD_cll.le.noutCritPointsDmax ) then
          call CritPointsOut_cll('D_cll',0,maxval(Dacc), CritPointsCntD_cll)
          if( CritPointsCntD_cll.eq.noutCritPointsDmax) then
...
!      if (mode_cll.ne.2) then      
      ...
      if(maxval(Dacc2).gt.critacc_cll) then
        ...
        if ( CritPointsCntD2_cll.le.noutCritPointsDmax ) then
          call CritPointsOut2_cll('D_cll',0,maxval(Dacc2), CritPointsCntD2_cll)
          if( CritPointsCntD2_cll.eq.noutCritPointsDmax) then
...
!      end if
  ...
  subroutine E_main_cll(E,Euv,p10,p21,p32,p43,p40,p20,p31,p42,p30,p41, &
    ...
    double complex :: Edd(0:rmax5_DD,0:rmax5_DD,0:rmax5_DD,0:rmax5_DD,0:rmax5_DD)
    double complex :: elimcminf2
    ...
    integer, parameter :: noutCritPointsEmax=50
    ...
    select case (mode_cll)
      ...
      case (2)
        ...
        else if (rmax.gt.rmax5_DD) then
          call setErrFlag_cll(-10)
          call ErrOut_cll('E_cll','subroutine call with rank higher than rmax5_DD',eflag)
          if(eflag) then
            write(nerrout_cll,*) 'E_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'E_cll: 5-point functions in DD library'
            write(nerrout_cll,*) 'E_cll: --> increase value of rmax5_DD'
          end if
        end if
        ...
        rank = min(rmax,rmax5_DD)
        rank_aux = rank
        call E_dd(Edd,q10,q21,q32,q43,q40,q20,q31,q42,q30,q41,  &
                  mm02,mm12,mm22,mm32,mm42,rank_aux,id)  
        do n0=0,rank/2
          E(n0,0:rank,0:rank,0:rank,0:rank) = Edd(2*n0,0:rank,0:rank,0:rank,0:rank)
        end do
        Euv = 0d0
      ...
      case (3)
        ...
        if (rmax.gt.5) then
          ...
          if(eflag) then
            write(nerrout_cll,*) 'E_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'E_cll: 5-point functions in DD library'
            write(nerrout_cll,*) 'E_cll: --> increase value of rmax5'
          end if
        end if
        ...
        call E_dd(Edd,q10,q21,q32,q43,q40,q20,q31,q42,q30,q41,  &
                  mm02,mm12,mm22,mm32,mm42,rank_aux,id)  
        do n0=0,rank/2
          E2(n0,0:rank,0:rank,0:rank,0:rank) = Edd(2*n0,0:rank,0:rank,0:rank,0:rank)
        end do
        E2uv = 0d0
    ...
    if (Monitoring) then
      ...
      if(maxval(Eacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntE_cll.le.noutCritPointsEmax ) then
          call CritPointsOut_cll('E_cll',0,maxval(Eacc), CritPointsCntE_cll)
          if( CritPointsCntE_cll.eq.noutCritPointsEmax) then
...
!      if (mode_cll.ne.2) then      
      ...
      if(maxval(Eacc2).gt.critacc_cll) then
        ...
        if ( CritPointsCntE2_cll.le.noutCritPointsEmax ) then
          call CritPointsOut2_cll('E_cll',0,maxval(Eacc2), CritPointsCntE2_cll)
          if( CritPointsCntE2_cll.eq.noutCritPointsEmax) then
...
!      end if
  ...
  subroutine F_main_cll(F,Fuv,p10,p21,p32,p43,p54,p50,p20,p31,p42,p53,p40,  &
    ...
    double complex :: Fdd(0:rmax6_DD,0:rmax6_DD,0:rmax6_DD,0:rmax6_DD,0:rmax6_DD,0:rmax6_DD)
    ...
    integer, parameter :: noutCritPointsFmax=50
    ...
    select case (mode_cll)
      ...
      case (2)
        ...
        if (rmax.gt.7) then
          ...
          if(eflag) then
            write(nerrout_cll,*) 'F_cll: 6-point function of rank>7 not implemented in DD library'
          end if
        else if (rmax.gt.rmax6_DD) then
          call setErrFlag_cll(-10)
          call ErrOut_cll('F_cll','subroutine call with rank higher than rmax6_DD',eflag)
          if(eflag) then
            write(nerrout_cll,*) 'F_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'F_cll: 6-point functions in DD library'
            write(nerrout_cll,*) 'F_cll: --> increase value of rmax6_DD'
          end if
        end if
        ...
        rank = min(rmax,rmax6_DD)
        rank_aux = rank
        call F_dd(Fdd,q10,q21,q32,q43,q54,q50,q20,q31,q42,q53,q40,  &
                  q51,q30,q41,q52,mm02,mm12,mm22,mm32,mm42,mm52,rank_aux,id)  
        do n0=0,rank/2
          F(n0,0:rank,0:rank,0:rank,0:rank,0:rank) = Fdd(2*n0,0:rank,0:rank,0:rank,0:rank,0:rank)
        end do
        Fuv = 0d0
      ...
      case (3)
        ...
        call F_dd(Fdd,q10,q21,q32,q43,q54,q50,q20,q31,q42,q53,q40,  &
                  q51,q30,q41,q52,mm02,mm12,mm22,mm32,mm42,mm52,rank_aux,id)  
        do n0=0,rank/2
          F2(n0,0:rank,0:rank,0:rank,0:rank,0:rank) = Fdd(2*n0,0:rank,0:rank,0:rank,0:rank,0:rank)
        end do
        F2uv = 0d0
    ...
    if (Monitoring) then
      ...
      if(maxval(Facc).gt.critacc_cll) then
        ...
        if ( CritPointsCntF_cll.le.noutCritPointsFmax ) then
          call CritPointsOut_cll('F_cll',0,maxval(Facc), CritPointsCntF_cll)
          if( CritPointsCntF_cll.eq.noutCritPointsFmax) then
...
!      if (mode_cll.ne.2) then      
      ...
      if(maxval(Facc2).gt.critacc_cll) then
        ...
        if ( CritPointsCntF2_cll.le.noutCritPointsFmax ) then
          call CritPointsOut2_cll('F_cll',0,maxval(Facc2), CritPointsCntF2_cll)
          if( CritPointsCntF2_cll.eq.noutCritPointsFmax) then
...
!      end if
  ...
  subroutine G_main_cll(G,Guv,p10,p21,p32,p43,p54,p65,p60,p20,p31,p42,p53,  &
    ...
    integer, parameter :: noutCritPointsGmax=50
    ...
    if (Monitoring) then
      ...
      if(maxval(Gacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntG_cll.le.noutCritPointsGmax ) then
          call CritPointsOut_cll('G_cll',0,maxval(Gacc), CritPointsCntG_cll)
          if( CritPointsCntG_cll.eq.noutCritPointsGmax) then
      ...
      if(maxval(Gacc2).gt.critacc_cll) then
        ...
        if ( CritPointsCntG2_cll.le.noutCritPointsGmax ) then
          call CritPointsOut2_cll('G_cll',0,maxval(Gacc2), CritPointsCntG2_cll)
          if( CritPointsCntG2_cll.eq.noutCritPointsGmax) then
  ...
  subroutine TN_main_cll(TN,TNuv,MomInv,masses2,N,rmax,TNerr,id_in,TNerr2)
    ...
    double complex :: Adduv(0:rmax1_DD), Bdduv(0:2*rmax2_DD,0:rmax2_DD)
    double complex :: Cdduv(0:2*rmax3_DD,0:rmax3_DD,0:rmax3_DD)
    double complex :: Ddduv(0:2*rmax4_DD,0:rmax4_DD,0:rmax4_DD,0:rmax4_DD)
    double complex :: Add(0:rmax1_DD), Bdd(0:2*rmax2_DD,0:rmax2_DD)
    double complex :: Cdd(0:2*rmax3_DD,0:rmax3_DD,0:rmax3_DD)
    double complex :: Ddd(0:2*rmax4_DD,0:rmax4_DD,0:rmax4_DD,0:rmax4_DD)
    double complex :: Edd(0:rmax5_DD,0:rmax5_DD,0:rmax5_DD,0:rmax5_DD,0:rmax5_DD)
    double complex :: Fdd(0:rmax6_DD,0:rmax6_DD,0:rmax6_DD,0:rmax6_DD,0:rmax6_DD,0:rmax6_DD)
      ...
      case (2)
        ...
        select case (N)
          ...
          case(2)
            ...
            if (rmax.gt.rmax2_DD) then
              call setErrFlag_cll(-10)
              call ErrOut_cll('TN_cll','subroutine call with rank higher than rmax2_DD',eflag)
              if(eflag) then
                write(nerrout_cll,*) 'TN_cll: r =', rmax, 'bigger than maximum rank of'
                write(nerrout_cll,*) 'TN_cll: 2-point functions in DD library'
                write(nerrout_cll,*) 'TN_cll: --> increase value of rmax2_DD'
              end if
            end if
          ...
          case(3)
            ...
            if (rmax.gt.rmax3_DD) then
              call setErrFlag_cll(-10)
              call ErrOut_cll('TN_cll','subroutine call with rank higher than rmax3_DD',eflag)
              if(eflag) then
                write(nerrout_cll,*) 'TN_cll: r =', rmax, 'bigger than maximum rank of'
                write(nerrout_cll,*) 'TN_cll: 3-point functions in DD library'
                write(nerrout_cll,*) 'TN_cll: --> increase value of rmax3_DD'
              end if
            end if
            ...
            do r=0,rank
              do n0=r/2,0,-1
                do n1=r-2*n0,0,-1
                  ...
                  cnt = cnt+1
                  TN(cnt) = Cdd(2*n0,n1,n2)
                  TNuv(cnt) = Cdduv(2*n0,n1,n2)
          ...
          case(4)
            ...
            if (rmax.gt.rmax4_DD) then
              call setErrFlag_cll(-10)
              call ErrOut_cll('TN_cll','subroutine call with rank higher than rmax4_DD',eflag)
              if(eflag) then
                write(nerrout_cll,*) 'TN_cll: r =', rmax, 'bigger than maximum rank of'
                write(nerrout_cll,*) 'TN_cll: 4-point functions in DD library'
                write(nerrout_cll,*) 'TN_cll: --> increase value of rmax4_DD'
              end if
            end if
              ...
              do n0=r/2,0,-1
                do n1=r-2*n0,0,-1
                  do n2=r-2*n0-n1,0,-1
                    ...
                    cnt = cnt+1
                    TN(cnt) = Ddd(2*n0,n1,n2,n3)
                    TNuv(cnt) = Ddduv(2*n0,n1,n2,n3)
          ...
          case(5)
            ...
            if (rmax.gt.5) then
              ...
              call setErrFlag_cll(-10)
              call ErrOut_cll('TN_cll','subroutine call with N=5 and rank higher than rmax5_DD',eflag)
              if(eflag) then
                write(nerrout_cll,*) 'TN_cll: r =', rmax, 'bigger than maximum rank of'
                write(nerrout_cll,*) 'TN_cll: 5-point functions in DD library'
                write(nerrout_cll,*) 'TN_cll: --> increase value of rmax5'
              end if
            end if
          ...
          case(6)
            ...
            if (rmax.gt.7) then
              ...
              call ErrOut_cll('TN_cll','rank higher than maximum rank implemented in DD library',eflag)
              if(eflag) then
                write(nerrout_cll,*) 'TN_cll: 6-point function of rank>7 not implemented in DD library'
              end if
            else if (rmax.gt.rmax6_DD) then
              call setErrFlag_cll(-10)
              call ErrOut_cll('TN_cll','subroutine call with N=6 and rank higher than rmax6_DD',eflag)
              if(eflag) then
                write(nerrout_cll,*) 'TN_cll: r =', rmax, 'bigger than maximum rank of'
                write(nerrout_cll,*) 'TN_cll: 6-point functions in DD library'
                write(nerrout_cll,*) 'TN_cll: --> increase value of rmax6_DD'
              end if
            end if
                  ...
                  do n2=r-2*n0-n1,0,-1
                    do n3=r-2*n0-n1-n2,0,-1
                      do n4=r-2*n0-n1-n2-n3,0,-1
                        ...
                        cnt = cnt+1
                        TN(cnt) = Fdd(2*n0,n1,n2,n3,n4,n5)
            ...
            do r=0,rank
              do n0=r/2,0,-1
                do n1=r-2*n0,0,-1
                  ...
                  cnt = cnt+1
                  TN2(cnt) = Cdd(2*n0,n1,n2)
                  TN2uv(cnt) = Cdduv(2*n0,n1,n2)
              ...
              do n0=r/2,0,-1
                do n1=r-2*n0,0,-1
                  do n2=r-2*n0-n1,0,-1
                    ...
                    cnt = cnt+1
                    TN2(cnt) = Ddd(2*n0,n1,n2,n3)
                    TN2uv(cnt) = Ddduv(2*n0,n1,n2,n3)
                  ...
                  do n2=r-2*n0-n1,0,-1
                    do n3=r-2*n0-n1-n2,0,-1
                      do n4=r-2*n0-n1-n2-n3,0,-1
                        ...
                        cnt = cnt+1
                        TN2(cnt) = Fdd(2*n0,n1,n2,n3,n4,n5)
    ...
    if (Monitoring) then
      ...
      if(maxval(TNacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntTN_cll(N).le.noutCritPointsTNmax ) then
          call CritPointsOut_cll('TN_cll',N,maxval(TNacc),CritPointsCntTN_cll(N))
          if( CritPointsCntTN_cll(N).eq.noutCritPointsTNmax) then
...
!      if (mode_cll.ne.2) then      
      ...
      if(maxval(TNacc2).gt.critacc_cll) then
        ...
        if ( CritPointsCntTN2_cll(N).le.noutCritPointsTNmax ) then
          call CritPointsOut2_cll('TN_cll',N,maxval(TNacc2),CritPointsCntTN2_cll(N))
          if( CritPointsCntTN2_cll(N).eq.noutCritPointsTNmax) then
...
!      end if
  ...
  subroutine T1_cll(A,Auv,masses2,N,rmax,Aerr,id_in)
    ...
    integer :: n0,r,i,cnt,rank,rank_aux,errflag
    double precision :: accrelDD(0:rmax_DD),accabsDD(0:rmax_DD)
    double precision :: accrel2DD(0:rmax_DD),accabs2DD(0:rmax_DD)
    ...
    select case (mode_cll)
      ...
      case (2)
        ...
        if (rmax.gt.rmax1_DD) then
          call setErrFlag_cll(-10)
          call ErrOut_cll('TN_cll','subroutine call with rank higher than rmax1_DD',eflag)
          if(eflag) then
            write(nerrout_cll,*) 'TN_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'TN_cll: 1-point functions in DD library'
            write(nerrout_cll,*) 'TN_cll: --> increase value of rmax1_DD'
          end if
        end if
      ...
      case (3)
        ...
        call CheckCoefsTN_cll(A,A2,MomInvDummy,masses2,1,rmax,norm,Adiff)
    ...
    if (Monitoring) then
      ...
      if(maxval(Aacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntTN_cll(1).le.noutCritPointsTNmax ) then
          call CritPointsOut_cll('TN_cll',N,maxval(Aacc),CritPointsCntTN_cll(1))
          if( CritPointsCntTN_cll(1).eq.noutCritPointsTNmax) then
  ...
  subroutine A0_cll(A0,m02)
    ...
    double complex :: Adduv(0:rmax1_DD) 
    double complex :: Add(0:rmax1_DD)
    double complex :: args(1)
...
!    write(*,*) 'F_main_cll mode',mode_cll
  ...
  subroutine DB0_main_cll(DB0,p10,m02,m12)
    ...
    double complex :: args(3)
    double complex :: DBdduv(0:2*rmax2_DD,0:rmax2_DD)
    double complex :: DBdd(0:2*rmax2_DD,0:rmax2_DD)
    integer :: errflag
  ...
  subroutine DB00_main_cll(DB00,DB00uv,p10,m02,m12)
    ...
    double complex :: DB0dd, DB1dd, DB00dd, DB00dduv
    double complex :: Bdduv(0:2*rmax2_DD,0:rmax2_DD) 
    double complex :: Bdd(0:2*rmax2_DD,0:rmax2_DD)
  ...
  subroutine DB_main_cll(DB,DBuv,p10,m02,m12,rmax,DBerr)
    ...
    integer :: accflagDD,errflagDD,NDD,rankDD
    integer :: n0,rank,rank_aux,errflag,i0,i1,n
    logical :: flag = .true.,eflag
    integer, parameter :: noutCritPointsDBmax=50
...
!    write(*,*) 'DB_main in',p10,m02,m12,rmax
    ...
    select case (mode_cll)
      ...
      case (2)
       ...
       if (rmax.gt.rmax2_DD) then
          call setErrFlag_cll(-10)
          call ErrOut_cll('DB_cll','subroutine call with rank higher than rmax2_DD',eflag)
          if(eflag) then
            write(nerrout_cll,*) 'DB_cll: r =', rmax, 'bigger than maximum rank of'
            write(nerrout_cll,*) 'DB_cll: 2-point functions in DD library'
            write(nerrout_cll,*) 'DB_cll: --> increase value of rmax2_DD'
          end if
        end if
        ...
        call DB_dd(DBdd,DBdduv,q10,mm02,mm12,rank_aux)

        do n0=0,rank/2
          DB(n0,0:rank) = DBdd(2*n0,0:rank)
          DBuv(n0,0:rank) = DBdduv(2*n0,0:rank)
        end do
      ...
      case (3)
        ...
        do n0=0,rank/2
          DB2(n0,0:rmax) = DBdd(2*n0,0:rmax)
          DB2uv(n0,0:rmax) = DBdduv(2*n0,0:rmax)
        end do
    ...
    if (Monitoring) then
      ...
      if(maxval(DBacc).gt.critacc_cll) then
        ...
        if ( CritPointsCntDB_cll.le.noutCritPointsDBmax ) then
          call CritPointsOut_cll('DB_cll',0,maxval(DBacc), CritPointsCntDB_cll)
          if( CritPointsCntDB_cll.eq.noutCritPointsDBmax) then
