c	file dcuhre.f
c	obtained from the homepage of Alan Genz, April 7, 2008,
c	http://www.math.wsu.edu/faculty/genz/homepage
c
c	"PROGRAM DTEST1" and "SUBROUTINE FTEST" commented out with "csd"
c	output variable "WORK" suppressed in this version
c	by S.Dittmaier, 7.4.08
c----------------------------------------------------------------------
C
...
csd      PROGRAM DTEST1
csd      EXTERNAL FTEST
csd      INTEGER KEY, N, NF, NDIM, MINCLS, MAXCLS, IFAIL, NEVAL, NW
csd      PARAMETER (NDIM = 5, NW = 5000, NF = NDIM+1)
csd      DOUBLE PRECISION A(NDIM), B(NDIM), WRKSTR(NW)
csd      DOUBLE PRECISION ABSEST(NF), FINEST(NF), ABSREQ, RELREQ
csd      DO 10 N = 1,NDIM
csd         A(N) = 0
csd         B(N) = 1
csd   10 CONTINUE
...
csd      ABSREQ = 0
csd      RELREQ = 1E-3
csd      CALL DCUHRE(NDIM, NF, A, B, MINCLS, MAXCLS, FTEST, ABSREQ, RELREQ,
csd     * KEY, NW, 0, FINEST, ABSEST, NEVAL, IFAIL, WRKSTR)
csd      PRINT 9999, NEVAL, IFAIL
csd 9999 FORMAT (8X, 'DCUHRE TEST RESULTS', //'     FTEST CALLS = ', I4,
csd     * ', IFAIL = ', I2, /'    N   ESTIMATED ERROR    INTEGRAL')
csd      DO 20 N = 1,NF
csd         PRINT 9998, N, ABSEST(N), FINEST(N)
csd 9998    FORMAT (3X, I2, 2F15.6)
csd   20 CONTINUE
csd      END
csd      SUBROUTINE FTEST(NDIM, Z, NFUN, F)
csd      INTEGER N, NDIM, NFUN
csd      DOUBLE PRECISION Z(NDIM), F(NFUN), SUM
csd      SUM = 0
csd      DO 10 N = 1,NDIM
csd         SUM = SUM + N*Z(N)**2
csd   10 CONTINUE
...
csd   20 CONTINUE
csd      END
csd      SUBROUTINE DCUHRE(NDIM,NUMFUN,A,B,MINPTS,MAXPTS,FUNSUB,EPSABS,
csd     +                  EPSREL,KEY,NW,RESTAR,RESULT,ABSERR,NEVAL,IFAIL,
csd     +                  WORK)
...
csd line inserted to prevent compiler warning
      lxchng = 0
C
      DO 10 J = 1,NUMFUN
          FULSMS(J) = 0
