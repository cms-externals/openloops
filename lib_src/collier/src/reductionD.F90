...
!  CalcDuv, CalcDpv, CalcDpv2, CalcDg, CalcDgy, CalcDgp
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
module globalD
  ...
  double complex :: mx(0:3,0:3),mxinv(0:3,0:3),Zinv(3,3)
...
!        if (use_cache(ncache).ge.4) then
          ...
          allocate(fct(2*NCoefsG(rmax,4)+2*(rmax+1)))
          call ReadCache(fct,2*NCoefsG(rmax,4)+2*(rmax+1),x,10,1,id,4,rank,nocalc,wrica)
...
!              do n0=0,r
          ...
          if (wrica) then
            ...
            if (rank.ne.rmax) then
              deallocate(fct)
              allocate(fct(2*NCoefsG(rank,4)+2*(rank+1)))
            end if
            do r=0,rank
!             do n0=0,r
              do n0=0,r/2+1
            ...
            call WriteCache(fct,2*NCoefsG(rank,4)+2*(rank+1),id,4,rank)
...
!    integer :: scheme_C2(rmax-1:rmax_C),scheme_C3(rmax-1,rmax_C)
    ...
    character(len=*),parameter :: fmt1 = "(A7,'dcmplx( ',d24.18,' , ',d24.18,' )')"
    character(len=*),parameter :: fmt10 = "(A17,'( ',d24.18,' , ',d24.18,' )')"
...
!          write(*,*) 'CalcDred acc_pv: ',err_pv(rmax), w_pv**((rmax-1)/2) * v_pv * err_D0,  &
!                        w_pv**((rmax-1)/2) * z_pv * err_C0, z_pv * err_C(rmax-1)
...
!      D0est =  abs(D(0,0,0,0))
      ...
      write(*,*) 'CalcDred err_D',Derr(rmax)
      ...
      write(*,*) 'CalcDred err_D2',Derr2(rmax)
#endif
...
#ifdef Dredtest
      write(*,*) 'CalcDred after exp Derr=',Derr,err_req_D
      write(*,*) 'CalcDred after exp Dacc=',Derr/abs(D(0,0,0,0)),Derr/Dtyp
      write(*,*) 'CalcDred after exp method=',Drmethod
#endif
#ifdef Dredtest
...
!          call CopyDimp2(D,D_alt(0:r,0:r,0:r,0:r),Derr,Derr_alt(0:r),Derr2,Derr2_alt(0:r),Drmethod,Drmethod_alt(0:r),rmax,r)
          ...
          write(*,*) 'CalcDred: after pv 2nd try Dacc_alt(r)=',Derr_alt/abs(D(0,0,0,0))
          write(*,*) 'CalcDred: after pv 2nd try Dmethod=',Drmethod
          write(*,*) 'CalcDred: after pv 2nd try Derr(r)=',Derr
          write(*,*) 'CalcDred: after pv 2nd try Dacc(r)=',Derr/abs(D(0,0,0,0))
#endif
...
!          call CopyDimp2(D,D_alt(0:r,0:r,0:r,0:r),Derr,Derr_alt(0:r),Derr2,Derr2_alt(0:r),Drmethod,Drmethod_alt(0:r),rmax,r)
          ...
          write(*,*) 'CalcDred: after exp 2nd try Dacc(r)=',Derr/abs(D(0,0,0,0))
#endif
...
!          call CopyDimp2(D,D_alt(0:r,0:r,0:r,0:r),Derr,Derr_alt(0:r),Derr2,Derr2_alt(0:r),Drmethod,Drmethod_alt(0:r),rmax,r)
          ...
          write(*,*) 'CalcDred: after exp 2nd try Dmethod=',Drmethod
          write(*,*) 'CalcDred: after exp 2nd try Derr(r)=',Derr
          write(*,*) 'CalcDred: after exp 2nd try Dacc(r)=',Derr/abs(D(0,0,0,0)),Derr/Dtyp
#endif
...
#ifdef Dredtest
    write(*,*) 'CalcDred final acc_D=',Derr/abs(D(0,0,0,0)),critacc_coli
...
#ifdef Dpv1test
    write(*,*) 'Dij(r)    ', r,Dij_err(r),D00_err(r)
...
!    if(rmax.ge.3)then
!      write(*,*) 'CalcDg D(0,1,1,1) = ',D(0,1,1,1)
!    endif
...
!       Derr(r) =  abs(maxZadjf/Xadj(a,b))*maxDexpgy(0,r,0)
      ...
      Dij_err2(r)=max(maxZadj/maxXadj*max(2*(r+1)*D00_err2(r+2),Cerr2_i(r,0)), &
                 maxZadj2f/maxXadj*max(2*D00_err(r+1),Cij_err2(r)))
