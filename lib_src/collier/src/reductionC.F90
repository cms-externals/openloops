...
!        if (use_cache(ncache).ge.3) then
          ...
          allocate(fct(2*NCoefsG(rmax,3)+2*(rmax+2)))
          call ReadCache(fct,2*NCoefsG(rmax,3)+2*(rmax+2),x,6,1,id,3,rank,nocalc,wrica)
...
! NOTE: coefficients shifted in cache by one slot
          ...
          if (wrica) then
            ...
            if (rank.ne.rmax) then
              deallocate(fct)
              allocate(fct(2*NCoefsG(rank,3)+2*(rank+2)))
            end if
            cnt = cnt+1
            if(present(rbasic)) then
            ...
            call WriteCache(fct,2*NCoefsG(rank,3)+2*(rank+2),id,3,rank)
...
#ifdef USEC0
    C0est = abs(C0_coli(p10,p21,p20,m02,m12,m22))
    lerr_C0 = .true.
#else
...
!      err_req_Cr = acc_req_Cr * abs(C(0,0,0))
      ...
      write(*,*) 'CalcCred Cacc after PV =',Cerr/abs(C(0,0,0)),Cerr/Ctyp
...
#ifdef TEST
    iexp = 2   ! TEST: force specific expansion
!    gy = 14
#endif
...
!        call CopyCimp2(C,C_alt,Cerr,Cerr_alt,Cerr2,Cerr2_alt,Crmethod,Crmethod_alt,rmax,rmax)
        ...
        write(*,*) 'CalcCred Cacc=',Cerr/abs(C(0,0,0)),Cerr/Ctyp
...
!          call CopyCimp2(C,C_alt(0:r,0:r,0:r),Cerr,Cerr_alt(0:r),Cerr2,Cerr2_alt(0:r),Crmethod,Crmethod_alt(0:r),rmax,r)
          ...
          write(*,*) 'CalcCred: after pv 2nd try Cacc(r)',Cerr/abs(C(0,0,0)),Cerr/Ctyp
#endif
...
!          call CopyCimp2(C,C_alt(0:r,0:r,0:r),Cerr,Cerr_alt(0:r),Cerr2,Cerr2_alt(0:r),Crmethod,Crmethod_alt(0:r),rmax,r)
          ...
          write(*,*) 'CalcCred: after exps 2nd try Cacc(r)',Cerr/abs(C(0,0,0)),Cerr/Ctyp
#endif
...
#ifdef Credtest              
    write(*,*) 'CalcCred final acc_C=',Cerr/abs(C(0,0,0)),critacc_coli
