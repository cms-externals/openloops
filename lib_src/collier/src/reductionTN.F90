...
!        write(*,*) 'CalcTN 4 TNerr2',TNerr2
  ...
  recursive subroutine CalcTNint(TN,TNuv,MomInv,masses2,N,rmax,id,TNerr,TNerr2)
    ...
    if ((use_cache_system).and.(tencache.gt.N)) then
      if ((ncache.gt.0).and.(ncache.le.ncache_max)) then
        ...
        allocate(fct(2*NCoefs(rmax,N)+2*(rmax+1)))
        call ReadCache(fct,2*NCoefs(rmax,N)+2*(rmax+1),x,bino+N,1,id,N,rank,nocalc,wrica)
        ...
        if (wrica) then
          ...
          if (rank.ne.rmax) then
            deallocate(fct)
            allocate(fct(2*NCoefs(rank,N)+2*(rank+1)))
          end if
          do r=0,rank
            do n0=r/2,0,-1
              do i=1,BinomTable(r-2*n0,N+r-2*n0-2)
          ...
          call WriteCache(fct,2*NCoefs(rank,N)+2*(rank+1),id,N,rank)
  ...
  recursive subroutine CalcTNred(TN,TNuv,MomInv,masses2,N,rmax,id,TNerr,TNerr2)
    ...
    character(len=*),parameter :: fmt10 = "(A18,'( ',d24.18,' , ',d24.18,' )')"
