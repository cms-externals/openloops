...
!        if (use_cache(ncache).ge.5) then
          ...
          allocate(fct(2*NCoefs(rmax,5)+2*(rmax+1)))
          call ReadCache(fct,2*NCoefs(rmax,5)+2*(rmax+1),x,15,1,id,5,rank,nocalc,wrica)
          ...
          if (wrica) then
            ...
            if (rank.ne.rmax) then
              deallocate(fct)
              allocate(fct(2*NCoefs(rank,5)+2*(rank+1)))
            end if
            do r=0,rank
              do n0=0,r/2
                do n1=0,r-2*n0
            ...
            call WriteCache(fct,2*NCoefs(rank,5)+2*(rank+1),id,5,rank)
...
!        if (use_cache(ncache).ge.6) then
          ...
          allocate(fct(2*NCoefs(rmax,6)+2*(rmax+1)))
          call ReadCache(fct,2*NCoefs(rmax,6)+2*(rmax+1),x,21,1,id,6,rank,nocalc,wrica)
          ...
          if (wrica) then
            ...
            if (rank.ne.rmax) then
              deallocate(fct)
              allocate(fct(2*NCoefs(rank,6)+2*(rank+1)))
            end if
            do r=0,rank
              do n0=0,r/2
                do n1=0,r-2*n0
            ...
            call WriteCache(fct,2*NCoefs(rank,6)+2*(rank+1),id,6,rank)
...
!          write(*,*) 'CalcFred Ferr=',r+1,Ferr(r+1)
            ...
            if (abs(gramdet/det).gt.critacc_coli) then
...
!        if (use_cache(ncache).ge.7) then 
          ...
          allocate(fct(2*NCoefs(rmax,7)+2*(rmax+1)))
          call ReadCache(fct,2*NCoefs(rmax,7)+2*(rmax+1),x,28,1,id,7,rank,nocalc,wrica)
          ...
          if (wrica) then
            ...
            if (rank.ne.rmax) then
              deallocate(fct)
              allocate(fct(2*NCoefs(rank,7)+2*(rank+1)))
            end if
            do r=0,rank
              do n0=0,r/2
                do n1=0,r-2*n0
            ...
            call WriteCache(fct,2*NCoefs(rank,7)+2*(rank+1),id,7,rank)
