...
module b0_QP
  use triangle_aux_QP, only: target_precision,qp,cone,cnul,choose,Lphi,zlogzf
  implicit none
  ...
  function B0_0(z,p2,muUV2) result(B0)
    complex(qp), intent(in) :: z,muUV2,p2
    complex(qp) :: B0

    ! log terms become numerically instable for large/small z
    !B0 = 2._qp + log(muUV2/p2) + z*log(z/(z + cone))-log(z + cone)
    B0 = 2._qp + log(muUV2/p2) + zlogzf(z)
