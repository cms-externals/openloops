...
module openloops
  ...
  function register_process_lib(libname, proc, perm, content, amptype)
    ...
    use ol_loop_parameters_decl_dp, only: maxpoint, maxrank
    implicit none
...
!    end do
  ...
  function write_shopping_list(ext, proc)
    use ol_parameters_decl_dp, only: shopping_list, order_ew, order_qcd, verbose, &
  ...
  subroutine phase_space_point_c(id, sqrt_s, pp) bind(c,name="ol_phase_space_point")
    ...
    integer :: f_id
    real(dp) :: f_sqrt_s
    real(dp) :: f_psp(0:3,n_external(id))
    f_id = id
    f_sqrt_s = sqrt_s
  ...
  subroutine evaluate_tree_c(id, pp, res) bind(c,name="ol_evaluate_tree")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,process_handles(id)%n_particles)
    real(dp) :: f_res
    f_id = id
  ...
  subroutine evaluate_cc_c(id, pp, res) bind(c,name="ol_evaluate_cc")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,process_handles(id)%n_particles)
    real(dp) :: f_res(rval_size(process_handles(id)%n_particles,2))
    f_id = id
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_ccmatrix_c(id, pp, res, resij) bind(c,name="ol_evaluate_ccmatrix")
    ...
    real(dp) :: f_pp(0:4,process_handles(id)%n_particles)
    real(dp) :: f_res
    real(dp) :: f_resij(process_handles(id)%n_particles-1,0:process_handles(id)%n_particles-1)
    f_id = id
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_sc_c(id, pp, emitter, polvect, res) bind(c,name="ol_evaluate_sc")
    ...
    real(dp) :: f_pp(0:4,process_handles(id)%n_particles), f_polvect(4)
    real(dp) :: f_res(process_handles(id)%n_particles)
    f_id = id
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
    f_emitter = emitter
    f_polvect = polvect
