...
function samurai_num(ncut, q, mu2)
  ...
  use ol_OPP_storing_dp
  implicit none
  complex(dp), intent(in) :: q(4) ! q = [p_x,p_y,p_z,E]
  complex(dp), intent(in) :: mu2
  integer,           intent(in) :: ncut
  complex(dp) :: samurai_num
  complex(dp) :: Qtensor(array_length_stored), q_tmp(0:3)
