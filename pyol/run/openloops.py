...
class Process:
    ...
    def _get_info(name=None, loops="auto", info_file=None):
        ...
        # {SubProcessA: {1: {VarA11: ValA11, ...}, 2: {...}}, SubProcessB: ..., ...}
        available_subprocesses = collections.defaultdict(dict)
        with open(info_file) as fh:
            for infoline in fh.readlines():
                ...
                available_subprocesses[subprocinfo[0]][subprocnum] = subprocinfo[1][subprocnum]
    ...
    def __init__(self, name, loops="auto"):
        ...
        self.subprocesses = set(sum([[sp + "_" + spnum for spnum in dsc.keys()] for (sp, dsc) in self.info.items()], []))
        ...
        # Easier to use (wrt. self.loops) information about the library content
        self.contains_tree = "t" in loops
        self.contains_loop = "l" in loops
        self.contains_pseudotree = "p" in loops
        self.contains_loopsquared = "s" in loops
        if self.contains_loopsquared:
            self.loop_type = "loopsquared"
        elif self.contains_loop:
            self.loop_type = "loop"
        else:
            self.loop_type = "tree"
...
class MatrixElement:
    ...
    def __init__(self, process=None, subprocess=None, ps_point=None,
        ...
        if process is not None:
            ...
            looptree = tree and not loop and not process.contains_tree
            tree = tree and process.contains_tree
            loop = loop and process.contains_loop
                ...
                if tree:
                    ...
                    if consistency < tree_accuracy_warn:
                        if consistency < tree_accuracy_fatal:
                            ...
                            + str(consistency) + " digits.")
                if process.contains_loopsquared:
                    # flatten tuple
