...
module ol_colourmatrix_eett_eexttx_1_/**/REALKIND
...
end module ol_colourmatrix_eett_eexttx_1_/**/REALKIND



module ol_forced_parameters_eett_eexttx_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_eett_eexttx_1_/**/REALKIND

module ol_loop_eett_eexttx_1_/**/REALKIND
  ...
  ! tree wavefunctions
  complex(REALKIND), save :: wf(4,-4+1:7)
  ! denominators
...
! **********************************************************************
  ...
  ! factors of the diagrams
    ...
    f(2) = CI*eQED**2
    f(3) = (2*CI*countertermnorm*ctVtt*eQED**2*gQCD**2)/3._/**/REALKIND
    f(4) = CI*countertermnorm*ctVtt*eQED**2*gQCD**2
...
! **********************************************************************
  ...
  call prop_W_W(wf(:,3),Q(:,3),MZ,1_intkind1,wf(:,5))
  call counter_QA_V(wf(:,-2),wf(:,-3),wf(:,6))
  call counter_QA_Z(gZu,wf(:,-2),wf(:,-3),wf(:,7))
...
subroutine diagrams(A)
  ...
  A(2) = cont_VV(wf(:,4),wf(:,5)) * den(2)

  A(3) = cont_VV(wf(:,1),wf(:,6)) * den(1)
  A(4) = cont_VV(wf(:,5),wf(:,7)) * den(2)
...
subroutine colourvectors(A, M1, M2)
  ...
  M1(1) = -(A(1)*f(1))+A(2)*f(2)

  M2(1) = -(A(3)*f(3))+A(4)*f(4)

end subroutine colourvectors

end module ol_loop_eett_eexttx_1_/**/REALKIND
