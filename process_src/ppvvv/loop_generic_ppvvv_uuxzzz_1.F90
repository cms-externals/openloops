...
module ol_external_ppvvv_uuxzzz_1
  implicit none
  integer :: dummy_counter
  ! Permutation and inverse permutation of external particles
  integer, save :: external_perm_ppvvv_uuxzzz_1(5) = &
                     [ (dummy_counter, dummy_counter = 1, 5) ]
  integer, save :: external_perm_inv_ppvvv_uuxzzz_1(5) = &
                     [ (dummy_counter, dummy_counter = 1, 5) ]
  integer, save :: extcomb_perm_ppvvv_uuxzzz_1(0:16) = &
                     [ (dummy_counter, dummy_counter = 0, 16) ]
  ! Particle types (mapping of fields to integers is not fixed!)
  integer, save :: particle_types_ppvvv_uuxzzz_1(5) = &
                     [ 1, 2, 3, 3, 3 ]
  ! Colour and helicity average factors per particle
  integer, save :: average_factors_ppvvv_uuxzzz_1(5) = &
                     [ 6, 6, 3, 3, 3 ]
  ! Average factor; initialised to the identity permutation
  integer, save :: average_factor_ppvvv_uuxzzz_1 = &
                     216
  integer, save :: channel_number_ppvvv_uuxzzz_1 = -1
  ...
  subroutine n_external(n) &
      & bind(c,name="ol_f_n_external_ppvvv_uuxzzz_1")
  ...
  subroutine n_external_c(n) &
      & bind(c,name="ol_n_external_ppvvv_uuxzzz_1")
  ...
  subroutine set_permutation(perm) &
      & bind(c,name="ol_f_set_permutation_ppvvv_uuxzzz_1")
    use ol_parameters_decl_/**/DREALKIND, only: out_symmetry_on
    use ol_generic, only: factorial
    implicit none
    integer, intent(in) :: perm(5)
    integer :: i, j, ii, jj
    integer :: particle_types_perm_ppvvv_uuxzzz_1(5)
    external_perm_ppvvv_uuxzzz_1 = perm
    do i = 1, 5
      external_perm_inv_ppvvv_uuxzzz_1( &
        external_perm_ppvvv_uuxzzz_1(i)) = i
      particle_types_perm_ppvvv_uuxzzz_1(i) = &
        particle_types_ppvvv_uuxzzz_1( &
        external_perm_ppvvv_uuxzzz_1(i))
    end do
    do i = 1, 5
      do j = 1, i
        if (external_perm_ppvvv_uuxzzz_1(i) >= &
          external_perm_ppvvv_uuxzzz_1(j)) then
          ii = external_perm_ppvvv_uuxzzz_1(i)
          jj = external_perm_ppvvv_uuxzzz_1(j)
        else
          ii = external_perm_ppvvv_uuxzzz_1(j)
          jj = external_perm_ppvvv_uuxzzz_1(i)
        end if
        extcomb_perm_ppvvv_uuxzzz_1((i*(i-1))/2 + j) = (ii*(ii-1))/2 + jj
      end do
    end do
    ! Colour and helicity average factor
    average_factor_ppvvv_uuxzzz_1 = &
      average_factors_ppvvv_uuxzzz_1( &
      external_perm_ppvvv_uuxzzz_1(1)) &
      * average_factors_ppvvv_uuxzzz_1( &
      external_perm_ppvvv_uuxzzz_1(2))
    ! Symmetry factor for outgoing particles
    if (out_symmetry_on /= 0) then
      do i = 1, 5
        average_factor_ppvvv_uuxzzz_1 = &
          average_factor_ppvvv_uuxzzz_1 &
          * factorial(count(particle_types_perm_ppvvv_uuxzzz_1(3:5) == i))
      end do
    end if
  ...
  subroutine set_permutation_c(perm) &
      & bind(c,name="ol_set_permutation_ppvvv_uuxzzz_1")
    use, intrinsic :: iso_c_binding, only: c_int
    implicit none
  ...
  subroutine get_masses(m_ex) &
      & bind(c,name="ol_f_get_masses_ppvvv_uuxzzz_1")
    ...
    do i = 1, 5
      m_ex(i) = m_ex_orig(external_perm_ppvvv_uuxzzz_1(i))
    end do
  end subroutine get_masses


  subroutine get_masses_c(m_ex) &
      & bind(c,name="ol_get_masses_ppvvv_uuxzzz_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo(sqrt_s, p_rambo) &
      & bind(c,name="ol_f_rambo_ppvvv_uuxzzz_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo_c(sqrt_s, p_rambo) &
      & bind(c,name="ol_rambo_ppvvv_uuxzzz_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
...
end module ol_external_ppvvv_uuxzzz_1


module colour_basis_ppvvv_uuxzzz_1
  implicit none
  ! tree colour basis
  ...
  pure subroutine tree_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_tree_colbasis_dim_ppvvv_uuxzzz_1")
    implicit none
  ...
  subroutine tree_colbasis(basis, powers) &
    & bind(c, name="ol_tree_colbasis_ppvvv_uuxzzz_1")
    implicit none
...
end module colour_basis_ppvvv_uuxzzz_1


! Only for compatibility with the Sherpa interface

subroutine set_permutation_ppvvv_uuxzzz_1(perm)
  use ol_external_ppvvv_uuxzzz_1, only: set_permutation
  implicit none
  integer, intent(in) :: perm(5)
  call set_permutation(perm)
end subroutine set_permutation_ppvvv_uuxzzz_1
