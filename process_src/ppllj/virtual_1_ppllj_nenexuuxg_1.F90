...
! **********************************************************************
...
#endif
  use ol_loop_ppllj_nenexuuxg_1_/**/REALKIND, only: c, f, wf, den
  ...
  call loop_VQ_A(G0(:,:,:,1),wf(:,4),G0(:,:,:,8))
  call loop_Q_A(G0(:,:,:,8),Q(:,20),ZERO,G1(:,:,:,13))
  call loop_QZ_A(G1(:,:,:,13),wf(:,3),G1(:,:,:,14),gZu)
  call loop_Q_A(G1(:,:,:,14),Q(:,23),ZERO,G2(:,:,:,15))
  call check_last_QA_V(l_switch,G2(:,:,:,15),wf(:,-3),G2tensor(:,1))
  call check_last_QA_V(l_switch,G1(:,:,:,13),wf(:,18),G1tensor(:,1))
  call loop_VA_Q(G0(:,:,:,1),wf(:,7),G0(:,:,:,9))
  call loop_A_Q(G0(:,:,:,9),Q(:,24),ZERO,G1(:,:,:,15))
  call loop_AZ_Q(G1(:,:,:,15),wf(:,3),G1(:,:,:,16),gZu)
  call loop_A_Q(G1(:,:,:,16),Q(:,27),ZERO,G2(:,:,:,16))
  call check_last_AQ_V(l_switch,G2(:,:,:,16),wf(:,-2),G2tensor(:,2))
  call check_last_AQ_V(l_switch,G1(:,:,:,15),wf(:,20),G1tensor(:,2))
  call loop_VA_Q(G0(:,:,:,1),wf(:,-3),G0(:,:,:,10))
  call loop_A_Q(G0(:,:,:,10),Q(:,8),ZERO,G1(:,:,:,17))
  call loop_AV_Q(G1(:,:,:,17),wf(:,-4),G1(:,:,:,18))
  call loop_A_Q(G1(:,:,:,18),Q(:,24),ZERO,G2(:,:,:,17))
  call check_last_AQ_V(l_switch,G2(:,:,:,17),wf(:,20),G2tensor(:,3))
  call loop_AZ_Q(G2(:,:,:,17),wf(:,3),G2(:,:,:,18),gZu)
  call loop_A_Q(G2(:,:,:,18),Q(:,27),ZERO,G3(:,:,:,1))
  call check_last_AQ_V(l_switch,G3(:,:,:,1),wf(:,-2),G3tensor(:,9))
  call loop_AZ_Q(G1(:,:,:,17),wf(:,3),G1(:,:,:,19),gZu)
  call loop_A_Q(G1(:,:,:,19),Q(:,11),ZERO,G2(:,:,:,19))
  call loop_AV_Q(G2(:,:,:,19),wf(:,-4),G2(:,:,:,20))
  call loop_A_Q(G2(:,:,:,20),Q(:,27),ZERO,G3(:,:,:,2))
  call check_last_AQ_V(l_switch,G3(:,:,:,2),wf(:,-2),G3tensor(:,10))
  call loop_UV_W(G0(:,:,:,1),Q(:,0),wf(:,-4),Q(:,16),G1(:,:,:,20))
  call loop_VA_Q(G1(:,:,:,20),wf(:,-3),G1(:,:,:,21))
  call loop_A_Q(G1(:,:,:,21),Q(:,24),ZERO,G2(:,:,:,21))
  call check_last_AQ_V(l_switch,G2(:,:,:,21),wf(:,20),G2tensor(:,4))
  call loop_AZ_Q(G2(:,:,:,21),wf(:,3),G2(:,:,:,22),gZu)
  call loop_A_Q(G2(:,:,:,22),Q(:,27),ZERO,G3(:,:,:,3))
  call check_last_AQ_V(l_switch,G3(:,:,:,3),wf(:,-2),G3tensor(:,11))
  call loop_VA_Q(G1(:,:,:,20),wf(:,18),G1(:,:,:,22))
  call loop_A_Q(G1(:,:,:,22),Q(:,27),ZERO,G2(:,:,:,23))
  call check_last_AQ_V(l_switch,G2(:,:,:,23),wf(:,-2),G2tensor(:,5))
  call loop_VA_Q(G0(:,:,:,1),wf(:,18),G0(:,:,:,11))
  call loop_A_Q(G0(:,:,:,11),Q(:,11),ZERO,G1(:,:,:,23))
  call loop_AV_Q(G1(:,:,:,23),wf(:,-4),G1(:,:,:,24))
  call loop_A_Q(G1(:,:,:,24),Q(:,27),ZERO,G2(:,:,:,24))
