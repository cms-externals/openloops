...
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppllj_eexuuxg_1_/**/DREALKIND, only: ntry, nhel, Hel, M1helarr
#endif
...
! **********************************************************************
  ...
  use ol_external_ppllj_eexuuxg_1, only: external_perm_ppllj_eexuuxg_1, &
    & external_perm_inv_ppllj_eexuuxg_1, extcomb_perm_ppllj_eexuuxg_1, &
    & average_factor_ppllj_eexuuxg_1
  use ol_external_ppllj_eexuuxg_1, only: H, hel_not_initialised, hel_init, POLSEL
  use ol_colourmatrix_ppllj_eexuuxg_1_/**/REALKIND, only: colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppllj_eexuuxg_1_/**/REALKIND, only: check_forced_parameters
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_ppllj_eexuuxg_1,5)
  ...
  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_ppllj_eexuuxg_1(extcombs(k))
  end do
...
subroutine amp2tree_legacy(p, m2) &
    & bind(c,name="amp2tree_ppllj_eexuuxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccone_legacy(p, m2, i, j) &
    & bind(c,name="amp2ccone_ppllj_eexuuxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccall_legacy(p, m2) &
    & bind(c,name="amp2ccall_ppllj_eexuuxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
  real(c_double), intent(out) :: m2(0:17-1)
...
subroutine amp2hcone_legacy(p, m2, i, j, mom) &
    & bind(c,name="amp2hcone_ppllj_eexuuxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_legacy(p, m2, i, mom) &
    & bind(c,name="amp2hcall_ppllj_eexuuxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
  real(c_double), intent(out) :: m2(5)
