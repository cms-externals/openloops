...
module ol_vamp_1_pphlnj_nexeudxhg_1_/**/REALKIND
contains

! **********************************************************************
subroutine vamp_1(M)
! P(0:3,nlegs) = incoming external momenta
! Uses tree structures 'wf', factors 'c', and denominators 'den' from loop_pphlnj_nexeudxhg_1.
! Sets colour stripped amplitudes A from the module loop_amplitudes_pphlnj_nexeudxhg_1.
! **********************************************************************
...
#endif
  use ol_loop_pphlnj_nexeudxhg_1_/**/REALKIND, only: c, f, wf, den
  use ol_vert_interface_/**/REALKIND
  use ol_prop_interface_/**/REALKIND
  use ol_last_step_/**/REALKIND
  use ol_tensor_sum_storage_pphlnj_nexeudxhg_1_/**/REALKIND
  ...
  call loop_QW_A(G1(:,:,:,1),wf(:,20),G1(:,:,:,2))
  call loop_Q_A(G1(:,:,:,2),Q(:,55),ZERO,G2(:,:,:,1))
  call check_last_QA_V(l_switch,G2(:,:,:,1),wf(:,-3),G2tensor(:,1))
  call check_last_QA_V(l_switch,G1(:,:,:,1),wf(:,42),G1tensor(:,1))
  call loop_VA_Q(G0(:,:,:,1),wf(:,9),G0(:,:,:,3))
  call loop_A_Q(G0(:,:,:,3),Q(:,40),ZERO,G1(:,:,:,3))
  call loop_AW_Q(G1(:,:,:,3),wf(:,20),G1(:,:,:,4))
  call loop_A_Q(G1(:,:,:,4),Q(:,59),ZERO,G2(:,:,:,2))
  call check_last_AQ_V(l_switch,G2(:,:,:,2),wf(:,-2),G2tensor(:,2))
  call check_last_AQ_V(l_switch,G1(:,:,:,3),wf(:,40),G1tensor(:,2))
  ...
  call loop_VQ_A(G0(:,:,:,1),wf(:,40),G0(:,:,:,8))
  call loop_Q_A(G0(:,:,:,8),Q(:,23),ZERO,G1(:,:,:,13))
  call loop_QV_A(G1(:,:,:,13),wf(:,-5),G1(:,:,:,14))
  call loop_Q_A(G1(:,:,:,14),Q(:,55),ZERO,G2(:,:,:,15))
  call check_last_QA_V(l_switch,G2(:,:,:,15),wf(:,-3),G2tensor(:,3))
  call loop_UV_W(G0(:,:,:,1),Q(:,0),wf(:,-5),Q(:,32),G1(:,:,:,15))
  call loop_VQ_A(G1(:,:,:,15),wf(:,40),G1(:,:,:,16))
  call loop_Q_A(G1(:,:,:,16),Q(:,55),ZERO,G2(:,:,:,16))
  call check_last_QA_V(l_switch,G2(:,:,:,16),wf(:,-3),G2tensor(:,4))
  call loop_VA_Q(G1(:,:,:,15),wf(:,42),G1(:,:,:,17))
  call loop_A_Q(G1(:,:,:,17),Q(:,59),ZERO,G2(:,:,:,17))
  call check_last_AQ_V(l_switch,G2(:,:,:,17),wf(:,-2),G2tensor(:,5))
  call loop_VA_Q(G1(:,:,:,15),wf(:,-3),G1(:,:,:,18))
  call loop_A_Q(G1(:,:,:,18),Q(:,40),ZERO,G2(:,:,:,18))
  call loop_AW_Q(G2(:,:,:,18),wf(:,20),G2(:,:,:,19))
  call loop_A_Q(G2(:,:,:,19),Q(:,59),ZERO,G3(:,:,:,1))
  call check_last_AQ_V(l_switch,G3(:,:,:,1),wf(:,-2),G3tensor(:,9))
  call loop_VA_Q(G0(:,:,:,1),wf(:,42),G0(:,:,:,9))
  call loop_A_Q(G0(:,:,:,9),Q(:,27),ZERO,G1(:,:,:,19))
  call loop_AV_Q(G1(:,:,:,19),wf(:,-5),G1(:,:,:,20))
  call loop_A_Q(G1(:,:,:,20),Q(:,59),ZERO,G2(:,:,:,20))
  call check_last_AQ_V(l_switch,G2(:,:,:,20),wf(:,-2),G2tensor(:,6))
  call loop_VA_Q(G0(:,:,:,1),wf(:,-3),G0(:,:,:,10))
  call loop_A_Q(G0(:,:,:,10),Q(:,8),ZERO,G1(:,:,:,21))
  call loop_AW_Q(G1(:,:,:,21),wf(:,20),G1(:,:,:,22))
  call loop_A_Q(G1(:,:,:,22),Q(:,27),ZERO,G2(:,:,:,21))
  call loop_AV_Q(G2(:,:,:,21),wf(:,-5),G2(:,:,:,22))
  call loop_A_Q(G2(:,:,:,22),Q(:,59),ZERO,G3(:,:,:,2))
  call check_last_AQ_V(l_switch,G3(:,:,:,2),wf(:,-2),G3tensor(:,10))
  call loop_AV_Q(G1(:,:,:,21),wf(:,-5),G1(:,:,:,23))
  call loop_A_Q(G1(:,:,:,23),Q(:,40),ZERO,G2(:,:,:,23))
  call loop_AW_Q(G2(:,:,:,23),wf(:,20),G2(:,:,:,24))
  call loop_A_Q(G2(:,:,:,24),Q(:,59),ZERO,G3(:,:,:,3))
...
end module ol_vamp_1_pphlnj_nexeudxhg_1_/**/REALKIND
