...
module ol_vamp_1_ppllj_nf5_nenexuuxg_1_/**/REALKIND
...
! Uses tree structures 'wf', factors 'c', and denominators 'den' from loop_ppllj_nf5_nenexuuxg_1.
! Sets colour stripped amplitudes A from the module loop_amplitudes_ppllj_nf5_nenexuuxg_1.
! **********************************************************************
...
#endif
  use ol_loop_ppllj_nf5_nenexuuxg_1_/**/REALKIND, only: c, f, wf, den
  use ol_vert_interface_/**/REALKIND
  use ol_prop_interface_/**/REALKIND
  use ol_last_step_/**/REALKIND
  use ol_tensor_sum_storage_ppllj_nf5_nenexuuxg_1_/**/REALKIND
...
if (mode == -1) return
  ...
  call loop_A_Q(G1(:,:,:,6),Q(:,28),MB,G2(:,:,:,7))
  call loop_AZ_Q(G2(:,:,:,7),wf(:,3),G2(:,:,:,8),gZd)
  call check_last_A_Q(l_switch,G2(:,:,:,8),Q(:,31),MB,G3tensor(:,5))
  call loop_QV_A(G0(:,:,:,1),wf(:,9),G0(:,:,:,5))
  call loop_Q_A(G0(:,:,:,5),Q(:,12),MB,G1(:,:,:,7))
  call loop_QV_A(G1(:,:,:,7),wf(:,-4),G1(:,:,:,8))
  call loop_Q_A(G1(:,:,:,8),Q(:,28),MB,G2(:,:,:,9))
  call loop_QZ_A(G2(:,:,:,9),wf(:,3),G2(:,:,:,10),gZd)
  call check_last_Q_A(l_switch,G2(:,:,:,10),Q(:,31),MB,G3tensor(:,6))
  call loop_VQ_A(G0(:,:,:,1),wf(:,4),G0(:,:,:,6))
  call loop_Q_A(G0(:,:,:,6),Q(:,20),ZERO,G1(:,:,:,9))
  call loop_QZ_A(G1(:,:,:,9),wf(:,3),G1(:,:,:,10),gZu)
  call loop_Q_A(G1(:,:,:,10),Q(:,23),ZERO,G2(:,:,:,11))
  call check_last_QA_V(l_switch,G2(:,:,:,11),wf(:,-3),G2tensor(:,1))
  call check_last_QA_V(l_switch,G1(:,:,:,9),wf(:,18),G1tensor(:,1))
  call loop_VA_Q(G0(:,:,:,1),wf(:,7),G0(:,:,:,7))
  call loop_A_Q(G0(:,:,:,7),Q(:,24),ZERO,G1(:,:,:,11))
  call loop_AZ_Q(G1(:,:,:,11),wf(:,3),G1(:,:,:,12),gZu)
  call loop_A_Q(G1(:,:,:,12),Q(:,27),ZERO,G2(:,:,:,12))
  call check_last_AQ_V(l_switch,G2(:,:,:,12),wf(:,-2),G2tensor(:,2))
  call check_last_AQ_V(l_switch,G1(:,:,:,11),wf(:,20),G1tensor(:,2))
  call loop_VA_Q(G0(:,:,:,1),wf(:,-3),G0(:,:,:,8))
  call loop_A_Q(G0(:,:,:,8),Q(:,8),ZERO,G1(:,:,:,13))
  call loop_AV_Q(G1(:,:,:,13),wf(:,-4),G1(:,:,:,14))
  call loop_A_Q(G1(:,:,:,14),Q(:,24),ZERO,G2(:,:,:,13))
  call check_last_AQ_V(l_switch,G2(:,:,:,13),wf(:,20),G2tensor(:,3))
  call loop_AZ_Q(G2(:,:,:,13),wf(:,3),G2(:,:,:,14),gZu)
  call loop_A_Q(G2(:,:,:,14),Q(:,27),ZERO,G3(:,:,:,1))
  call check_last_AQ_V(l_switch,G3(:,:,:,1),wf(:,-2),G3tensor(:,7))
  call loop_AZ_Q(G1(:,:,:,13),wf(:,3),G1(:,:,:,15),gZu)
  call loop_A_Q(G1(:,:,:,15),Q(:,11),ZERO,G2(:,:,:,15))
  call loop_AV_Q(G2(:,:,:,15),wf(:,-4),G2(:,:,:,16))
  call loop_A_Q(G2(:,:,:,16),Q(:,27),ZERO,G3(:,:,:,2))
  call check_last_AQ_V(l_switch,G3(:,:,:,2),wf(:,-2),G3tensor(:,8))
  call loop_UV_W(G0(:,:,:,1),Q(:,0),wf(:,-4),Q(:,16),G1(:,:,:,16))
  call loop_VA_Q(G1(:,:,:,16),wf(:,-3),G1(:,:,:,17))
  call loop_A_Q(G1(:,:,:,17),Q(:,24),ZERO,G2(:,:,:,17))
  call check_last_AQ_V(l_switch,G2(:,:,:,17),wf(:,20),G2tensor(:,4))
  call loop_AZ_Q(G2(:,:,:,17),wf(:,3),G2(:,:,:,18),gZu)
  call loop_A_Q(G2(:,:,:,18),Q(:,27),ZERO,G3(:,:,:,3))
  call check_last_AQ_V(l_switch,G3(:,:,:,3),wf(:,-2),G3tensor(:,9))
  call loop_VA_Q(G1(:,:,:,16),wf(:,18),G1(:,:,:,18))
  call loop_A_Q(G1(:,:,:,18),Q(:,27),ZERO,G2(:,:,:,19))
  call check_last_AQ_V(l_switch,G2(:,:,:,19),wf(:,-2),G2tensor(:,5))
  call loop_VA_Q(G0(:,:,:,1),wf(:,18),G0(:,:,:,9))
  call loop_A_Q(G0(:,:,:,9),Q(:,11),ZERO,G1(:,:,:,19))
  call loop_AV_Q(G1(:,:,:,19),wf(:,-4),G1(:,:,:,20))
  call loop_A_Q(G1(:,:,:,20),Q(:,27),ZERO,G2(:,:,:,20))
...
end module ol_vamp_1_ppllj_nf5_nenexuuxg_1_/**/REALKIND
