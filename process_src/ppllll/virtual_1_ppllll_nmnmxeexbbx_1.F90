
module ol_vamp_1_ppllll_nmnmxeexbbx_1_/**/REALKIND
contains

! **********************************************************************
subroutine vamp_1(M)
! P(0:3,nlegs) = incoming external momenta
! Uses tree structures 'wf', factors 'c', and denominators 'den' from loop_ppllll_nmnmxeexbbx_1.
! Sets colour stripped amplitudes A from the module loop_amplitudes_ppllll_nmnmxeexbbx_1.
! **********************************************************************
  use KIND_TYPES, only: REALKIND
  use ol_momenta_decl_/**/REALKIND, only: Q
  use ol_parameters_decl_/**/REALKIND ! masses
#ifndef PRECISION_dp
  use ol_parameters_decl_/**/DREALKIND, only: l_switch
#endif
  use ol_loop_ppllll_nmnmxeexbbx_1_/**/REALKIND, only: c, f, wf, den
  use ol_vert_interface_/**/REALKIND
  use ol_prop_interface_/**/REALKIND
  use ol_last_step_/**/REALKIND
  use ol_tensor_sum_storage_ppllll_nmnmxeexbbx_1_/**/REALKIND
  use ol_loop_routines_/**/REALKIND, only: G0initialisation, TI2_call
  implicit none

  complex(REALKIND) :: Gcoeff
  complex(REALKIND), intent(in) :: M(1)

  ! Declarations of loop wave function tensors
  complex(REALKIND), dimension(4,1,4,5) :: G0
  complex(REALKIND), dimension(4,5,4,18) :: G1
  complex(REALKIND), dimension(4,15,4,18) :: G2
  complex(REALKIND), dimension(4,35,4,4) :: G3
  complex(REALKIND), dimension(5,4) :: G1tensor
  complex(REALKIND), dimension(15,15) :: G2tensor
  complex(REALKIND), dimension(35,4) :: G3tensor


  call G0initialisation(G0(:,:,:,1))

  ! Vertex and propagator calls to build loop structures
  call loop_VA_Q(G0(:,:,:,1),wf(:,-5),G0(:,:,:,2))
  call loop_A_Q(G0(:,:,:,2),Q(:,32),MB,G1(:,:,:,1))
  call loop_AV_Q(G1(:,:,:,1),wf(:,7),G1(:,:,:,2))
  call loop_A_Q(G1(:,:,:,2),Q(:,44),MB,G2(:,:,:,1))
  call check_last_AQ_V(l_switch,G2(:,:,:,1),wf(:,10),G2tensor(:,1))
  call loop_AZ_Q(G2(:,:,:,1),wf(:,4),G2(:,:,:,2),gZd)
  call loop_A_Q(G2(:,:,:,2),Q(:,47),MB,G3(:,:,:,1))
  call check_last_AQ_V(l_switch,G3(:,:,:,1),wf(:,-4),G3tensor(:,1))
  call loop_AZ_Q(G1(:,:,:,1),wf(:,5),G1(:,:,:,3),gZd)
  call loop_A_Q(G1(:,:,:,3),Q(:,44),MB,G2(:,:,:,3))
  call check_last_AQ_V(l_switch,G2(:,:,:,3),wf(:,10),G2tensor(:,2))
  call loop_AZ_Q(G2(:,:,:,3),wf(:,4),G2(:,:,:,4),gZd)
  call loop_A_Q(G2(:,:,:,4),Q(:,47),MB,G3(:,:,:,2))
  call check_last_AQ_V(l_switch,G3(:,:,:,2),wf(:,-4),G3tensor(:,2))
  call loop_AZ_Q(G1(:,:,:,1),wf(:,4),G1(:,:,:,4),gZd)
  call loop_A_Q(G1(:,:,:,4),Q(:,35),MB,G2(:,:,:,5))
  call check_last_AQ_V(l_switch,G2(:,:,:,5),wf(:,14),G2tensor(:,3))
  call check_last_AQ_V(l_switch,G2(:,:,:,5),wf(:,16),G2tensor(:,4))
  call loop_AV_Q(G2(:,:,:,5),wf(:,7),G2(:,:,:,6))
  call loop_A_Q(G2(:,:,:,6),Q(:,47),MB,G3(:,:,:,3))
  call check_last_AQ_V(l_switch,G3(:,:,:,3),wf(:,-4),G3tensor(:,3))
  call loop_AZ_Q(G2(:,:,:,5),wf(:,5),G2(:,:,:,7),gZd)
  call loop_A_Q(G2(:,:,:,7),Q(:,47),MB,G3(:,:,:,4))
  call check_last_AQ_V(l_switch,G3(:,:,:,4),wf(:,-4),G3tensor(:,4))
  call loop_AS_Q(G1(:,:,:,1),wf(:,6),G1(:,:,:,5),gH)
  call loop_A_Q(G1(:,:,:,5),Q(:,47),MB,G2(:,:,:,8))
  call check_last_AQ_V(l_switch,G2(:,:,:,8),wf(:,-4),G2tensor(:,5))
  call loop_AV_Q(G1(:,:,:,1),wf(:,60),G1(:,:,:,6))
  call loop_A_Q(G1(:,:,:,6),Q(:,47),MB,G2(:,:,:,9))
  call check_last_AQ_V(l_switch,G2(:,:,:,9),wf(:,-4),G2tensor(:,6))
  call loop_AZ_Q(G1(:,:,:,1),wf(:,62),G1(:,:,:,7),gZd)
  call loop_A_Q(G1(:,:,:,7),Q(:,47),MB,G2(:,:,:,10))
  call check_last_AQ_V(l_switch,G2(:,:,:,10),wf(:,-4),G2tensor(:,7))
  call loop_AV_Q(G1(:,:,:,1),wf(:,64),G1(:,:,:,8))
  call loop_A_Q(G1(:,:,:,8),Q(:,47),MB,G2(:,:,:,11))
  call check_last_AQ_V(l_switch,G2(:,:,:,11),wf(:,-4),G2tensor(:,8))
  call loop_AZ_Q(G1(:,:,:,1),wf(:,66),G1(:,:,:,9),gZd)
  call loop_A_Q(G1(:,:,:,9),Q(:,47),MB,G2(:,:,:,12))
  call check_last_AQ_V(l_switch,G2(:,:,:,12),wf(:,-4),G2tensor(:,9))
  call loop_AZ_Q(G1(:,:,:,1),wf(:,68),G1(:,:,:,10),gZd)
  call loop_A_Q(G1(:,:,:,10),Q(:,47),MB,G2(:,:,:,13))
  call check_last_AQ_V(l_switch,G2(:,:,:,13),wf(:,-4),G2tensor(:,10))
  call loop_AZ_Q(G1(:,:,:,1),wf(:,71),G1(:,:,:,11),gZd)
  call loop_A_Q(G1(:,:,:,11),Q(:,47),MB,G2(:,:,:,14))
  call check_last_AQ_V(l_switch,G2(:,:,:,14),wf(:,-4),G2tensor(:,11))
  call loop_VA_Q(G0(:,:,:,1),wf(:,39),G0(:,:,:,3))
  call loop_A_Q(G0(:,:,:,3),Q(:,35),MB,G1(:,:,:,12))
  call loop_AV_Q(G1(:,:,:,12),wf(:,7),G1(:,:,:,13))
  call loop_A_Q(G1(:,:,:,13),Q(:,47),MB,G2(:,:,:,15))
  call check_last_AQ_V(l_switch,G2(:,:,:,15),wf(:,-4),G2tensor(:,12))
  call loop_AZ_Q(G1(:,:,:,12),wf(:,5),G1(:,:,:,14),gZd)
  call loop_A_Q(G1(:,:,:,14),Q(:,47),MB,G2(:,:,:,16))
  call check_last_AQ_V(l_switch,G2(:,:,:,16),wf(:,-4),G2tensor(:,13))
  call check_last_AQ_V(l_switch,G1(:,:,:,12),wf(:,14),G1tensor(:,1))
  call check_last_AQ_V(l_switch,G1(:,:,:,12),wf(:,16),G1tensor(:,2))
  call loop_VA_Q(G0(:,:,:,1),wf(:,42),G0(:,:,:,4))
  call loop_A_Q(G0(:,:,:,4),Q(:,44),MB,G1(:,:,:,15))
  call loop_AZ_Q(G1(:,:,:,15),wf(:,4),G1(:,:,:,16),gZd)
  call loop_A_Q(G1(:,:,:,16),Q(:,47),MB,G2(:,:,:,17))
  call check_last_AQ_V(l_switch,G2(:,:,:,17),wf(:,-4),G2tensor(:,14))
  call check_last_AQ_V(l_switch,G1(:,:,:,15),wf(:,10),G1tensor(:,3))
  call loop_VA_Q(G0(:,:,:,1),wf(:,43),G0(:,:,:,5))
  call loop_A_Q(G0(:,:,:,5),Q(:,44),MB,G1(:,:,:,17))
  call loop_AZ_Q(G1(:,:,:,17),wf(:,4),G1(:,:,:,18),gZd)
  call loop_A_Q(G1(:,:,:,18),Q(:,47),MB,G2(:,:,:,18))
  call check_last_AQ_V(l_switch,G2(:,:,:,18),wf(:,-4),G2tensor(:,15))
  call check_last_AQ_V(l_switch,G1(:,:,:,17),wf(:,10),G1tensor(:,4))


  ! add colour interference with born to the sum of coefficient tensors for the proper tensor integral
  Gcoeff = (c(1)*M(1)) * den(9)
  T2sum(1:15,1) = T2sum(1:15,1) + Gcoeff * G2tensor(:,1)
  Gcoeff = (c(2)*M(1)) * den(10)
  T2sum(1:15,1) = T2sum(1:15,1) + Gcoeff * G2tensor(:,2)
  Gcoeff = (c(1)*M(1)) * den(35)
  T2sum(1:15,2) = T2sum(1:15,2) + Gcoeff * G2tensor(:,12)
  Gcoeff = (c(2)*M(1)) * den(36)
  T2sum(1:15,2) = T2sum(1:15,2) + Gcoeff * G2tensor(:,13)
  Gcoeff = (c(1)*M(1)) * den(13)
  T2sum(1:15,3) = T2sum(1:15,3) + Gcoeff * G2tensor(:,3)
  Gcoeff = (c(2)*M(1)) * den(15)
  T2sum(1:15,3) = T2sum(1:15,3) + Gcoeff * G2tensor(:,4)
  Gcoeff = (c(1)*M(1)) * den(39)
  T2sum(1:15,4) = T2sum(1:15,4) + Gcoeff * G2tensor(:,14)
  Gcoeff = (c(2)*M(1)) * den(41)
  T2sum(1:15,4) = T2sum(1:15,4) + Gcoeff * G2tensor(:,15)
  Gcoeff = (-(c(3)*M(1))) * den(47)
  T2sum(1:15,5) = T2sum(1:15,5) + Gcoeff * G2tensor(:,5)
  Gcoeff = (c(1)*M(1)) * den(60)
  T3sum(1:35,1) = T3sum(1:35,1) + Gcoeff * G3tensor(:,1)
  Gcoeff = (c(2)*M(1)) * den(4)
  T3sum(1:35,1) = T3sum(1:35,1) + Gcoeff * G3tensor(:,2)
  Gcoeff = (c(1)*M(1)) * den(60)
  T3sum(1:35,2) = T3sum(1:35,2) + Gcoeff * G3tensor(:,3)
  Gcoeff = (c(2)*M(1)) * den(4)
  T3sum(1:35,2) = T3sum(1:35,2) + Gcoeff * G3tensor(:,4)
  Gcoeff = (c(1)*M(1)) * den(49)
  T2sum(1:15,5) = T2sum(1:15,5) + Gcoeff * G2tensor(:,6)
  Gcoeff = (c(2)*M(1)) * den(51)
  T2sum(1:15,5) = T2sum(1:15,5) + Gcoeff * G2tensor(:,7)
  Gcoeff = (c(1)*M(1)) * den(54)
  T2sum(1:15,5) = T2sum(1:15,5) + Gcoeff * G2tensor(:,8)
  Gcoeff = (c(2)*M(1)) * den(55)
  T2sum(1:15,5) = T2sum(1:15,5) + Gcoeff * G2tensor(:,9)
  Gcoeff = (c(2)*M(1)) * den(56)
  T2sum(1:15,5) = T2sum(1:15,5) + Gcoeff * G2tensor(:,10)
  Gcoeff = (c(2)*M(1)) * den(59)
  T2sum(1:15,5) = T2sum(1:15,5) + Gcoeff * G2tensor(:,11)
  Gcoeff = (c(1)*M(1)) * den(42)
  T1sum(1:5,1) = T1sum(1:5,1) + Gcoeff * G1tensor(:,3)
  Gcoeff = (c(2)*M(1)) * den(43)
  T1sum(1:5,1) = T1sum(1:5,1) + Gcoeff * G1tensor(:,4)
  Gcoeff = (c(1)*M(1)) * den(44)
  T1sum(1:5,2) = T1sum(1:5,2) + Gcoeff * G1tensor(:,1)
  Gcoeff = (c(2)*M(1)) * den(45)
  T1sum(1:5,2) = T1sum(1:5,2) + Gcoeff * G1tensor(:,2)

end subroutine vamp_1

end module ol_vamp_1_ppllll_nmnmxeexbbx_1_/**/REALKIND
