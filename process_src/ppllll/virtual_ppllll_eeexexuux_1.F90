...
module ol_vamp_ppllll_eeexexuux_1_/**/REALKIND
  contains
...
! **********************************************************************
  ...
  use ol_vamp_1_ppllll_eeexexuux_1_/**/REALKIND, only: vamp_1
...
! **********************************************************************
  ...
  use ol_external_ppllll_eeexexuux_1, only: &
    & external_perm_inv_ppllll_eeexexuux_1, average_factor_ppllll_eeexexuux_1
  use ol_external_ppllll_eeexexuux_1, only: hel_not_initialised, hel_init, H
  use ol_colourmatrix_ppllll_eeexexuux_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  ...
  use ol_tensor_sum_storage_ppllll_eeexexuux_1_/**/REALKIND, only: &
    & reset_tensor_sum, integrate_tensor_sum, scale_tensor_sum
  use ol_loop_ppllll_eeexexuux_1_/**/REALKIND, only: &
...
! #ifdef LOOPSQUARED
...
#ifdef LOOPSQUARED
  M2L2 = (M2L2 + 2*M2L2ct + M2L2ct2) / average_factor_ppllll_eeexexuux_1
#else
  ...
  if (IR_is_on > 0) then
    ...
    IRL1(0) = c_dip(0) / average_factor_ppllll_eeexexuux_1
    IRL1(1) = c_dip(1) / average_factor_ppllll_eeexexuux_1
    IRL1(2) = c_dip(2) / average_factor_ppllll_eeexexuux_1
  else
    vdip = 0
    IRL1 = 0
  end if
  ...
  M2L0 = M2L0 / average_factor_ppllll_eeexexuux_1
  M2L1 = 2*(M2L1 + M2ct)
  if (IR_is_on > 1) then
    M2L1 = M2L1 + vdip
  end if
  M2L1 = M2L1 / average_factor_ppllll_eeexexuux_1
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_external_ppllll_eeexexuux_1, only: hel_not_initialised, hel_init, H
  use ol_colourmatrix_ppllll_eeexexuux_1_/**/REALKIND, only: &
  ...
  scalebackfactor = scalefactor**(2*6-8)
  M2tree = scalebackfactor * M2tree / average_factor_ppllll_eeexexuux_1
  M2ct   = scalebackfactor * 2*M2ct / average_factor_ppllll_eeexexuux_1
...
end module ol_vamp_ppllll_eeexexuux_1_/**/REALKIND



#ifdef PRECISION_dp

module ol_vamp_ppllll_eeexexuux_1
  use ol_parameters_decl_/**/DREALKIND, only: procname_length
  use ol_data_types_/**/DREALKIND, only: me_cache
  implicit none
  character(procname_length) :: processname = 'ppllll_eeexexuux_1'
  ...
  subroutine finish_ppllll_eeexexuux_1()
  ...
  end subroutine finish_ppllll_eeexexuux_1


  subroutine vamp2(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_f_vamp2_ppllll_eeexexuux_1")
    use KIND_TYPES, only: DREALKIND
    use ol_init, only: register_cleanup
    use ol_external_ppllll_eeexexuux_1, only: external_perm_ppllll_eeexexuux_1
    use ol_vamp_ppllll_eeexexuux_1_/**/DREALKIND, only: vamp2dp => vamp2pc
#ifdef USE_qp
    use ol_vamp_ppllll_eeexexuux_1_/**/QREALKIND, only: vamp2qp => vamp2pc
#endif
...
#endif
    if (first_call) then
      call register_cleanup(finish_ppllll_eeexexuux_1)
      first_call = .false.
    end if
    call vamp2generic(vamp2dp, vamp2qp, processname, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, &
                    ...
                    & external_perm_ppllll_eeexexuux_1, me_caches)
  end subroutine vamp2


  subroutine ctamp2(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_f_ctamp2_ppllll_eeexexuux_1")
    use KIND_TYPES, only: DREALKIND
    use ol_vamp_ppllll_eeexexuux_1_/**/DREALKIND, only: ctamp2base
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine vamp2_c(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_vamp2_ppllll_eeexexuux_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine ctamp2_c(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_ctamp2_ppllll_eeexexuux_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine vamp2_legacy(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="vamp2_ppllll_eeexexuux_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine ctamp2_legacy(P_scatt, M2tree, M2ct) &
      & bind(c,name="ctamp2_ppllll_eeexexuux_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
...
end module ol_vamp_ppllll_eeexexuux_1
