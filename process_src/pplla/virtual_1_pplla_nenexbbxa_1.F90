...
module ol_vamp_1_pplla_nenexbbxa_1_/**/REALKIND
contains

! **********************************************************************
subroutine vamp_1(M)
! P(0:3,nlegs) = incoming external momenta
! Uses tree structures 'wf', factors 'c', and denominators 'den' from loop_pplla_nenexbbxa_1.
! Sets colour stripped amplitudes A from the module loop_amplitudes_pplla_nenexbbxa_1.
! **********************************************************************
...
#endif
  use ol_loop_pplla_nenexbbxa_1_/**/REALKIND, only: c, f, wf, den
  use ol_vert_interface_/**/REALKIND
  use ol_prop_interface_/**/REALKIND
  use ol_last_step_/**/REALKIND
  use ol_tensor_sum_storage_pplla_nenexbbxa_1_/**/REALKIND
  ...
  call loop_AZ_Q(G1(:,:,:,5),wf(:,3),G1(:,:,:,7),gZd)
  call loop_A_Q(G1(:,:,:,7),Q(:,11),MB,G2(:,:,:,5))
  call loop_AV_Q(G2(:,:,:,5),wf(:,-4),G2(:,:,:,6))
  call loop_A_Q(G2(:,:,:,6),Q(:,27),MB,G3(:,:,:,2))
  call check_last_AQ_V(l_switch,G3(:,:,:,2),wf(:,-2),G3tensor(:,2))
  call loop_VA_Q(G0(:,:,:,1),wf(:,16),G0(:,:,:,5))
  call loop_A_Q(G0(:,:,:,5),Q(:,11),MB,G1(:,:,:,8))
  call loop_AV_Q(G1(:,:,:,8),wf(:,-4),G1(:,:,:,9))
  call loop_A_Q(G1(:,:,:,9),Q(:,27),MB,G2(:,:,:,7))
...
end module ol_vamp_1_pplla_nenexbbxa_1_/**/REALKIND
