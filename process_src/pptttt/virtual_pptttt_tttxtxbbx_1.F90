...
module ol_vamp_pptttt_tttxtxbbx_1_/**/REALKIND
  contains
...
! **********************************************************************
  ...
  use ol_vamp_1_pptttt_tttxtxbbx_1_/**/REALKIND, only: vamp_1
  use ol_vamp_2_pptttt_tttxtxbbx_1_/**/REALKIND, only: vamp_2
...
! M2tree = helicity-summed squared tree matrix element for top top anti-top anti-top bottom anti-bottom -> 0
! M2loop0 = helicity-summed squared loop matrix element for top top anti-top anti-top bottom anti-bottom -> 0
...
! **********************************************************************
  ...
  use ol_external_pptttt_tttxtxbbx_1, only: &
    & external_perm_inv_pptttt_tttxtxbbx_1, &
    & average_factor_pptttt_tttxtxbbx_1, &
    external_perm_pptttt_tttxtxbbx_1
  use ol_external_pptttt_tttxtxbbx_1, only: &
    & hel_not_initialised, hel_init, H, H_HC, POLSEL
  use ol_colourmatrix_pptttt_tttxtxbbx_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  ...
  use ol_forced_parameters_pptttt_tttxtxbbx_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_momenta_decl_/**/REALKIND, only: QInvariantsMatrix
  use ol_tensor_sum_storage_pptttt_tttxtxbbx_1_/**/REALKIND, only: &
    & reset_tensor_sum, integrate_tensor_sum, scale_tensor_sum
  use ol_loop_pptttt_tttxtxbbx_1_/**/REALKIND, only: &
    & fac_status_loop1, fac_status_loop2, zerohel, &
    & fac_init_loop, tree_wavefunctions
  use ol_loop_pptttt_tttxtxbbx_1_/**/DREALKIND, only: &
  ...
  complex(REALKIND)    :: M0_col1(6)
  complex(REALKIND)    :: M0(6,64), Mct(6,64), Mcol_loop(6,64)
  ...
  extmasses2 = [ rMT2, rMT2, rMT2, rMT2, rMB2, rMB2 ]
  call conv_mom_scatt2in(P_scatt, extmasses2, P, &
     & external_perm_inv_pptttt_tttxtxbbx_1, 6)
  ...
  do
    ...
    M2hel = 0
    do j = 1, 6
      M2hel = M2hel + real(sum(conjg(Mcol_loop(:,la))*KL2(1:6,j))*Mcol_loop(j,la))
    end do
    M2L2 = M2L2 + M2hel
#if 6 > 0
    do j = 1, 6
      M2L2ct  = M2L2ct  + real(sum(conjg(Mcol_loop(:,la))*KL2ct(1:6,j))*Mct(j,la))
      M2L2ct2 = M2L2ct2 + real(sum(conjg(Mct(:,la))*KL2ct2(1:6,j))*Mct(j,la))
    end do
    if (IR_is_on > 0) then
#if 6 > 0
        call colint(Mcol_loop(:,la), Mcol_loop(:,la), 6, KL2, [(i, i = 0, 22-1)], M2L2colint)  ! <M1|M1>
        call colint(2*Mcol_loop(:,la), Mct(:,la), 6, KL2ct, [(i, i = 0, 22-1)], M2L2colint)    ! 2*<M1|Mct>
#endif
#if 6 > 0
        call colint(Mct(:,la), Mct(:,la), 6, KL2ct2, [(i, i = 0, 22-1)], M2L2colint)            ! <Mct|Mct>
#endif
    end if
...
! #ifdef LOOPSQUARED
  ...
  if (IR_is_on > 0) then
    ...
    IRL1(0) = c_dip(0) / average_factor_pptttt_tttxtxbbx_1
    IRL1(1) = c_dip(1) / average_factor_pptttt_tttxtxbbx_1
    IRL1(2) = c_dip(2) / average_factor_pptttt_tttxtxbbx_1
  else
    vdip = 0
    IRL1 = 0
  end if
  ...
  if (IR_is_on > 0) then
    ...
    call intdip(ioperator_mode, M2L2, M2L2CC, M2CC_EW, [2,2,2,2,2,2], [2,2,-2,-2,-1,1]/3._/**/REALKIND, &
      & 6, extmasses2, QInvariantsMatrix, vdip, c_dip)
    IRL2(0) = c_dip(0) / average_factor_pptttt_tttxtxbbx_1
    IRL2(1) = c_dip(1) / average_factor_pptttt_tttxtxbbx_1
    IRL2(2) = c_dip(2) / average_factor_pptttt_tttxtxbbx_1
  else
    IRL2 = 0
  end if
#endif
  ...
  M2L0 = M2L0 / average_factor_pptttt_tttxtxbbx_1
  M2L1 = 2*(M2L1 + M2ct)
  if (IR_is_on > 1) then
    M2L1 = M2L1 + vdip
  end if
  M2L1 = M2L1 / average_factor_pptttt_tttxtxbbx_1
...
! P_scatt(0:3,Npart) = external momenta
! M2L0 = helicity-summed squared tree matrix element for top top anti-top anti-top bottom anti-bottom -> 0
! M2L2 = helicity-summed tree x loop matrix element for top top anti-top anti-top bottom anti-bottom -> 0
! M2L2 = helicity-summed squared loop matrix element for top top anti-top anti-top bottom anti-bottom -> 0
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_debug, only: ol_fatal
  use ol_external_pptttt_tttxtxbbx_1, only: H, H_HC, &
    average_factor_pptttt_tttxtxbbx_1, &
    external_perm_pptttt_tttxtxbbx_1, &
    extcomb_perm_pptttt_tttxtxbbx_1
  use ol_colourmatrix_pptttt_tttxtxbbx_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  use ol_parameters_decl_/**/REALKIND ! parameters_status, scalefactor, <masses>
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_loop_pptttt_tttxtxbbx_1_/**/REALKIND, only: zerohel, M0, Mct, Mcol_loop
  use ol_wavefunctions_/**/REALKIND, only: wf_V_Std
  use ol_kinematics_/**/REALKIND, only: LC2Std_Rep_cmplx
  ...
  emitter = external_perm_pptttt_tttxtxbbx_1(corr%emitter)
  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_pptttt_tttxtxbbx_1(extcombs(k))
  end do
  ...
  if (cr == 1) then
    ...
    do la=1,64
#if 6 > 0
        call colint(Mcol_loop(:,la), Mcol_loop(:,la), 6, KL2, extcombs_permuted, M2L2cc)  ! <M1|M1>
        call colint(2*Mcol_loop(:,la), Mct(:,la), 6, KL2ct, extcombs_permuted, M2L2cc)    ! 2*<M1|Mct>
#endif
#if 6 > 0
        call colint(Mct(:,la), Mct(:,la), 6, KL2ct2, extcombs_permuted, M2L2cc)            ! <Mct|Mct>
#endif
    end do
    do k = 0, 22
      corr%rescc(k) = M2L2cc(extcomb_perm_pptttt_tttxtxbbx_1(k)) / &
                      average_factor_pptttt_tttxtxbbx_1
    end do
    M2L2 = corr%rescc(0)
...
#if 6 > 0
          call colint(Mcol_loop(:,hels(1,l)), om*Mcol_loop(:,hels(2,l)), 6, KL2, extcombs_permuted, M2L2cc) ! <M1|M1>
          call colint(Mcol_loop(:,hels(1,l)), om*Mct(:,hels(2,l)), 6, KL2ct, extcombs_permuted, M2L2cc)     ! <M1|Mct>
          call colint(om*Mcol_loop(:,hels(1,l)), Mct(:,hels(2,l)), 6, KL2ct, extcombs_permuted, M2L2cc)     ! <Mct|M1>
#endif
#if 6 > 0
          call colint(Mct(:,hels(1,l)), om*Mct(:,hels(2,l)), 6, KL2ct2, extcombs_permuted, M2L2cc)           ! <Mct|Mct>
#endif
      end do
    end do

    do k = 0, 22
      corr%rescc(k) = 0.5_/**/REALKIND*M2L2cc(extcomb_perm_pptttt_tttxtxbbx_1(k)) / &
                      average_factor_pptttt_tttxtxbbx_1
    end do
  ...
  ! get wfs
    ...
    do k=1,64,2
      ...
      do l=1,size(hels,2)
#if 6 > 0
        call colint(Mcol_loop(:,hels(1,l)), Mcol_loop(:,hels(2,l)), 6, KL2(1:6,:), [0], M2hels(l:l))  ! <M1|M1>
        call colint(2*Mcol_loop(:,hels(1,l)), Mct(:,hels(2,l)), 6, KL2ct(1:6,:), [0], M2hels(l:l))    ! <M1|Mct>
#endif
#if 6 > 0
        call colint(Mct(:,hels(1,l)), Mct(:,hels(2,l)), 6, KL2ct2(1:6,:), [0], M2hels(l:l))             ! <Mct|Mct>
#endif
    ...
    if (any(corr%resmunu(:,:) /= corr%resmunu(:,:))) then
      ...
      corr%resmunu = corr%resmunu / average_factor_pptttt_tttxtxbbx_1
    ...
    do k = 0, 22
      corr%rescc(k) = M2L1cc(extcomb_perm_pptttt_tttxtxbbx_1(k)) / average_factor_pptttt_tttxtxbbx_1
    end do
    M2L1 = corr%rescc(0)
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_external_pptttt_tttxtxbbx_1, only: hel_not_initialised, hel_init, H
  use ol_colourmatrix_pptttt_tttxtxbbx_1_/**/REALKIND, only: &
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  use ol_external_pptttt_tttxtxbbx_1, only: &
    & external_perm_inv_pptttt_tttxtxbbx_1, average_factor_pptttt_tttxtxbbx_1
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_kinematics_/**/REALKIND, only: conv_mom_scatt2in, internal_momenta
  use ol_forced_parameters_pptttt_tttxtxbbx_1_/**/REALKIND, only: check_forced_parameters
  use ol_loop_pptttt_tttxtxbbx_1_/**/REALKIND, only: &
...
!  DOI      = 0
  ...
  ! Convert 2 -> n-2 PS-point to n -> 0 (so that P(1) + ... + P(n) = 0)
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_pptttt_tttxtxbbx_1, 6)
  ! internal-propagator momenta in light-cone representation
  call internal_momenta(P, 6)
...
!  DOI      = DOI_bak
  ...
  scalebackfactor = scalefactor**(2*6-8)
  M2tree = scalebackfactor * M2tree / average_factor_pptttt_tttxtxbbx_1
  M2ct   = scalebackfactor * 2*M2ct / average_factor_pptttt_tttxtxbbx_1
...
end module ol_vamp_pptttt_tttxtxbbx_1_/**/REALKIND



#ifdef PRECISION_dp

module ol_vamp_pptttt_tttxtxbbx_1
  use ol_parameters_decl_/**/DREALKIND, only: procname_length
  use ol_data_types_/**/DREALKIND, only: me_cache
  implicit none
  character(procname_length) :: processname = 'pptttt_tttxtxbbx_1'
  ...
  subroutine finish_pptttt_tttxtxbbx_1()
    ! final update of the stability histogram
    use KIND_TYPES, only: DREALKIND
    use ol_stability, only: finish_histograms
    use ol_vamp_pptttt_tttxtxbbx_1_/**/DREALKIND, only: vamp
    implicit none
    complex(DREALKIND) :: Mdummy(6)
    integer :: k
  ...
  end subroutine finish_pptttt_tttxtxbbx_1


  subroutine vamp2(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_f_vamp2_pptttt_tttxtxbbx_1")
    use KIND_TYPES, only: DREALKIND
    use ol_init, only: register_cleanup
    use ol_external_pptttt_tttxtxbbx_1, only: external_perm_pptttt_tttxtxbbx_1
    use ol_vamp_pptttt_tttxtxbbx_1_/**/DREALKIND, only: vamp2dp => vamp2pc
#ifdef USE_qp
    use ol_vamp_pptttt_tttxtxbbx_1_/**/QREALKIND, only: vamp2qp => vamp2pc
#endif
...
#endif
    if (first_call) then
      call register_cleanup(finish_pptttt_tttxtxbbx_1)
      first_call = .false.
    end if
    call vamp2generic(vamp2dp, vamp2qp, processname, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, &
                    ...
                    & external_perm_pptttt_tttxtxbbx_1, me_caches)
...
! **********************************************************************
  subroutine vampcr2(P_scatt, M2L0, M2L1, M2L2, crtype, &
      ...
      & bind(c,name="ol_f_vampcr2_pptttt_tttxtxbbx_1")
...
! **********************************************************************
    use KIND_TYPES, only: DREALKIND
    use ol_external_pptttt_tttxtxbbx_1, only: external_perm_pptttt_tttxtxbbx_1
    use ol_vamp_pptttt_tttxtxbbx_1_/**/DREALKIND, only: vamp2cache
    use ol_data_types_/**/DREALKIND, only: correlator
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine ctamp2(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_f_ctamp2_pptttt_tttxtxbbx_1")
    use KIND_TYPES, only: DREALKIND
    use ol_vamp_pptttt_tttxtxbbx_1_/**/DREALKIND, only: ctamp2base
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine vamp2_c(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_vamp2_pptttt_tttxtxbbx_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine ctamp2_c(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_ctamp2_pptttt_tttxtxbbx_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine vamp2_legacy(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="vamp2_pptttt_tttxtxbbx_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine ctamp2_legacy(P_scatt, M2tree, M2ct) &
      & bind(c,name="ctamp2_pptttt_tttxtxbbx_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
...
end module ol_vamp_pptttt_tttxtxbbx_1
