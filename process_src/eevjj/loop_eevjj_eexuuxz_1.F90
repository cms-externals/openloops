...
module ol_colourmatrix_eevjj_eexuuxz_1_/**/REALKIND
...
end module ol_colourmatrix_eevjj_eexuuxz_1_/**/REALKIND



module ol_forced_parameters_eevjj_eexuuxz_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_eevjj_eexuuxz_1_/**/REALKIND

module ol_loop_eevjj_eexuuxz_1_/**/REALKIND
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f(2) = CI*eQED**3
    f(3) = (2*CI*countertermnorm*eQED**3*gQCD**2)/3._/**/REALKIND
    f(4) = CI*countertermnorm*eQED**3*gQCD**2
    f(5) = (2*CI*countertermnorm*ctVqq*eQED**3*gQCD**2)/3._/**/REALKIND
    f(6) = CI*countertermnorm*ctVqq*eQED**3*gQCD**2
    f(7) = (2*eQED**3*gQCD**2*integralnorm*SwB)/3._/**/REALKIND
    f(8) = eQED**3*gQCD**2*integralnorm*SwB
...
! **********************************************************************
  ...
  call counter_AZ_Q(gZu,wf(:,-3),wf(:,6),wf(:,24))
  call counter_AZ_Q(gZu,wf(:,-3),wf(:,-4),wf(:,25))
  call prop_A_Q(wf(:,25),Q(:,24),ZERO,0_intkind1,wf(:,26))
  call counter_VQ_A(wf(:,1),wf(:,-2),wf(:,27))
  call counter_ZQ_A(gZu,wf(:,6),wf(:,-2),wf(:,28))
  call counter_ZQ_A(gZu,wf(:,-4),wf(:,-2),wf(:,29))
  call prop_Q_A(wf(:,29),Q(:,20),ZERO,0_intkind1,wf(:,30))
  call counter_QA_V(wf(:,-2),wf(:,-3),wf(:,31))
  call counter_QA_Z(gZu,wf(:,-2),wf(:,-3),wf(:,32))
  call prop_W_W(wf(:,32),Q(:,12),MZ,1_intkind1,wf(:,33))
  call counter_Q_A(ctqq,wf(:,3),Q(:,20),wf(:,34))
  call prop_A_Q(wf(:,4),Q(:,11),ZERO,0_intkind1,wf(:,35))
  call prop_A_Q(wf(:,7),Q(:,11),ZERO,0_intkind1,wf(:,36))
  call counter_A_Q(ctqq,wf(:,9),Q(:,24),wf(:,37))
  call prop_Q_A(wf(:,10),Q(:,7),ZERO,0_intkind1,wf(:,38))
  call prop_Q_A(wf(:,11),Q(:,7),ZERO,0_intkind1,wf(:,39))
...
subroutine colourvectors(A, M1, M2)
  ...
  M1(1) = (A(1)+A(3)+A(5)+A(7))*f(1)+(-A(2)-A(4)-A(6)-A(8))*f(2)

  M2(1) = (-A(21)-A(23))*f(3)+(A(22)+A(24))*f(4)+(A(9)+A(11)+A(13)+A(15)+A(17)+A(19))*f(5)+(-A(10)-A(12)-A(14)-A(16)-A(18) &
       -A(20))*f(6)

end subroutine colourvectors

end module ol_loop_eevjj_eexuuxz_1_/**/REALKIND
