...
module ol_vamp_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND
  contains
...
! **********************************************************************
  ...
  use ol_vamp_1_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND, only: vamp_1
...
! M2tree = helicity-summed squared tree matrix element for nu_e e+ up anti-up anti-charm bottom -> 0
! M2loop0 = helicity-summed squared loop matrix element for nu_e e+ up anti-up anti-charm bottom -> 0
...
! **********************************************************************
  ...
  use ol_external_pplnjj_ckm_neexuuxcxb_1, only: &
    & external_perm_inv_pplnjj_ckm_neexuuxcxb_1, &
    & average_factor_pplnjj_ckm_neexuuxcxb_1, &
    external_perm_pplnjj_ckm_neexuuxcxb_1
  use ol_external_pplnjj_ckm_neexuuxcxb_1, only: &
    & hel_not_initialised, hel_init, H, H_HC, POLSEL
  use ol_colourmatrix_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  ...
  use ol_forced_parameters_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_momenta_decl_/**/REALKIND, only: QInvariantsMatrix
  use ol_tensor_sum_storage_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND, only: &
    & reset_tensor_sum, integrate_tensor_sum, scale_tensor_sum
  use ol_loop_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND, only: &
...
!  complex(REALKIND)    :: Mcol_loop(2), Mct(2)
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, &
     & external_perm_inv_pplnjj_ckm_neexuuxcxb_1, 6)
...
! #ifdef LOOPSQUARED
...
#ifdef LOOPSQUARED
  M2L2 = (M2L2 + 2*M2L2ct + M2L2ct2) / average_factor_pplnjj_ckm_neexuuxcxb_1
#else
  ...
  if (IR_is_on > 0) then
    ...
    IRL1(0) = c_dip(0) / average_factor_pplnjj_ckm_neexuuxcxb_1
    IRL1(1) = c_dip(1) / average_factor_pplnjj_ckm_neexuuxcxb_1
    IRL1(2) = c_dip(2) / average_factor_pplnjj_ckm_neexuuxcxb_1
  else
    vdip = 0
    IRL1 = 0
  end if
  ...
  M2L0 = M2L0 / average_factor_pplnjj_ckm_neexuuxcxb_1
  M2L1 = 2*(M2L1 + M2ct)
  if (IR_is_on > 1) then
    M2L1 = M2L1 + vdip
  end if
  M2L1 = M2L1 / average_factor_pplnjj_ckm_neexuuxcxb_1
...
! P_scatt(0:3,Npart) = external momenta
! M2L0 = helicity-summed squared tree matrix element for nu_e e+ up anti-up anti-charm bottom -> 0
! M2L2 = helicity-summed tree x loop matrix element for nu_e e+ up anti-up anti-charm bottom -> 0
! M2L2 = helicity-summed squared loop matrix element for nu_e e+ up anti-up anti-charm bottom -> 0
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_debug, only: ol_fatal
  use ol_external_pplnjj_ckm_neexuuxcxb_1, only: average_factor_pplnjj_ckm_neexuuxcxb_1, H, H_HC, &
   external_perm_pplnjj_ckm_neexuuxcxb_1, extcomb_perm_pplnjj_ckm_neexuuxcxb_1
  use ol_colourmatrix_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  use ol_parameters_decl_/**/REALKIND ! parameters_status, scalefactor, <masses>
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_loop_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND, only: zerohel, M0, Mct, Mcol_loop
  use ol_wavefunctions_/**/REALKIND, only: wf_V_Std
  use ol_kinematics_/**/REALKIND, only: LC2Std_Rep_cmplx
  ...
  emitter = external_perm_pplnjj_ckm_neexuuxcxb_1(corr%emitter)
  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_pplnjj_ckm_neexuuxcxb_1(extcombs(k))
  end do
  ...
  if (cr == 1) then
    ...
    do k = 0, 22
      corr%rescc(k) = M2L2cc(extcomb_perm_pplnjj_ckm_neexuuxcxb_1(k)) / average_factor_pplnjj_ckm_neexuuxcxb_1
    end do
    M2L2 = corr%rescc(0)
    ...
    do k = 0, 22
      corr%rescc(k) = 0.5_/**/REALKIND*M2L2cc(extcomb_perm_pplnjj_ckm_neexuuxcxb_1(k)) / average_factor_pplnjj_ckm_neexuuxcxb_1
    end do
  ...
  ! get wfs
    ...
    if (any(corr%resmunu(:,:) /= corr%resmunu(:,:))) then
      ...
      corr%resmunu = corr%resmunu / average_factor_pplnjj_ckm_neexuuxcxb_1
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_external_pplnjj_ckm_neexuuxcxb_1, only: hel_not_initialised, hel_init, H
  use ol_colourmatrix_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND, only: &
  ...
  use ol_external_pplnjj_ckm_neexuuxcxb_1, only: &
    & external_perm_inv_pplnjj_ckm_neexuuxcxb_1, average_factor_pplnjj_ckm_neexuuxcxb_1
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_kinematics_/**/REALKIND, only: conv_mom_scatt2in, internal_momenta
  use ol_forced_parameters_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND, only: check_forced_parameters
  use ol_loop_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND, only: &
  ...
  ! Convert 2 -> n-2 PS-point to n -> 0 (so that P(1) + ... + P(n) = 0)
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_pplnjj_ckm_neexuuxcxb_1, 6)
  ! internal-propagator momenta in light-cone representation
  call internal_momenta(P, 6)
  ...
  scalebackfactor = scalefactor**(2*6-8)
  M2tree = scalebackfactor * M2tree / average_factor_pplnjj_ckm_neexuuxcxb_1
  M2ct   = scalebackfactor * 2*M2ct / average_factor_pplnjj_ckm_neexuuxcxb_1
...
end module ol_vamp_pplnjj_ckm_neexuuxcxb_1_/**/REALKIND



#ifdef PRECISION_dp

module ol_vamp_pplnjj_ckm_neexuuxcxb_1
  use ol_parameters_decl_/**/DREALKIND, only: procname_length
  use ol_data_types_/**/DREALKIND, only: me_cache
  implicit none
  character(procname_length) :: processname = 'pplnjj_ckm_neexuuxcxb_1'
  ...
  subroutine finish_pplnjj_ckm_neexuuxcxb_1()
    ! final update of the stability histogram
    use KIND_TYPES, only: DREALKIND
    use ol_stability, only: finish_histograms
    use ol_vamp_pplnjj_ckm_neexuuxcxb_1_/**/DREALKIND, only: vamp
    implicit none
  ...
  end subroutine finish_pplnjj_ckm_neexuuxcxb_1


  subroutine vamp2(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_f_vamp2_pplnjj_ckm_neexuuxcxb_1")
    use KIND_TYPES, only: DREALKIND
    use ol_init, only: register_cleanup
    use ol_external_pplnjj_ckm_neexuuxcxb_1, only: external_perm_pplnjj_ckm_neexuuxcxb_1
    use ol_vamp_pplnjj_ckm_neexuuxcxb_1_/**/DREALKIND, only: vamp2dp => vamp2pc
#ifdef USE_qp
    use ol_vamp_pplnjj_ckm_neexuuxcxb_1_/**/QREALKIND, only: vamp2qp => vamp2pc
#endif
...
#endif
    if (first_call) then
      call register_cleanup(finish_pplnjj_ckm_neexuuxcxb_1)
      first_call = .false.
    end if
    call vamp2generic(vamp2dp, vamp2qp, processname, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, &
                    ...
                    & external_perm_pplnjj_ckm_neexuuxcxb_1, me_caches)
...
! **********************************************************************
  subroutine vampcr2(P_scatt, M2L0, M2L1, M2L2, crtype, &
      ...
      & bind(c,name="ol_f_vampcr2_pplnjj_ckm_neexuuxcxb_1")
...
! **********************************************************************
    use KIND_TYPES, only: DREALKIND
    use ol_external_pplnjj_ckm_neexuuxcxb_1, only: external_perm_pplnjj_ckm_neexuuxcxb_1
    use ol_vamp_pplnjj_ckm_neexuuxcxb_1_/**/DREALKIND, only: vamp2cache
    use ol_data_types_/**/DREALKIND, only: correlator
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine ctamp2(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_f_ctamp2_pplnjj_ckm_neexuuxcxb_1")
    use KIND_TYPES, only: DREALKIND
    use ol_vamp_pplnjj_ckm_neexuuxcxb_1_/**/DREALKIND, only: ctamp2base
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine vamp2_c(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_vamp2_pplnjj_ckm_neexuuxcxb_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine ctamp2_c(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_ctamp2_pplnjj_ckm_neexuuxcxb_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine vamp2_legacy(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="vamp2_pplnjj_ckm_neexuuxcxb_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine ctamp2_legacy(P_scatt, M2tree, M2ct) &
      & bind(c,name="ctamp2_pplnjj_ckm_neexuuxcxb_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
...
end module ol_vamp_pplnjj_ckm_neexuuxcxb_1
