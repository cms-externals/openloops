...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f(3) = eQED**4*gQCD**3*integralnorm*SwF
    f(4) = 2*eQED**4*gQCD**3*integralnorm*SwF
    f(5) = (CI*countertermnorm*ctHGG*eQED**4*gQCD**3*lambdaHZZ*MB*MW*YB)/(cw**2*MQ2sum*sw)
    f(6) = (eQED**4*gQCD**3*integralnorm*lambdaHZZ*SwF*YB)/(cw**2*sw**2*2._/**/REALKIND)
    f(7) = (CI*countertermnorm*ctHGG*eQED**4*gQCD**3*lambdaHZZ*MT*MW*YT)/(cw**2*MQ2sum*sw)
...
subroutine diagrams(A)
  ...
  A(4) = cont_SS(wf(:,12),wf(:,13)) * den(12)
  A(5) = cont_VV(wf(:,15),wf(:,17)) * den(17)
  A(6) = cont_VV(wf(:,15),wf(:,20)) * den(20)
  A(7) = cont_VV(wf(:,22),wf(:,24)) * den(25)
  A(8) = cont_VV(wf(:,22),wf(:,27)) * den(28)
  A(9) = cont_VV(wf(:,31),wf(:,32)) * den(33)
  A(10) = cont_VV(wf(:,31),wf(:,35)) * den(36)
  A(11) = cont_SS(wf(:,13),wf(:,36)) * den(38)
  A(12) = cont_SS(wf(:,13),wf(:,36)) * den(38)
  A(13) = cont_VV(wf(:,38),wf(:,40)) * den(43)
  A(14) = cont_VV(wf(:,38),wf(:,43)) * den(46)
  A(15) = cont_VV(wf(:,45),wf(:,47)) * den(51)
  A(16) = cont_VV(wf(:,45),wf(:,50)) * den(54)
  A(17) = cont_VV(wf(:,31),wf(:,53)) * den(56)
  A(18) = cont_VV(wf(:,31),wf(:,56)) * den(59)
  A(19) = cont_VV(wf(:,31),wf(:,59)) * den(62)
  A(20) = cont_VV(wf(:,31),wf(:,62)) * den(64)
  A(21) = cont_VV(wf(:,31),wf(:,65)) * den(66)
  A(22) = cont_VV(wf(:,31),wf(:,68)) * den(68)
...
subroutine colourvectors(A, M1, M2)
  ...
  M2(1) = (A(5)+A(6)+A(7)+A(8)+A(9)+A(10)-A(13)-A(14)-A(15)-A(16)-A(17)-A(18)-A(19)-A(20)+A(21)+A(22))*f(1)+(-A(1)+A(2))*f(2) &
       +(A(4)-A(12))*f(5)+(A(3)-A(11))*f(7)
