...
module ol_versiondata_ppz
  implicit none
  character(4) :: generator_revision = "1248"
  interface
    subroutine version_ppz(outstring)
      implicit none
      character(60), intent(out), optional :: outstring
    end subroutine version_ppz
  end interface
end module ol_versiondata_ppz


subroutine ol_version_ppz(outstring)
  use ol_version, only: version, revision
  use ol_versiondata_ppz, only: generator_revision
  implicit none
  character(60), intent(out), optional :: outstring
