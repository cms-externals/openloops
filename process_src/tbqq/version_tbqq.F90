...
module ol_versiondata_tbqq
  implicit none
  character(4) :: generator_revision = "1811"
  interface
    subroutine version_tbqq(outstring)
      implicit none
      character(60), intent(out), optional :: outstring
    end subroutine version_tbqq
  end interface
end module ol_versiondata_tbqq


subroutine ol_version_tbqq(outstring)
  use ol_version, only: version, revision
  use ol_versiondata_tbqq, only: generator_revision
  implicit none
  character(60), intent(out), optional :: outstring
...
end subroutine ol_version_tbqq
