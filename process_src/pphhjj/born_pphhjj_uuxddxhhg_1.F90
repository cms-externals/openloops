...
module ol_colourmatrix_pphhjj_uuxddxhhg_1_/**/REALKIND
...
end module ol_colourmatrix_pphhjj_uuxddxhhg_1_/**/REALKIND



module ol_forced_parameters_pphhjj_uuxddxhhg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_pphhjj_uuxddxhhg_1_/**/REALKIND

module ol_tree_pphhjj_uuxddxhhg_1_/**/REALKIND
...
subroutine factors_init()
  ...
  ! factors of the diagrams
    ...
    f(5) = (CI*eQED**4*gQCD)/(4._/**/REALKIND*cw**2*sw**2)
    f(6) = (CI*eQED**4*gQCD)/(2._/**/REALKIND*cw**2*sw**2)
    f(7) = (3*CI*eQED**4*gQCD*lambdaHHH*lambdaHZZ*MH**2)/(2._/**/REALKIND*cw**2*sw**2)
    f(8) = (CI*eQED**4*gQCD*lambdaHZZ**2*MW**2)/(cw**4*sw**2)
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_pphhjj_uuxddxhhg_1")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_pphhjj_uuxddxhhg_1_/**/DREALKIND, only: ntry, nhel, Hel, M1helarr
#endif
...
! **********************************************************************
  ...
  use ol_external_pphhjj_uuxddxhhg_1, only: external_perm_pphhjj_uuxddxhhg_1, &
    & external_perm_inv_pphhjj_uuxddxhhg_1, extcomb_perm_pphhjj_uuxddxhhg_1, &
    & average_factor_pphhjj_uuxddxhhg_1
  use ol_external_pphhjj_uuxddxhhg_1, only: H, hel_not_initialised, hel_init, POLSEL
  use ol_colourmatrix_pphhjj_uuxddxhhg_1_/**/REALKIND, only: colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_pphhjj_uuxddxhhg_1_/**/REALKIND, only: check_forced_parameters
  use ol_heltables_pphhjj_uuxddxhhg_1
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_pphhjj_uuxddxhhg_1,7)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_pphhjj_uuxddxhhg_1(I)
  else if (I < 0) then
    JBmunu = external_perm_pphhjj_uuxddxhhg_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_pphhjj_uuxddxhhg_1(extcombs(k))
  end do
  ...
  call vert_QA_W(ntry, ex3, ex2, wf4(:,8), n3(:,7), t3x4(:,:,5))
  call prop_Q_A(ntry, wf4(:,7), Q(:,65), ZERO, 0_intkind1, wf4(:,9), n2(3))
  call vert_QA_W(ntry, wf4(:,9), ex4, wf8(:,3), n3(:,8), t3x8(:,:,3))
  call vert_SSV_V(ntry, ex5, ex6, wf4(:,8), wf4(:,10), n4(:,2), t4x4(:,:,2))
  call vert_QA_W(ntry, ex1, wf4(:,6), wf8(:,4), n3(:,9), t3x8(:,:,4))
  call vert_QA_W(ntry, ex1, ex4, wf4(:,11), n3(:,10), t3x4(:,:,6))
  call vert_AV_Q(ntry, ex2, ex7, wf4(:,12), n3(:,11), t3x4(:,:,7))
  call prop_A_Q(ntry, wf4(:,12), Q(:,66), ZERO, 0_intkind1, wf4(:,13), n2(4))
  call vert_QA_W(ntry, ex3, wf4(:,13), wf8(:,5), n3(:,12), t3x8(:,:,5))
  call vert_SSV_V(ntry, ex5, ex6, wf4(:,11), wf4(:,14), n4(:,3), t4x4(:,:,3))
  call vert_QA_W(ntry, wf4(:,3), ex2, wf8(:,6), n3(:,13), t3x8(:,:,6))
  call vert_QA_Z(gZd,ntry, ex3, ex4, wf4(:,15), n3(:,14), t3x4(:,:,8))
  call vert_QA_Z(gZu,ntry, wf4(:,9), ex2, wf8(:,7), n3(:,15), t3x8(:,:,7))
  call vert_SSV_V(ntry, ex5, ex6, wf4(:,15), wf4(:,16), n4(:,4), t4x4(:,:,4))
  call vert_QA_Z(gZu,ntry, ex1, wf4(:,13), wf8(:,8), n3(:,16), t3x8(:,:,8))
  call vert_SS_S(ntry, ex5, ex6, wf1(:,1), n3(:,17), t3x1(:,:,1))
  call vert_SV_V(ntry, wf1(:,1), wf4(:,1), wf4(:,17), n3(:,18), t3x4(:,:,9))
  call vert_TV_S(ntry, ex5, Q(:,16), wf4(:,1), Q(:,3), wf4(:,18), n3(:,19), t3x4(:,:,10))
  call vert_TV_S(ntry, ex6, Q(:,32), wf8(:,1), Q(:,76), wf8(:,9), n3(:,20), t3x8(:,:,9))
  call vert_SV_V(ntry, ex5, wf4(:,1), wf4(:,19), n3(:,21), t3x4(:,:,11))
  call vert_SV_V(ntry, ex6, wf8(:,1), wf8(:,10), n3(:,22), t3x8(:,:,10))
  call vert_TV_S(ntry, ex6, Q(:,32), wf4(:,1), Q(:,3), wf4(:,20), n3(:,23), t3x4(:,:,12))
  call vert_TV_S(ntry, ex5, Q(:,16), wf8(:,1), Q(:,76), wf8(:,11), n3(:,24), t3x8(:,:,11))
  call vert_SV_V(ntry, ex6, wf4(:,1), wf4(:,21), n3(:,25), t3x4(:,:,13))
  call vert_SV_V(ntry, ex5, wf8(:,1), wf8(:,12), n3(:,26), t3x8(:,:,12))
  call vert_TV_S(ntry, ex6, Q(:,32), wf8(:,2), Q(:,76), wf8(:,13), n3(:,27), t3x8(:,:,13))
  call vert_SV_V(ntry, ex6, wf8(:,2), wf8(:,14), n3(:,28), t3x8(:,:,14))
  call vert_TV_S(ntry, ex5, Q(:,16), wf8(:,2), Q(:,76), wf8(:,15), n3(:,29), t3x8(:,:,15))
  ...
  call vert_TV_S(ntry, ex6, Q(:,32), wf4(:,15), Q(:,12), wf4(:,33), n3(:,58), t3x4(:,:,25))
  call vert_TV_S(ntry, ex5, Q(:,16), wf8(:,7), Q(:,67), wf8(:,33), n3(:,59), t3x8(:,:,33))
  call vert_SV_V(ntry, ex6, wf4(:,15), wf4(:,34), n3(:,60), t3x4(:,:,26))
  call vert_SV_V(ntry, ex5, wf8(:,7), wf8(:,34), n3(:,61), t3x8(:,:,34))
  call vert_TV_S(ntry, ex5, Q(:,16), wf4(:,15), Q(:,12), wf4(:,35), n3(:,62), t3x4(:,:,27))
  call vert_TV_S(ntry, ex6, Q(:,32), wf8(:,7), Q(:,67), wf8(:,35), n3(:,63), t3x8(:,:,35))
  call vert_SV_V(ntry, ex5, wf4(:,15), wf4(:,36), n3(:,64), t3x4(:,:,28))
  call vert_SV_V(ntry, ex6, wf8(:,7), wf8(:,36), n3(:,65), t3x8(:,:,36))
  call vert_TV_S(ntry, ex5, Q(:,16), wf8(:,8), Q(:,67), wf8(:,37), n3(:,66), t3x8(:,:,37))
  call vert_SV_V(ntry, ex5, wf8(:,8), wf8(:,38), n3(:,67), t3x8(:,:,38))
  call vert_TV_S(ntry, ex6, Q(:,32), wf8(:,8), Q(:,67), wf8(:,39), n3(:,68), t3x8(:,:,39))
  ...
  if ( JBmunu /= 0 ) then ! POWHEG's Bmunu helicity correlation
    ...
    M2munu = M2munu / average_factor_pphhjj_uuxddxhhg_1
  end if

  ! Colour and helicity average and symmetry factor of outgoing particles
  M2add = M2 / average_factor_pphhjj_uuxddxhhg_1

  do k = 0, 30-1
    M2(k) = M2add(extcomb_perm_pphhjj_uuxddxhhg_1(k))
  end do
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call cont_VV(nsync, wf8(:,5), wf4(:,14), A(:,5), n3(:,74), t3x32(:,:,5), nhel, den(20))
    call cont_VV(nsync, wf4(:,14), wf8(:,6), A(:,6), n3(:,75), t3x32(:,:,6), nhel, den(22))
    ...
    call cont_SS(nsync, wf4(:,33), wf8(:,37), A(:,45), n3(:,114), t3x32(:,:,45), nhel, den(72))
    call cont_VV(nsync, wf4(:,34), wf8(:,38), A(:,46), n3(:,115), t3x32(:,:,46), nhel, den(72))
    call cont_SS(nsync, wf4(:,35), wf8(:,39), A(:,47), n3(:,116), t3x32(:,:,47), nhel, den(73))
    call cont_VV(nsync, wf4(:,36), wf8(:,40), A(:,48), n3(:,117), t3x32(:,:,48), nhel, den(73))
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = (A(j,41)%j+A(j,43)%j+A(j,45)%j+A(j,47)%j)*f(5)+(A(j,7)%j+A(j,8)%j)*f(6)+(A(j,39)%j+A(j,40)%j)*f(7)+(A(j,42)%j+A(j,44)%j &
       +A(j,46)%j+A(j,48)%j)*f(8)
  M1(2) = (-A(j,31)%j-A(j,33)%j-A(j,35)%j-A(j,37)%j)*f(1)+(-A(j,5)%j-A(j,6)%j)*f(2)+(-A(j,29)%j-A(j,30)%j)*f(3)+(-A(j,32)%j &
       -A(j,34)%j-A(j,36)%j-A(j,38)%j)*f(4)
  M1(3) = (-A(j,21)%j-A(j,23)%j-A(j,25)%j-A(j,27)%j)*f(1)+(-A(j,3)%j-A(j,4)%j)*f(2)+(-A(j,19)%j-A(j,20)%j)*f(3)+(-A(j,22)%j &
       -A(j,24)%j-A(j,26)%j-A(j,28)%j)*f(4)
  M1(4) = (A(j,11)%j+A(j,13)%j+A(j,15)%j+A(j,17)%j)*f(5)+(A(j,1)%j+A(j,2)%j)*f(6)+(A(j,9)%j+A(j,10)%j)*f(7)+(A(j,12)%j+A(j,14)%j &
       +A(j,16)%j+A(j,18)%j)*f(8)
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_pphhjj_uuxddxhhg_1_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_pphhjj_uuxddxhhg_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_pphhjj_uuxddxhhg_1")
#else
subroutine amp2tree(P, M2)
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_pphhjj_uuxddxhhg_1")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_pphhjj_uuxddxhhg_1")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_pphhjj_uuxddxhhg_1")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_pphhjj_uuxddxhhg_1")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_pphhjj_uuxddxhhg_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_pphhjj_uuxddxhhg_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_pphhjj_uuxddxhhg_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_pphhjj_uuxddxhhg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_pphhjj_uuxddxhhg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2tree_legacy(p, m2) &
    & bind(c,name="amp2tree_pphhjj_uuxddxhhg_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccone_legacy(p, m2, i, j) &
    & bind(c,name="amp2ccone_pphhjj_uuxddxhhg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccall_legacy(p, m2) &
    & bind(c,name="amp2ccall_pphhjj_uuxddxhhg_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(0:30-1)
...
subroutine amp2hcone_legacy(p, m2, i, j, mom) &
    & bind(c,name="amp2hcone_pphhjj_uuxddxhhg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_legacy(p, m2, i, mom) &
    & bind(c,name="amp2hcall_pphhjj_uuxddxhhg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(7)
...
end module ol_tree_pphhjj_uuxddxhhg_1_/**/REALKIND
