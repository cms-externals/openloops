...
module ol_colourmatrix_ppzwjj_uxccxdzwxg_1_/**/REALKIND
...
end module ol_colourmatrix_ppzwjj_uxccxdzwxg_1_/**/REALKIND



module ol_forced_parameters_ppzwjj_uxccxdzwxg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppzwjj_uxccxdzwxg_1_/**/REALKIND

module ol_tree_ppzwjj_uxccxdzwxg_1_/**/REALKIND
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppzwjj_uxccxdzwxg_1")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppzwjj_uxccxdzwxg_1_/**/DREALKIND, only: ntry, nhel, Hel, M1helarr
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for anti-up charm anti-charm down Z W+ glue -> 0
...
! **********************************************************************
  ...
  use ol_external_ppzwjj_uxccxdzwxg_1, only: external_perm_ppzwjj_uxccxdzwxg_1, &
    & external_perm_inv_ppzwjj_uxccxdzwxg_1, extcomb_perm_ppzwjj_uxccxdzwxg_1, &
    & average_factor_ppzwjj_uxccxdzwxg_1
  use ol_external_ppzwjj_uxccxdzwxg_1, only: H, hel_not_initialised, hel_init
  use ol_colourmatrix_ppzwjj_uxccxdzwxg_1_/**/REALKIND, only: colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppzwjj_uxccxdzwxg_1_/**/REALKIND, only: check_forced_parameters
  use ol_heltables_ppzwjj_uxccxdzwxg_1
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_ppzwjj_uxccxdzwxg_1,7)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppzwjj_uxccxdzwxg_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppzwjj_uxccxdzwxg_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_ppzwjj_uxccxdzwxg_1(extcombs(k))
  end do
  ...
  call vert_QA_V(ntry, ex4, wf18(:,2), wf36(:,2), n3(:,16), t3x36(:,:,2))
  ...
  call vert_QA_V(ntry, ex4, wf18(:,6), wf36(:,5), n3(:,35), t3x36(:,:,5))
  ...
  call vert_QA_V(ntry, ex4, wf18(:,12), wf36(:,9), n3(:,46), t3x36(:,:,9))
  ...
  call vert_VQ_A(ntry, wf8(:,6), ex4, wf16(:,5), n3(:,53), t3x16(:,:,5))
  call vert_VQ_A(ntry, wf8(:,7), ex4, wf16(:,6), n3(:,54), t3x16(:,:,6))
  call vert_ZQ_A(gZu,ntry, ex5, ex2, wf6(:,9), n3(:,55), t3x6(:,:,5))
  call prop_Q_A(ntry, wf6(:,9), Q(:,18), ZERO, 0_intkind1, wf6(:,10), n2(26))
  call vert_QA_V(ntry, ex4, wf6(:,7), wf12(:,17), n3(:,56), t3x12(:,:,9))
  call vert_QA_V(ntry, wf6(:,10), wf4(:,9), wf24(:,9), n3(:,57), t3x24(:,:,9))
  call vert_QA_V(ntry, wf6(:,10), ex3, wf12(:,18), n3(:,58), t3x12(:,:,10))
  call vert_QA_V(ntry, wf4(:,3), wf6(:,7), wf24(:,10), n3(:,59), t3x24(:,:,10))
  ...
  call vert_VQ_A(ntry, wf12(:,18), ex4, wf24(:,13), n3(:,63), t3x24(:,:,13))
  call vert_QA_V(ntry, wf6(:,4), wf4(:,5), wf24(:,14), n3(:,64), t3x24(:,:,14))
  call vert_QA_V(ntry, wf6(:,4), ex1, wf12(:,21), n3(:,65), t3x12(:,:,12))
  call vert_AV_Q(ntry, ex3, wf12(:,21), wf24(:,15), n3(:,66), t3x24(:,:,15))
  call vert_AV_Q(ntry, ex1, wf12(:,18), wf24(:,16), n3(:,67), t3x24(:,:,16))
  call vert_UV_W(ntry, wf12(:,21), Q(:,41), ex7, Q(:,64), wf24(:,17), n3(:,68), t3x24(:,:,17))
  call vert_AZ_Q(gZu,ntry, ex3, ex5, wf6(:,11), n3(:,69), t3x6(:,:,6))
  ...
  call vert_VQ_A(ntry, wf12(:,22), ex4, wf24(:,21), n3(:,75), t3x24(:,:,21))
  call vert_VQ_A(ntry, wf12(:,21), ex2, wf24(:,22), n3(:,76), t3x24(:,:,22))
  call vert_AV_Q(ntry, ex1, wf12(:,22), wf24(:,23), n3(:,77), t3x24(:,:,23))
  call vert_AV_Q(ntry, ex1, wf8(:,6), wf16(:,7), n3(:,78), t3x16(:,:,7))
  call vert_AV_Q(ntry, ex1, wf8(:,7), wf16(:,8), n3(:,79), t3x16(:,:,8))
  call vert_ZQ_A(gZu,ntry, ex5, wf4(:,7), wf12(:,25), n3(:,80), t3x12(:,:,15))
  call prop_Q_A(ntry, wf12(:,25), Q(:,82), ZERO, 0_intkind1, wf12(:,26), n2(30))
  call vert_AZ_Q(gZu,ntry, wf4(:,9), ex5, wf12(:,27), n3(:,81), t3x12(:,:,16))
  ...
  M2add = M2 / average_factor_ppzwjj_uxccxdzwxg_1

  do k = 0, 30-1
    M2(k) = M2add(extcomb_perm_ppzwjj_uxccxdzwxg_1(k))
  end do
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = ((-A(j,27)%j-A(j,28)%j-A(j,34)%j-A(j,79)%j-A(j,80)%j)*f(1))/2._/**/REALKIND+(CI*(A(j,32)%j &
       +A(j,33)%j)*f(2))/2._/**/REALKIND+((-A(j,1)%j-A(j,6)%j-A(j,8)%j-A(j,9)%j-A(j,12)%j-A(j,13)%j-A(j,19)%j-A(j,21)%j-A(j,22)%j &
       -A(j,23)%j-A(j,35)%j-A(j,36)%j-A(j,39)%j-A(j,41)%j-A(j,46)%j-A(j,47)%j-A(j,48)%j-A(j,50)%j-A(j,53)%j-A(j,55)%j-A(j,59)%j &
       -A(j,60)%j-A(j,61)%j-A(j,65)%j-A(j,67)%j-A(j,69)%j-A(j,71)%j-A(j,72)%j-A(j,75)%j-A(j,76)%j)*f(3))/2._/**/REALKIND &
       +(CI*(A(j,2)%j+A(j,7)%j+A(j,10)%j+A(j,16)%j+A(j,20)%j+A(j,26)%j+A(j,45)%j-A(j,52)%j+A(j,57)%j &
       -A(j,64)%j)*f(4))/2._/**/REALKIND
  M1(2) = ((A(j,27)%j+A(j,28)%j+A(j,29)%j+A(j,30)%j+A(j,31)%j+A(j,34)%j)*f(1))/6._/**/REALKIND+((A(j,1)%j+A(j,3)%j+A(j,4)%j &
  ...
  M1(3) = ((A(j,79)%j+A(j,80)%j+A(j,81)%j+A(j,82)%j)*f(1))/6._/**/REALKIND+((A(j,39)%j+A(j,40)%j+A(j,41)%j+A(j,42)%j+A(j,43)%j &
  ...
  M1(4) = ((-A(j,29)%j-A(j,30)%j-A(j,31)%j-A(j,81)%j-A(j,82)%j)*f(1))/2._/**/REALKIND+(CI*(-A(j,32)%j &
       -A(j,33)%j)*f(2))/2._/**/REALKIND+((-A(j,3)%j-A(j,4)%j-A(j,5)%j-A(j,11)%j-A(j,14)%j-A(j,15)%j-A(j,17)%j-A(j,18)%j-A(j,24)%j &
       -A(j,25)%j-A(j,37)%j-A(j,38)%j-A(j,40)%j-A(j,42)%j-A(j,43)%j-A(j,44)%j-A(j,49)%j-A(j,51)%j-A(j,54)%j-A(j,56)%j-A(j,58)%j &
       -A(j,62)%j-A(j,63)%j-A(j,66)%j-A(j,68)%j-A(j,70)%j-A(j,73)%j-A(j,74)%j-A(j,77)%j-A(j,78)%j)*f(3))/2._/**/REALKIND+(CI*( &
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppzwjj_uxccxdzwxg_1_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppzwjj_uxccxdzwxg_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppzwjj_uxccxdzwxg_1")
#else
subroutine amp2tree(P, M2)
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppzwjj_uxccxdzwxg_1")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppzwjj_uxccxdzwxg_1")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppzwjj_uxccxdzwxg_1")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppzwjj_uxccxdzwxg_1")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppzwjj_uxccxdzwxg_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppzwjj_uxccxdzwxg_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppzwjj_uxccxdzwxg_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppzwjj_uxccxdzwxg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppzwjj_uxccxdzwxg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2tree_legacy(p, m2) &
    & bind(c,name="amp2tree_ppzwjj_uxccxdzwxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccone_legacy(p, m2, i, j) &
    & bind(c,name="amp2ccone_ppzwjj_uxccxdzwxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccall_legacy(p, m2) &
    & bind(c,name="amp2ccall_ppzwjj_uxccxdzwxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(0:30-1)
...
subroutine amp2hcone_legacy(p, m2, i, j, mom) &
    & bind(c,name="amp2hcone_ppzwjj_uxccxdzwxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_legacy(p, m2, i, mom) &
    & bind(c,name="amp2hcall_ppzwjj_uxccxdzwxg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(7)
...
end module ol_tree_ppzwjj_uxccxdzwxg_1_/**/REALKIND
