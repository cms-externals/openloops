...
#endif
  use ol_loop_eehtt_eexttxh_1_/**/REALKIND, only: c, f, wf, den
...
if (mode == -1) return
  ...
  call loop_A_Q(G0(:,:,:,4),Q(:,8),MT,G1(:,:,:,7))
  call loop_AS_Q(G1(:,:,:,7),wf(:,-4),G1(:,:,:,8),gH)
  call loop_A_Q(G1(:,:,:,8),Q(:,24),MT,G2(:,:,:,5))
