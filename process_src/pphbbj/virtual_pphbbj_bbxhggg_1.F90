...
module ol_vamp_pphbbj_bbxhggg_1_/**/REALKIND
  contains
...
! **********************************************************************
  ...
  use ol_vamp_1_pphbbj_bbxhggg_1_/**/REALKIND, only: vamp_1
  use ol_vamp_2_pphbbj_bbxhggg_1_/**/REALKIND, only: vamp_2
  use ol_vamp_3_pphbbj_bbxhggg_1_/**/REALKIND, only: vamp_3
  use ol_vamp_4_pphbbj_bbxhggg_1_/**/REALKIND, only: vamp_4
  use ol_vamp_5_pphbbj_bbxhggg_1_/**/REALKIND, only: vamp_5
  use ol_vamp_6_pphbbj_bbxhggg_1_/**/REALKIND, only: vamp_6
  use ol_vamp_7_pphbbj_bbxhggg_1_/**/REALKIND, only: vamp_7
  use ol_vamp_8_pphbbj_bbxhggg_1_/**/REALKIND, only: vamp_8
...
! M2tree = helicity-summed squared tree matrix element for bottom anti-bottom higgs glue glue glue -> 0
! M2loop0 = helicity-summed squared loop matrix element for bottom anti-bottom higgs glue glue glue -> 0
...
! **********************************************************************
  ...
  use ol_external_pphbbj_bbxhggg_1, only: &
    & external_perm_inv_pphbbj_bbxhggg_1, &
    & average_factor_pphbbj_bbxhggg_1, &
    external_perm_pphbbj_bbxhggg_1
  use ol_external_pphbbj_bbxhggg_1, only: &
    & hel_not_initialised, hel_init, H, H_HC, POLSEL
  use ol_colourmatrix_pphbbj_bbxhggg_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  ...
  use ol_forced_parameters_pphbbj_bbxhggg_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_momenta_decl_/**/REALKIND, only: QInvariantsMatrix
  use ol_tensor_sum_storage_pphbbj_bbxhggg_1_/**/REALKIND, only: &
    & reset_tensor_sum, integrate_tensor_sum, scale_tensor_sum
  use ol_loop_pphbbj_bbxhggg_1_/**/REALKIND, only: &
    & fac_status_loop1, fac_status_loop2, zerohel, &
    & fac_init_loop, tree_wavefunctions
  use ol_loop_pphbbj_bbxhggg_1_/**/DREALKIND, only: &
    & M0_cache => M0, Mct_cache => Mct, Mcol_loop_cache => Mcol_loop
  ...
  complex(REALKIND)    :: M0_col1(11)
  complex(REALKIND)    :: M0(6,32), Mct(11,32), Mcol_loop(11,32)
  real(REALKIND)       :: M2colint(22), M2CC(6,6), M2CC_EW ! colour correlations
  real(REALKIND)       :: M2L2colint(0:22), M2L2CC(6,6)
  integer,        save :: ntry = 1
  ...
  extmasses2 = [ rMB2, rMB2, rMH2, rZERO2, rZERO2, rZERO2 ]
  call conv_mom_scatt2in(P_scatt, extmasses2, P, &
     & external_perm_inv_pphbbj_bbxhggg_1, 6)
  ! internal-propagator momenta in light-cone representation
  call internal_momenta(P, 6)

#ifdef LOOPSQUARED
  call integrate_tensor_sum(M2L2)
#else
  ...
  do
...
#if 6 > 0
    do j = 1, 6
      M2L0 = M2L0 + real(sum(conjg(M0(:,la))*K1(1:6,j))*M0(j,la))
    end do
#endif
#if 11 > 0
    do j = 1, 11
      M2ct = M2ct + real(sum(conjg(M0(:,la))*K2(1:6,j))*Mct(j,la))
    end do
#endif
    do j = 1, 11
      M0_col1(j) = sum(conjg(M0(:,la))*KL(1:6,j))
    end do
#if 23 > 1
    if (IR_is_on > 0) then
      do j = 1, 23-1
    ...
    M2hel = 0
    do j = 1, 11
      M2hel = M2hel + real(sum(conjg(Mcol_loop(:,la))*KL2(1:11,j))*Mcol_loop(j,la))
    end do
    M2L2 = M2L2 + M2hel
#if 11 > 0
    do j = 1, 11
      M2L2ct  = M2L2ct  + real(sum(conjg(Mcol_loop(:,la))*KL2ct(1:11,j))*Mct(j,la))
      M2L2ct2 = M2L2ct2 + real(sum(conjg(Mct(:,la))*KL2ct2(1:11,j))*Mct(j,la))
    end do
    if (IR_is_on > 0) then
#if 11 > 0
        call colint(Mcol_loop(:,la), Mcol_loop(:,la), 11, KL2, [(i, i = 0, 22-1)], M2L2colint)  ! <M1|M1>
        call colint(2*Mcol_loop(:,la), Mct(:,la), 11, KL2ct, [(i, i = 0, 22-1)], M2L2colint)    ! 2*<M1|Mct>
#endif
#if 11 > 0
        call colint(Mct(:,la), Mct(:,la), 11, KL2ct2, [(i, i = 0, 22-1)], M2L2colint)            ! <Mct|Mct>
#endif
    end if
...
! #ifdef LOOPSQUARED
  ...
  if (IR_is_on > 0) then
    ...
    call intdip(ioperator_mode, M2L0, M2CC, M2CC_EW, [2,2,3,1,1,1], [-1,1,0,0,0,0]/3._/**/REALKIND, &
      & 6, extmasses2, QInvariantsMatrix, vdip, c_dip)
    IRL1(0) = c_dip(0) / average_factor_pphbbj_bbxhggg_1
    IRL1(1) = c_dip(1) / average_factor_pphbbj_bbxhggg_1
    IRL1(2) = c_dip(2) / average_factor_pphbbj_bbxhggg_1
  else
    vdip = 0
    IRL1 = 0
  end if

  ! loop^2 IR contributions
#ifdef LOOPSQUARED
  if (IR_is_on > 0) then
    do i = 1, 6
      do j = 1, i
        M2L2CC(i,j) = M2L2colint(i*(i-1)/2+j)
      end do
    end do
    do j = 2, 6
      do i = 1, j-1
        M2L2CC(i,j) = M2L2CC(j,i)
      end do
    end do
    M2CC_EW = M2L2colint(6*(6+1)/2+1)
    call intdip(ioperator_mode, M2L2, M2L2CC, M2CC_EW, [2,2,3,1,1,1], [-1,1,0,0,0,0]/3._/**/REALKIND, &
      & 6, extmasses2, QInvariantsMatrix, vdip, c_dip)
    IRL2(0) = c_dip(0) / average_factor_pphbbj_bbxhggg_1
    IRL2(1) = c_dip(1) / average_factor_pphbbj_bbxhggg_1
    IRL2(2) = c_dip(2) / average_factor_pphbbj_bbxhggg_1
  else
    IRL2 = 0
  end if
#endif
  ...
  M2L0 = M2L0 / average_factor_pphbbj_bbxhggg_1
  M2L1 = 2*(M2L1 + M2ct)
  if (IR_is_on > 1) then
    M2L1 = M2L1 + vdip
  end if
  M2L1 = M2L1 / average_factor_pphbbj_bbxhggg_1
  ...
  scalebackfactor = scalefactor**(6-4)
  M0_cache = scalebackfactor * M0
  Mct_cache = scalebackfactor * Mct
  Mcol_loop_cache = scalebackfactor *Mcol_loop
...
! P_scatt(0:3,Npart) = external momenta
! M2L0 = helicity-summed squared tree matrix element for bottom anti-bottom higgs glue glue glue -> 0
! M2L2 = helicity-summed tree x loop matrix element for bottom anti-bottom higgs glue glue glue -> 0
! M2L2 = helicity-summed squared loop matrix element for bottom anti-bottom higgs glue glue glue -> 0
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_debug, only: ol_fatal
  use ol_external_pphbbj_bbxhggg_1, only: H, H_HC, &
    average_factor_pphbbj_bbxhggg_1, &
    external_perm_pphbbj_bbxhggg_1, &
    extcomb_perm_pphbbj_bbxhggg_1
  use ol_colourmatrix_pphbbj_bbxhggg_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  use ol_parameters_decl_/**/REALKIND ! parameters_status, scalefactor, <masses>
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_loop_pphbbj_bbxhggg_1_/**/REALKIND, only: zerohel, M0, Mct, Mcol_loop
  use ol_wavefunctions_/**/REALKIND, only: wf_V_Std
  use ol_kinematics_/**/REALKIND, only: LC2Std_Rep_cmplx
  ...
  integer,        save :: ntry = 1
  real(REALKIND)       :: M2L1cc(0:22),M2L2cc(0:22)
  integer              :: cr, k, m, n, l, hels(2,4)
  integer              :: extcombs(22)
  ...
  if (cr == 1 .or. cr == 11) then
    extcombs = [(i, i = 0, 22-1)]
    nextcombs = 6*(6+1)/2+1
  else if (cr == 2 .or. cr == 12) then
  ...
  emitter = external_perm_pphbbj_bbxhggg_1(corr%emitter)
  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_pphbbj_bbxhggg_1(extcombs(k))
  end do
  ...
  if (cr == 1) then
    ...
    do la=1,32
#if 11 > 0
        call colint(Mcol_loop(:,la), Mcol_loop(:,la), 11, KL2, extcombs_permuted, M2L2cc)  ! <M1|M1>
        call colint(2*Mcol_loop(:,la), Mct(:,la), 11, KL2ct, extcombs_permuted, M2L2cc)    ! 2*<M1|Mct>
#endif
#if 11 > 0
        call colint(Mct(:,la), Mct(:,la), 11, KL2ct2, extcombs_permuted, M2L2cc)            ! <Mct|Mct>
#endif
    end do
    do k = 0, 22
      corr%rescc(k) = M2L2cc(extcomb_perm_pphbbj_bbxhggg_1(k)) / &
                      average_factor_pphbbj_bbxhggg_1
    end do
    M2L2 = corr%rescc(0)
...
#if 11 > 0
          call colint(Mcol_loop(:,hels(1,l)), om*Mcol_loop(:,hels(2,l)), 11, KL2, extcombs_permuted, M2L2cc) ! <M1|M1>
          call colint(Mcol_loop(:,hels(1,l)), om*Mct(:,hels(2,l)), 11, KL2ct, extcombs_permuted, M2L2cc)     ! <M1|Mct>
          call colint(om*Mcol_loop(:,hels(1,l)), Mct(:,hels(2,l)), 11, KL2ct, extcombs_permuted, M2L2cc)     ! <Mct|M1>
#endif
#if 11 > 0
          call colint(Mct(:,hels(1,l)), om*Mct(:,hels(2,l)), 11, KL2ct2, extcombs_permuted, M2L2cc)           ! <Mct|Mct>
#endif
      end do
    end do

    do k = 0, 22
      corr%rescc(k) = 0.5_/**/REALKIND*M2L2cc(extcomb_perm_pphbbj_bbxhggg_1(k)) / &
                      average_factor_pphbbj_bbxhggg_1
    end do
  ...
  ! get wfs
    ...
    do k=1,32,2
      ...
      do l=1,size(hels,2)
#if 11 > 0
        call colint(Mcol_loop(:,hels(1,l)), Mcol_loop(:,hels(2,l)), 11, KL2(1:11,:), [0], M2hels(l:l))  ! <M1|M1>
        call colint(2*Mcol_loop(:,hels(1,l)), Mct(:,hels(2,l)), 11, KL2ct(1:11,:), [0], M2hels(l:l))    ! <M1|Mct>
#endif
#if 11 > 0
        call colint(Mct(:,hels(1,l)), Mct(:,hels(2,l)), 11, KL2ct2(1:11,:), [0], M2hels(l:l))             ! <Mct|Mct>
#endif
    ...
    if (any(corr%resmunu(:,:) /= corr%resmunu(:,:))) then
      ...
      corr%resmunu = corr%resmunu / average_factor_pphbbj_bbxhggg_1
      M2L2 = -corr%resmunu(1,1)+corr%resmunu(2,2)+corr%resmunu(3,3)+corr%resmunu(4,4) ! |M1|^2 = -B^mu_mu
    end if
  else if (cr == 11) then
    do la=1,32
#if 6 > 0
        call colint(2*M0(:,la), Mcol_loop(:,la), 6, KL, extcombs_permuted, M2L1cc)  ! <M0|M1>
#endif
#if 6 > 0
        call colint(2*M0(:,la), Mct(:,la), 6, K2, extcombs_permuted, M2L1cc)    ! <M0|Mct>
#endif
    end do
    do k = 0, 22
      corr%rescc(k) = M2L1cc(extcomb_perm_pphbbj_bbxhggg_1(k)) / average_factor_pphbbj_bbxhggg_1
    end do
    M2L1 = corr%rescc(0)
...
#if 6 > 0
        call colint(2*M0(:,hels(1,l)), om*Mcol_loop(:,hels(2,l)), 6, KL(1:6,:), extcombs_permuted, M2L2cc)  ! <M0|M1>
        call colint(2*M0(:,hels(1,l)), om*Mct(:,hels(2,l)), 6, K2(1:6,:), extcombs_permuted, M2L2cc)    ! <M0|Mct>
#endif
      end do
    end do
    ...
    do k=1,32,2
      ...
      do l=1,size(hels,2)
#if 6 > 0
        call colint(2*M0(:,hels(1,l)), Mcol_loop(:,hels(2,l)), 6, KL(1:6,:), [0], M2hels(l:l))  ! <M0|M1>
        call colint(2*M0(:,hels(1,l)), Mct(:,hels(2,l)), 6, K2(1:6,:), [0], M2hels(l:l))    ! <M0|Mct>
#endif
      ! V^(mu,nu) = sum_(k) sum_(l,-l) M^*_(k) M_(k) (eps^(mu)_(l))^* eps^(nu)_(-l)
        do m = 1,4
          do n = 1,4
            corr%resmunu(m,n) = corr%resmunu(m,n) + M2hels(l)*conjg(epStd(m,(H(emitter,hels(1,l))+1)/2+1))* &
                                                 & epStd(n,(H(emitter,hels(2,l))+1)/2+1)
  ...
  if(allocated(extcombs_permuted)) deallocate(extcombs_permuted)
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_external_pphbbj_bbxhggg_1, only: hel_not_initialised, hel_init, H
  use ol_colourmatrix_pphbbj_bbxhggg_1_/**/REALKIND, only: &
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  use ol_external_pphbbj_bbxhggg_1, only: &
    & external_perm_inv_pphbbj_bbxhggg_1, average_factor_pphbbj_bbxhggg_1
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_kinematics_/**/REALKIND, only: conv_mom_scatt2in, internal_momenta
  use ol_forced_parameters_pphbbj_bbxhggg_1_/**/REALKIND, only: check_forced_parameters
  use ol_loop_pphbbj_bbxhggg_1_/**/REALKIND, only: &
...
!  integer           :: DOI_bak
  ...
  ! Convert 2 -> n-2 PS-point to n -> 0 (so that P(1) + ... + P(n) = 0)
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_pphbbj_bbxhggg_1, 6)
  ! internal-propagator momenta in light-cone representation
  call internal_momenta(P, 6)
...
#if 6 > 0
    do j = 1, 6
      M2tree = M2tree + real(sum(conjg(M0(:,la))*K1(1:6,j))*M0(j,la))
    end do
#endif
#if 11 > 0
    do j = 1, 11
      M2ct = M2ct + real(sum(conjg(M0(:,la))*K2(1:6,j))*Mct(j))
    end do
#endif
  end do
  ...
  ! call loop_parameters_init() is done in evaluate_ct().

  scalebackfactor = scalefactor**(2*6-8)
  M2tree = scalebackfactor * M2tree / average_factor_pphbbj_bbxhggg_1
  M2ct   = scalebackfactor * 2*M2ct / average_factor_pphbbj_bbxhggg_1
...
end module ol_vamp_pphbbj_bbxhggg_1_/**/REALKIND



#ifdef PRECISION_dp

module ol_vamp_pphbbj_bbxhggg_1
  use ol_parameters_decl_/**/DREALKIND, only: procname_length
  use ol_data_types_/**/DREALKIND, only: me_cache
  implicit none
  character(procname_length) :: processname = 'pphbbj_bbxhggg_1'
  ...
  subroutine finish_pphbbj_bbxhggg_1()
    ! final update of the stability histogram
    use KIND_TYPES, only: DREALKIND
    use ol_stability, only: finish_histograms
    use ol_vamp_pphbbj_bbxhggg_1_/**/DREALKIND, only: vamp
    implicit none
    complex(DREALKIND) :: Mdummy(11)
    integer :: k
  ...
  end subroutine finish_pphbbj_bbxhggg_1


  subroutine vamp2(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_f_vamp2_pphbbj_bbxhggg_1")
    use KIND_TYPES, only: DREALKIND
    use ol_init, only: register_cleanup
    use ol_external_pphbbj_bbxhggg_1, only: external_perm_pphbbj_bbxhggg_1
    use ol_vamp_pphbbj_bbxhggg_1_/**/DREALKIND, only: vamp2dp => vamp2pc
#ifdef USE_qp
    use ol_vamp_pphbbj_bbxhggg_1_/**/QREALKIND, only: vamp2qp => vamp2pc
#endif
...
#endif
    if (first_call) then
      call register_cleanup(finish_pphbbj_bbxhggg_1)
      first_call = .false.
    end if
    call vamp2generic(vamp2dp, vamp2qp, processname, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, &
                    ...
                    & external_perm_pphbbj_bbxhggg_1, me_caches)
...
! **********************************************************************
  subroutine vampcr2(P_scatt, M2L0, M2L1, M2L2, crtype, &
      ...
      & bind(c,name="ol_f_vampcr2_pphbbj_bbxhggg_1")
...
! **********************************************************************
    use KIND_TYPES, only: DREALKIND
    use ol_external_pphbbj_bbxhggg_1, only: external_perm_pphbbj_bbxhggg_1
    use ol_vamp_pphbbj_bbxhggg_1_/**/DREALKIND, only: vamp2cache
    use ol_data_types_/**/DREALKIND, only: correlator
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,6)
    ...
    real(REALKIND),  intent(out) :: M2cc(0:6*(6+1)/2+1)
    real(DREALKIND), intent(out) :: M2munu(4,4)
    integer :: n_cc = 6*(6+1)/2
    ...
    if (crtype == 0) then
      ...
      allocate(corr%rescc(0:6*(6+1)/2+1))
      ...
      call vamp2cache(P_scatt, M2L0, M2L1, M2L2, corr=corr)
      M2munu=corr%resmunu
    else if (crtype == 11) then
      allocate(corr%rescc(0:6*(6+1)/2+1))
      call vamp2cache(P_scatt, M2L0, M2L1, M2L2, corr=corr)
      M2cc = corr%rescc
      deallocate(corr%rescc)
    else if (crtype == 12) then
      corr%emitter=emitter
      corr%mom=mom
      corr%nextcombs=nextcombs
      allocate(corr%extcombs(nextcombs))
  ...
  subroutine ctamp2(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_f_ctamp2_pphbbj_bbxhggg_1")
    use KIND_TYPES, only: DREALKIND
    use ol_vamp_pphbbj_bbxhggg_1_/**/DREALKIND, only: ctamp2base
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine vamp2_c(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_vamp2_pphbbj_bbxhggg_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine ctamp2_c(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_ctamp2_pphbbj_bbxhggg_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine vamp2_legacy(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="vamp2_pphbbj_bbxhggg_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
  ...
  subroutine ctamp2_legacy(P_scatt, M2tree, M2ct) &
      & bind(c,name="ctamp2_pphbbj_bbxhggg_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,6)
...
end module ol_vamp_pphbbj_bbxhggg_1
