...
module ol_colourmatrix_ppllttj_eexuuuxuxttx_1_/**/REALKIND
...
end module ol_colourmatrix_ppllttj_eexuuuxuxttx_1_/**/REALKIND



module ol_forced_parameters_ppllttj_eexuuuxuxttx_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllttj_eexuuuxuxttx_1_/**/REALKIND

module ol_tree_ppllttj_eexuuuxuxttx_1_/**/REALKIND
...
subroutine born_denominators_init()
  ...
  den(6) = 1 / (Q(5,148) - MT2)
  den(10) = 1 / (Q(5,3) - MZ2)
  den(14) = 1 / (Q(5,168) - MT2)
  den(21) = 1 / (Q(5,60))
  den(25) = 1 / (Q(5,84) - MT2)
  den(27) = 1 / (Q(5,131) - MT2)
  den(33) = 1 / (Q(5,104) - MT2)
  den(48) = 1 / (Q(5,11))
  den(50) = 1 / (Q(5,52))
  den(52) = 1 / (Q(5,192))
  den(59) = 1 / (Q(5,212))
  ...
  den(116) = 1 / (Q(5,36))
  den(117) = 1 / (Q(5,24))
  den(118) = 1 / (Q(5,164) - MT2)
  den(124) = 1 / (Q(5,152) - MT2)
  den(134) = 1 / (Q(5,100) - MT2)
  den(139) = 1 / (Q(5,88) - MT2)
  den(155) = 1 / (Q(5,7))
  den(157) = 1 / (Q(5,216))
  den(162) = 1 / (Q(5,56))
  den(179) = 1 / (Q(5,196))
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppllttj_eexuuuxuxttx_1")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppllttj_eexuuuxuxttx_1_/**/DREALKIND, only: ntry, nhel, Hel, M1helarr
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for e- e+ up up anti-up anti-up top anti-top -> 0
...
! **********************************************************************
  ...
  use ol_external_ppllttj_eexuuuxuxttx_1, only: external_perm_ppllttj_eexuuuxuxttx_1, &
    & external_perm_inv_ppllttj_eexuuuxuxttx_1, extcomb_perm_ppllttj_eexuuuxuxttx_1, &
    & average_factor_ppllttj_eexuuuxuxttx_1
  use ol_external_ppllttj_eexuuuxuxttx_1, only: H, hel_not_initialised, hel_init
  use ol_colourmatrix_ppllttj_eexuuuxuxttx_1_/**/REALKIND, only: colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppllttj_eexuuuxuxttx_1_/**/REALKIND, only: check_forced_parameters
  use ol_heltables_ppllttj_eexuuuxuxttx_1
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_ppllttj_eexuuuxuxttx_1,8)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppllttj_eexuuuxuxttx_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppllttj_eexuuuxuxttx_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_ppllttj_eexuuuxuxttx_1(extcombs(k))
  end do
  ...
  call prop_Q_A(ntry, wf8(:,1), Q(:,67), MT, 1_intkind1, wf8(:,3), n2(1))
  call prop_A_Q(ntry, wf8(:,2), Q(:,148), MT, 1_intkind1, wf8(:,4), n2(2))
  ...
  call vert_ZQ_A(gZu,ntry, wf4(:,5), ex7, wf8(:,5), n3(:,8), t3x8(:,:,3))
  call prop_Q_A(ntry, wf8(:,5), Q(:,67), MT, 1_intkind1, wf8(:,6), n2(4))
  call vert_VQ_A(ntry, wf4(:,3), wf8(:,6), wf32(:,2), n3(:,9), t3x32(:,:,2))
  call vert_AV_Q(ntry, ex8, wf4(:,3), wf8(:,7), n3(:,10), t3x8(:,:,4))
  call prop_A_Q(ntry, wf8(:,7), Q(:,168), MT, 1_intkind1, wf8(:,8), n2(5))
  ...
  call prop_Q_A(ntry, wf8(:,9), Q(:,84), MT, 1_intkind1, wf8(:,11), n2(6))
  call prop_A_Q(ntry, wf8(:,10), Q(:,131), MT, 1_intkind1, wf8(:,12), n2(7))
  call vert_VQ_A(ntry, wf4(:,3), wf8(:,11), wf32(:,5), n3(:,18), t3x32(:,:,5))
  call vert_AZ_Q(gZu,ntry, ex8, wf4(:,5), wf8(:,13), n3(:,19), t3x8(:,:,7))
  call prop_A_Q(ntry, wf8(:,13), Q(:,131), MT, 1_intkind1, wf8(:,14), n2(8))
  call vert_VQ_A(ntry, wf4(:,3), ex7, wf8(:,15), n3(:,20), t3x8(:,:,8))
  call prop_Q_A(ntry, wf8(:,15), Q(:,104), MT, 1_intkind1, wf8(:,16), n2(9))
  ...
  call prop_A_Q(ntry, wf8(:,33), Q(:,164), MT, 1_intkind1, wf8(:,34), n2(18))
  ...
  call prop_A_Q(ntry, wf8(:,35), Q(:,152), MT, 1_intkind1, wf8(:,36), n2(19))
  ...
  M2add = M2 / average_factor_ppllttj_eexuuuxuxttx_1

  do k = 0, 38-1
    M2(k) = M2add(extcomb_perm_ppllttj_eexuuuxuxttx_1(k))
  end do
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = ((-A(j,49)%j-A(j,57)%j-A(j,63)%j-A(j,67)%j-A(j,75)%j-A(j,79)%j-A(j,81)%j-A(j,87)%j-A(j,91)%j-A(j,95)%j-A(j,101)%j &
       -A(j,103)%j-A(j,109)%j-A(j,115)%j-A(j,119)%j-A(j,121)%j-A(j,125)%j-A(j,165)%j-A(j,175)%j-A(j,183)%j &
       -A(j,187)%j)*f(1))/4._/**/REALKIND+((-A(j,1)%j-A(j,3)%j-A(j,7)%j-A(j,9)%j-A(j,13)%j-A(j,15)%j-A(j,33)%j-A(j,35)%j-A(j,37)%j &
       -A(j,39)%j-A(j,131)%j-A(j,133)%j-A(j,135)%j-A(j,139)%j-A(j,141)%j-A(j,143)%j-A(j,145)%j-A(j,147)%j-A(j,149)%j-A(j,151)%j &
       -A(j,153)%j-A(j,155)%j-A(j,157)%j-A(j,159)%j-A(j,171)%j-A(j,173)%j-A(j,185)%j-A(j,191)%j)*f(1))/12._/**/REALKIND &
       +((A(j,50)%j+A(j,58)%j+A(j,64)%j+A(j,68)%j+A(j,76)%j+A(j,80)%j+A(j,82)%j+A(j,88)%j+A(j,92)%j+A(j,96)%j+A(j,102)%j &
       +A(j,104)%j+A(j,110)%j+A(j,116)%j+A(j,120)%j+A(j,122)%j+A(j,126)%j+A(j,166)%j+A(j,176)%j+A(j,184)%j &
       +A(j,188)%j)*f(2))/4._/**/REALKIND+((A(j,2)%j+A(j,4)%j+A(j,8)%j+A(j,10)%j+A(j,14)%j+A(j,16)%j+A(j,34)%j+A(j,36)%j+A(j,38)%j &
       +A(j,40)%j+A(j,132)%j+A(j,134)%j+A(j,136)%j+A(j,140)%j+A(j,142)%j+A(j,144)%j+A(j,146)%j+A(j,148)%j+A(j,150)%j+A(j,152)%j &
       +A(j,154)%j+A(j,156)%j+A(j,158)%j+A(j,160)%j+A(j,172)%j+A(j,174)%j+A(j,186)%j+A(j,192)%j)*f(2))/12._/**/REALKIND &
       +(CI*(A(j,53)%j+A(j,59)%j-A(j,65)%j-A(j,73)%j+A(j,99)%j+A(j,105)%j)*f(3))/4._/**/REALKIND+(CI*(-A(j,54)%j-A(j,60)%j &
       +A(j,66)%j+A(j,74)%j-A(j,100)%j-A(j,106)%j)*f(4))/4._/**/REALKIND
  M1(2) = ((A(j,49)%j+A(j,51)%j+A(j,55)%j+A(j,57)%j+A(j,61)%j+A(j,63)%j+A(j,67)%j+A(j,69)%j+A(j,71)%j+A(j,75)%j+A(j,77)%j &
       +A(j,79)%j+A(j,81)%j+A(j,83)%j+A(j,85)%j+A(j,87)%j+A(j,89)%j+A(j,91)%j+A(j,93)%j+A(j,95)%j+A(j,113)%j+A(j,115)%j+A(j,117)%j &
       +A(j,119)%j+A(j,169)%j+A(j,175)%j+A(j,179)%j+A(j,183)%j)*f(1))/12._/**/REALKIND+((A(j,1)%j+A(j,9)%j+A(j,15)%j+A(j,21)%j &
       +A(j,23)%j+A(j,29)%j+A(j,35)%j+A(j,39)%j+A(j,41)%j+A(j,45)%j+A(j,131)%j+A(j,139)%j+A(j,143)%j+A(j,145)%j+A(j,151)%j &
       +A(j,155)%j+A(j,159)%j+A(j,161)%j+A(j,173)%j+A(j,177)%j+A(j,191)%j)*f(1))/4._/**/REALKIND+((-A(j,50)%j-A(j,52)%j-A(j,56)%j &
       ...
       -A(j,180)%j-A(j,184)%j)*f(2))/12._/**/REALKIND+((-A(j,2)%j-A(j,10)%j-A(j,16)%j-A(j,22)%j-A(j,24)%j-A(j,30)%j-A(j,36)%j &
       -A(j,40)%j-A(j,42)%j-A(j,46)%j-A(j,132)%j-A(j,140)%j-A(j,144)%j-A(j,146)%j-A(j,152)%j-A(j,156)%j-A(j,160)%j-A(j,162)%j &
       -A(j,174)%j-A(j,178)%j-A(j,192)%j)*f(2))/4._/**/REALKIND+(CI*(A(j,5)%j+A(j,11)%j-A(j,19)%j-A(j,25)%j+A(j,129)%j &
       +A(j,137)%j)*f(3))/4._/**/REALKIND+(CI*(-A(j,6)%j-A(j,12)%j+A(j,20)%j+A(j,26)%j-A(j,130)%j &
       -A(j,138)%j)*f(4))/4._/**/REALKIND
  M1(3) = ((A(j,3)%j+A(j,7)%j+A(j,13)%j+A(j,17)%j+A(j,27)%j+A(j,31)%j+A(j,33)%j+A(j,37)%j+A(j,43)%j+A(j,47)%j+A(j,133)%j &
       +A(j,135)%j+A(j,141)%j+A(j,147)%j+A(j,149)%j+A(j,153)%j+A(j,157)%j+A(j,163)%j+A(j,171)%j+A(j,181)%j &
       +A(j,185)%j)*f(1))/4._/**/REALKIND+((A(j,49)%j+A(j,51)%j+A(j,55)%j+A(j,57)%j+A(j,61)%j+A(j,63)%j+A(j,81)%j+A(j,83)%j &
       +A(j,85)%j+A(j,87)%j+A(j,97)%j+A(j,101)%j+A(j,103)%j+A(j,107)%j+A(j,109)%j+A(j,111)%j+A(j,113)%j+A(j,115)%j+A(j,117)%j &
       +A(j,119)%j+A(j,121)%j+A(j,123)%j+A(j,125)%j+A(j,127)%j+A(j,165)%j+A(j,167)%j+A(j,187)%j+A(j,189)%j)*f(1))/12._/**/REALKIND &
       +((-A(j,4)%j-A(j,8)%j-A(j,14)%j-A(j,18)%j-A(j,28)%j-A(j,32)%j-A(j,34)%j-A(j,38)%j-A(j,44)%j-A(j,48)%j-A(j,134)%j-A(j,136)%j &
       -A(j,142)%j-A(j,148)%j-A(j,150)%j-A(j,154)%j-A(j,158)%j-A(j,164)%j-A(j,172)%j-A(j,182)%j-A(j,186)%j)*f(2))/4._/**/REALKIND &
       +((-A(j,50)%j-A(j,52)%j-A(j,56)%j-A(j,58)%j-A(j,62)%j-A(j,64)%j-A(j,82)%j-A(j,84)%j-A(j,86)%j-A(j,88)%j-A(j,98)%j &
       -A(j,102)%j-A(j,104)%j-A(j,108)%j-A(j,110)%j-A(j,112)%j-A(j,114)%j-A(j,116)%j-A(j,118)%j-A(j,120)%j-A(j,122)%j-A(j,124)%j &
       -A(j,126)%j-A(j,128)%j-A(j,166)%j-A(j,168)%j-A(j,188)%j-A(j,190)%j)*f(2))/12._/**/REALKIND+(CI*(-A(j,5)%j-A(j,11)%j &
       +A(j,19)%j+A(j,25)%j-A(j,129)%j-A(j,137)%j)*f(3))/4._/**/REALKIND+(CI*(A(j,6)%j+A(j,12)%j-A(j,20)%j-A(j,26)%j+A(j,130)%j &
       +A(j,138)%j)*f(4))/4._/**/REALKIND
  M1(4) = ((-A(j,1)%j-A(j,3)%j-A(j,7)%j-A(j,9)%j-A(j,13)%j-A(j,15)%j-A(j,17)%j-A(j,21)%j-A(j,23)%j-A(j,27)%j-A(j,29)%j-A(j,31)%j &
       -A(j,33)%j-A(j,35)%j-A(j,37)%j-A(j,39)%j-A(j,41)%j-A(j,43)%j-A(j,45)%j-A(j,47)%j-A(j,145)%j-A(j,147)%j-A(j,149)%j &
       -A(j,151)%j-A(j,161)%j-A(j,163)%j-A(j,177)%j-A(j,181)%j)*f(1))/12._/**/REALKIND+((-A(j,51)%j-A(j,55)%j-A(j,61)%j-A(j,69)%j &
       -A(j,71)%j-A(j,77)%j-A(j,83)%j-A(j,85)%j-A(j,89)%j-A(j,93)%j-A(j,97)%j-A(j,107)%j-A(j,111)%j-A(j,113)%j-A(j,117)%j &
       -A(j,123)%j-A(j,127)%j-A(j,167)%j-A(j,169)%j-A(j,179)%j-A(j,189)%j)*f(1))/4._/**/REALKIND+((A(j,2)%j+A(j,4)%j+A(j,8)%j &
       ...
       +A(j,178)%j+A(j,182)%j)*f(2))/12._/**/REALKIND+((A(j,52)%j+A(j,56)%j+A(j,62)%j+A(j,70)%j+A(j,72)%j+A(j,78)%j+A(j,84)%j &
       +A(j,86)%j+A(j,90)%j+A(j,94)%j+A(j,98)%j+A(j,108)%j+A(j,112)%j+A(j,114)%j+A(j,118)%j+A(j,124)%j+A(j,128)%j+A(j,168)%j &
       +A(j,170)%j+A(j,180)%j+A(j,190)%j)*f(2))/4._/**/REALKIND+(CI*(-A(j,53)%j-A(j,59)%j+A(j,65)%j+A(j,73)%j-A(j,99)%j &
       -A(j,105)%j)*f(3))/4._/**/REALKIND+(CI*(A(j,54)%j+A(j,60)%j-A(j,66)%j-A(j,74)%j+A(j,100)%j &
       +A(j,106)%j)*f(4))/4._/**/REALKIND
  M1(5) = ((-A(j,49)%j-A(j,51)%j-A(j,55)%j-A(j,57)%j-A(j,61)%j-A(j,63)%j-A(j,67)%j-A(j,69)%j-A(j,71)%j-A(j,75)%j-A(j,77)%j &
       -A(j,79)%j-A(j,81)%j-A(j,83)%j-A(j,85)%j-A(j,87)%j-A(j,89)%j-A(j,91)%j-A(j,93)%j-A(j,95)%j-A(j,97)%j-A(j,101)%j-A(j,103)%j &
       -A(j,107)%j-A(j,109)%j-A(j,111)%j-A(j,113)%j-A(j,115)%j-A(j,117)%j-A(j,119)%j-A(j,121)%j-A(j,123)%j-A(j,125)%j-A(j,127)%j &
       -A(j,165)%j-A(j,167)%j-A(j,169)%j-A(j,175)%j-A(j,179)%j-A(j,183)%j-A(j,187)%j-A(j,189)%j)*f(1))/36._/**/REALKIND+(( &
       -A(j,17)%j-A(j,21)%j-A(j,23)%j-A(j,27)%j-A(j,29)%j-A(j,31)%j-A(j,41)%j-A(j,43)%j-A(j,45)%j-A(j,47)%j-A(j,131)%j-A(j,133)%j &
       -A(j,135)%j-A(j,139)%j-A(j,141)%j-A(j,143)%j-A(j,153)%j-A(j,155)%j-A(j,157)%j-A(j,159)%j-A(j,161)%j-A(j,163)%j-A(j,171)%j &
       -A(j,173)%j-A(j,177)%j-A(j,181)%j-A(j,185)%j-A(j,191)%j)*f(1))/12._/**/REALKIND+((A(j,50)%j+A(j,52)%j+A(j,56)%j+A(j,58)%j &
       +A(j,62)%j+A(j,64)%j+A(j,68)%j+A(j,70)%j+A(j,72)%j+A(j,76)%j+A(j,78)%j+A(j,80)%j+A(j,82)%j+A(j,84)%j+A(j,86)%j+A(j,88)%j &
       +A(j,90)%j+A(j,92)%j+A(j,94)%j+A(j,96)%j+A(j,98)%j+A(j,102)%j+A(j,104)%j+A(j,108)%j+A(j,110)%j+A(j,112)%j+A(j,114)%j &
       +A(j,116)%j+A(j,118)%j+A(j,120)%j+A(j,122)%j+A(j,124)%j+A(j,126)%j+A(j,128)%j+A(j,166)%j+A(j,168)%j+A(j,170)%j+A(j,176)%j &
       +A(j,180)%j+A(j,184)%j+A(j,188)%j+A(j,190)%j)*f(2))/36._/**/REALKIND+((A(j,18)%j+A(j,22)%j+A(j,24)%j+A(j,28)%j+A(j,30)%j &
       +A(j,32)%j+A(j,42)%j+A(j,44)%j+A(j,46)%j+A(j,48)%j+A(j,132)%j+A(j,134)%j+A(j,136)%j+A(j,140)%j+A(j,142)%j+A(j,144)%j &
       +A(j,154)%j+A(j,156)%j+A(j,158)%j+A(j,160)%j+A(j,162)%j+A(j,164)%j+A(j,172)%j+A(j,174)%j+A(j,178)%j+A(j,182)%j+A(j,186)%j &
       +A(j,192)%j)*f(2))/12._/**/REALKIND
  M1(6) = ((A(j,67)%j+A(j,69)%j+A(j,71)%j+A(j,75)%j+A(j,77)%j+A(j,79)%j+A(j,89)%j+A(j,91)%j+A(j,93)%j+A(j,95)%j+A(j,97)%j &
       +A(j,101)%j+A(j,103)%j+A(j,107)%j+A(j,109)%j+A(j,111)%j+A(j,121)%j+A(j,123)%j+A(j,125)%j+A(j,127)%j+A(j,165)%j+A(j,167)%j &
       +A(j,169)%j+A(j,175)%j+A(j,179)%j+A(j,183)%j+A(j,187)%j+A(j,189)%j)*f(1))/12._/**/REALKIND+((A(j,1)%j+A(j,3)%j+A(j,7)%j &
       +A(j,9)%j+A(j,13)%j+A(j,15)%j+A(j,17)%j+A(j,21)%j+A(j,23)%j+A(j,27)%j+A(j,29)%j+A(j,31)%j+A(j,33)%j+A(j,35)%j+A(j,37)%j &
       +A(j,39)%j+A(j,41)%j+A(j,43)%j+A(j,45)%j+A(j,47)%j+A(j,131)%j+A(j,133)%j+A(j,135)%j+A(j,139)%j+A(j,141)%j+A(j,143)%j &
       +A(j,145)%j+A(j,147)%j+A(j,149)%j+A(j,151)%j+A(j,153)%j+A(j,155)%j+A(j,157)%j+A(j,159)%j+A(j,161)%j+A(j,163)%j+A(j,171)%j &
       +A(j,173)%j+A(j,177)%j+A(j,181)%j+A(j,185)%j+A(j,191)%j)*f(1))/36._/**/REALKIND+((-A(j,68)%j-A(j,70)%j-A(j,72)%j-A(j,76)%j &
       -A(j,78)%j-A(j,80)%j-A(j,90)%j-A(j,92)%j-A(j,94)%j-A(j,96)%j-A(j,98)%j-A(j,102)%j-A(j,104)%j-A(j,108)%j-A(j,110)%j &
       -A(j,112)%j-A(j,122)%j-A(j,124)%j-A(j,126)%j-A(j,128)%j-A(j,166)%j-A(j,168)%j-A(j,170)%j-A(j,176)%j-A(j,180)%j-A(j,184)%j &
       -A(j,188)%j-A(j,190)%j)*f(2))/12._/**/REALKIND+((-A(j,2)%j-A(j,4)%j-A(j,8)%j-A(j,10)%j-A(j,14)%j-A(j,16)%j-A(j,18)%j &
       -A(j,22)%j-A(j,24)%j-A(j,28)%j-A(j,30)%j-A(j,32)%j-A(j,34)%j-A(j,36)%j-A(j,38)%j-A(j,40)%j-A(j,42)%j-A(j,44)%j-A(j,46)%j &
       -A(j,48)%j-A(j,132)%j-A(j,134)%j-A(j,136)%j-A(j,140)%j-A(j,142)%j-A(j,144)%j-A(j,146)%j-A(j,148)%j-A(j,150)%j-A(j,152)%j &
       -A(j,154)%j-A(j,156)%j-A(j,158)%j-A(j,160)%j-A(j,162)%j-A(j,164)%j-A(j,172)%j-A(j,174)%j-A(j,178)%j-A(j,182)%j-A(j,186)%j &
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppllttj_eexuuuxuxttx_1_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppllttj_eexuuuxuxttx_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppllttj_eexuuuxuxttx_1")
#else
subroutine amp2tree(P, M2)
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppllttj_eexuuuxuxttx_1")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppllttj_eexuuuxuxttx_1")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppllttj_eexuuuxuxttx_1")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppllttj_eexuuuxuxttx_1")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppllttj_eexuuuxuxttx_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppllttj_eexuuuxuxttx_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppllttj_eexuuuxuxttx_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppllttj_eexuuuxuxttx_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppllttj_eexuuuxuxttx_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
...
subroutine amp2tree_legacy(p, m2) &
    & bind(c,name="amp2tree_ppllttj_eexuuuxuxttx_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccone_legacy(p, m2, i, j) &
    & bind(c,name="amp2ccone_ppllttj_eexuuuxuxttx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccall_legacy(p, m2) &
    & bind(c,name="amp2ccall_ppllttj_eexuuuxuxttx_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2(0:38-1)
...
subroutine amp2hcone_legacy(p, m2, i, j, mom) &
    & bind(c,name="amp2hcone_ppllttj_eexuuuxuxttx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_legacy(p, m2, i, mom) &
    & bind(c,name="amp2hcall_ppllttj_eexuuuxuxttx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2(8)
...
end module ol_tree_ppllttj_eexuuuxuxttx_1_/**/REALKIND
