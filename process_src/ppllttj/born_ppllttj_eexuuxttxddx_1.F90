...
module ol_colourmatrix_ppllttj_eexuuxttxddx_1_/**/REALKIND
...
end module ol_colourmatrix_ppllttj_eexuuxttxddx_1_/**/REALKIND



module ol_forced_parameters_ppllttj_eexuuxttxddx_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllttj_eexuuxttxddx_1_/**/REALKIND

module ol_tree_ppllttj_eexuuxttxddx_1_/**/REALKIND
...
subroutine born_denominators_init()
  ...
  den(52) = 1 / (Q(5,192))
  den(59) = 1 / (Q(5,204))
  den(63) = 1 / (Q(5,224) - MT2)
  den(69) = 1 / (Q(5,28) - MT2)
  den(71) = 1 / (Q(5,35) - MT2)
  den(80) = 1 / (Q(5,208) - MT2)
  den(93) = 1 / (Q(5,51))
  den(117) = 1 / (Q(5,7))
  den(119) = 1 / (Q(5,240))
  den(124) = 1 / (Q(5,56))
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppllttj_eexuuxttxddx_1")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppllttj_eexuuxttxddx_1_/**/DREALKIND, only: ntry, nhel, Hel, M1helarr
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for e- e+ up anti-up top anti-top down anti-down -> 0
...
! **********************************************************************
  ...
  use ol_external_ppllttj_eexuuxttxddx_1, only: external_perm_ppllttj_eexuuxttxddx_1, &
    & external_perm_inv_ppllttj_eexuuxttxddx_1, extcomb_perm_ppllttj_eexuuxttxddx_1, &
    & average_factor_ppllttj_eexuuxttxddx_1
  use ol_external_ppllttj_eexuuxttxddx_1, only: H, hel_not_initialised, hel_init
  use ol_colourmatrix_ppllttj_eexuuxttxddx_1_/**/REALKIND, only: colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppllttj_eexuuxttxddx_1_/**/REALKIND, only: check_forced_parameters
  use ol_heltables_ppllttj_eexuuxttxddx_1
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_ppllttj_eexuuxttxddx_1,8)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppllttj_eexuuxttxddx_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppllttj_eexuuxttxddx_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_ppllttj_eexuuxttxddx_1(extcombs(k))
  end do
  ...
  call prop_A_Q(ntry, wf8(:,7), Q(:,176), ZERO, 0_intkind1, wf8(:,8), n2(5))
  ...
  call prop_Q_A(ntry, wf8(:,9), Q(:,76), ZERO, 0_intkind1, wf8(:,11), n2(6))
  ...
  call prop_Q_A(ntry, wf8(:,17), Q(:,19), MT, 1_intkind1, wf8(:,19), n2(10))
  call prop_A_Q(ntry, wf8(:,18), Q(:,44), MT, 1_intkind1, wf8(:,20), n2(11))
  call vert_QA_V(ntry, wf8(:,19), wf8(:,20), wf64(:,1), n3(:,31), t3x64(:,:,1))
  call vert_ZQ_A(gZu,ntry, wf4(:,5), ex5, wf8(:,21), n3(:,32), t3x8(:,:,11))
  call prop_Q_A(ntry, wf8(:,21), Q(:,19), MT, 1_intkind1, wf8(:,22), n2(12))
  ...
  call prop_A_Q(ntry, wf8(:,23), Q(:,224), MT, 1_intkind1, wf8(:,24), n2(13))
  ...
  call prop_Q_A(ntry, wf8(:,25), Q(:,28), MT, 1_intkind1, wf8(:,27), n2(14))
  call prop_A_Q(ntry, wf8(:,26), Q(:,35), MT, 1_intkind1, wf8(:,28), n2(15))
  call vert_QA_V(ntry, wf8(:,27), wf8(:,28), wf64(:,3), n3(:,42), t3x64(:,:,3))
  call vert_AZ_Q(gZu,ntry, ex6, wf4(:,5), wf8(:,29), n3(:,43), t3x8(:,:,15))
  call prop_A_Q(ntry, wf8(:,29), Q(:,35), MT, 1_intkind1, wf8(:,30), n2(16))
  ...
  call prop_Q_A(ntry, wf8(:,31), Q(:,208), MT, 1_intkind1, wf8(:,32), n2(17))
  ...
  M2add = M2 / average_factor_ppllttj_eexuuxttxddx_1

  do k = 0, 38-1
    M2(k) = M2add(extcomb_perm_ppllttj_eexuuxttxddx_1(k))
  end do
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = ((-A(j,1)%j-A(j,3)%j-A(j,7)%j-A(j,9)%j-A(j,13)%j-A(j,15)%j-A(j,73)%j-A(j,75)%j-A(j,77)%j &
       -A(j,79)%j)*f(1))/12._/**/REALKIND+((A(j,33)%j+A(j,35)%j+A(j,37)%j+A(j,39)%j+A(j,51)%j+A(j,53)%j+A(j,57)%j+A(j,59)%j &
       +A(j,61)%j+A(j,63)%j+A(j,65)%j+A(j,67)%j+A(j,69)%j+A(j,71)%j+A(j,89)%j+A(j,91)%j+A(j,93)%j &
       +A(j,95)%j)*f(2))/12._/**/REALKIND+((-A(j,2)%j-A(j,4)%j-A(j,8)%j-A(j,10)%j-A(j,14)%j-A(j,16)%j-A(j,34)%j-A(j,36)%j &
       -A(j,38)%j-A(j,40)%j-A(j,52)%j-A(j,54)%j-A(j,58)%j-A(j,60)%j-A(j,62)%j-A(j,64)%j-A(j,66)%j-A(j,68)%j-A(j,70)%j-A(j,72)%j &
       -A(j,74)%j-A(j,76)%j-A(j,78)%j-A(j,80)%j-A(j,90)%j-A(j,92)%j-A(j,94)%j-A(j,96)%j)*f(3))/12._/**/REALKIND
  M1(2) = ((A(j,1)%j+A(j,9)%j+A(j,15)%j+A(j,41)%j+A(j,45)%j+A(j,75)%j+A(j,79)%j)*f(1))/4._/**/REALKIND+((-A(j,21)%j-A(j,23)%j &
       -A(j,29)%j-A(j,35)%j-A(j,39)%j-A(j,51)%j-A(j,59)%j-A(j,63)%j-A(j,67)%j-A(j,71)%j-A(j,81)%j-A(j,85)%j-A(j,91)%j &
       -A(j,95)%j)*f(2))/4._/**/REALKIND+((A(j,2)%j+A(j,10)%j+A(j,16)%j+A(j,22)%j+A(j,24)%j+A(j,30)%j+A(j,36)%j+A(j,40)%j &
       +A(j,42)%j+A(j,46)%j+A(j,52)%j+A(j,60)%j+A(j,64)%j+A(j,68)%j+A(j,72)%j+A(j,76)%j+A(j,80)%j+A(j,82)%j+A(j,86)%j+A(j,92)%j &
       +A(j,96)%j)*f(3))/4._/**/REALKIND+(CI*(A(j,5)%j+A(j,11)%j)*f(4))/4._/**/REALKIND+(CI*(A(j,19)%j+A(j,25)%j-A(j,49)%j &
       -A(j,55)%j)*f(5))/4._/**/REALKIND+(CI*(A(j,6)%j+A(j,12)%j-A(j,20)%j-A(j,26)%j+A(j,50)%j+A(j,56)%j)*f(6))/4._/**/REALKIND
  M1(3) = ((A(j,3)%j+A(j,7)%j+A(j,13)%j+A(j,43)%j+A(j,47)%j+A(j,73)%j+A(j,77)%j)*f(1))/4._/**/REALKIND+((-A(j,17)%j-A(j,27)%j &
       -A(j,31)%j-A(j,33)%j-A(j,37)%j-A(j,53)%j-A(j,57)%j-A(j,61)%j-A(j,65)%j-A(j,69)%j-A(j,83)%j-A(j,87)%j-A(j,89)%j &
       -A(j,93)%j)*f(2))/4._/**/REALKIND+((A(j,4)%j+A(j,8)%j+A(j,14)%j+A(j,18)%j+A(j,28)%j+A(j,32)%j+A(j,34)%j+A(j,38)%j+A(j,44)%j &
       +A(j,48)%j+A(j,54)%j+A(j,58)%j+A(j,62)%j+A(j,66)%j+A(j,70)%j+A(j,74)%j+A(j,78)%j+A(j,84)%j+A(j,88)%j+A(j,90)%j &
       +A(j,94)%j)*f(3))/4._/**/REALKIND+(CI*(-A(j,5)%j-A(j,11)%j)*f(4))/4._/**/REALKIND+(CI*(-A(j,19)%j-A(j,25)%j+A(j,49)%j &
       +A(j,55)%j)*f(5))/4._/**/REALKIND+(CI*(-A(j,6)%j-A(j,12)%j+A(j,20)%j+A(j,26)%j-A(j,50)%j-A(j,56)%j)*f(6))/4._/**/REALKIND
  M1(4) = ((-A(j,1)%j-A(j,3)%j-A(j,7)%j-A(j,9)%j-A(j,13)%j-A(j,15)%j-A(j,41)%j-A(j,43)%j-A(j,45)%j &
       -A(j,47)%j)*f(1))/12._/**/REALKIND+((A(j,17)%j+A(j,21)%j+A(j,23)%j+A(j,27)%j+A(j,29)%j+A(j,31)%j+A(j,33)%j+A(j,35)%j &
       +A(j,37)%j+A(j,39)%j+A(j,65)%j+A(j,67)%j+A(j,69)%j+A(j,71)%j+A(j,81)%j+A(j,83)%j+A(j,85)%j &
       +A(j,87)%j)*f(2))/12._/**/REALKIND+((-A(j,2)%j-A(j,4)%j-A(j,8)%j-A(j,10)%j-A(j,14)%j-A(j,16)%j-A(j,18)%j-A(j,22)%j &
       -A(j,24)%j-A(j,28)%j-A(j,30)%j-A(j,32)%j-A(j,34)%j-A(j,36)%j-A(j,38)%j-A(j,40)%j-A(j,42)%j-A(j,44)%j-A(j,46)%j-A(j,48)%j &
       -A(j,66)%j-A(j,68)%j-A(j,70)%j-A(j,72)%j-A(j,82)%j-A(j,84)%j-A(j,86)%j-A(j,88)%j)*f(3))/12._/**/REALKIND
  M1(5) = ((-A(j,41)%j-A(j,43)%j-A(j,45)%j-A(j,47)%j-A(j,73)%j-A(j,75)%j-A(j,77)%j-A(j,79)%j)*f(1))/12._/**/REALKIND+((A(j,17)%j &
       +A(j,21)%j+A(j,23)%j+A(j,27)%j+A(j,29)%j+A(j,31)%j+A(j,51)%j+A(j,53)%j+A(j,57)%j+A(j,59)%j+A(j,61)%j+A(j,63)%j+A(j,81)%j &
       +A(j,83)%j+A(j,85)%j+A(j,87)%j+A(j,89)%j+A(j,91)%j+A(j,93)%j+A(j,95)%j)*f(2))/12._/**/REALKIND+((-A(j,18)%j-A(j,22)%j &
       -A(j,24)%j-A(j,28)%j-A(j,30)%j-A(j,32)%j-A(j,42)%j-A(j,44)%j-A(j,46)%j-A(j,48)%j-A(j,52)%j-A(j,54)%j-A(j,58)%j-A(j,60)%j &
       -A(j,62)%j-A(j,64)%j-A(j,74)%j-A(j,76)%j-A(j,78)%j-A(j,80)%j-A(j,82)%j-A(j,84)%j-A(j,86)%j-A(j,88)%j-A(j,90)%j-A(j,92)%j &
       -A(j,94)%j-A(j,96)%j)*f(3))/12._/**/REALKIND
  M1(6) = ((A(j,1)%j+A(j,3)%j+A(j,7)%j+A(j,9)%j+A(j,13)%j+A(j,15)%j+A(j,41)%j+A(j,43)%j+A(j,45)%j+A(j,47)%j+A(j,73)%j+A(j,75)%j &
       +A(j,77)%j+A(j,79)%j)*f(1))/36._/**/REALKIND+((-A(j,17)%j-A(j,21)%j-A(j,23)%j-A(j,27)%j-A(j,29)%j-A(j,31)%j-A(j,33)%j &
       -A(j,35)%j-A(j,37)%j-A(j,39)%j-A(j,51)%j-A(j,53)%j-A(j,57)%j-A(j,59)%j-A(j,61)%j-A(j,63)%j-A(j,65)%j-A(j,67)%j-A(j,69)%j &
       -A(j,71)%j-A(j,81)%j-A(j,83)%j-A(j,85)%j-A(j,87)%j-A(j,89)%j-A(j,91)%j-A(j,93)%j-A(j,95)%j)*f(2))/36._/**/REALKIND &
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppllttj_eexuuxttxddx_1_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppllttj_eexuuxttxddx_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppllttj_eexuuxttxddx_1")
#else
subroutine amp2tree(P, M2)
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppllttj_eexuuxttxddx_1")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppllttj_eexuuxttxddx_1")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppllttj_eexuuxttxddx_1")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppllttj_eexuuxttxddx_1")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppllttj_eexuuxttxddx_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppllttj_eexuuxttxddx_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppllttj_eexuuxttxddx_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppllttj_eexuuxttxddx_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppllttj_eexuuxttxddx_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
...
subroutine amp2tree_legacy(p, m2) &
    & bind(c,name="amp2tree_ppllttj_eexuuxttxddx_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccone_legacy(p, m2, i, j) &
    & bind(c,name="amp2ccone_ppllttj_eexuuxttxddx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccall_legacy(p, m2) &
    & bind(c,name="amp2ccall_ppllttj_eexuuxttxddx_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2(0:38-1)
...
subroutine amp2hcone_legacy(p, m2, i, j, mom) &
    & bind(c,name="amp2hcone_ppllttj_eexuuxttxddx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_legacy(p, m2, i, mom) &
    & bind(c,name="amp2hcall_ppllttj_eexuuxttxddx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,8)
  real(c_double), intent(out) :: m2(8)
...
end module ol_tree_ppllttj_eexuuxttxddx_1_/**/REALKIND
