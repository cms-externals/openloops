...
module ol_vamp_bbh_bbxh_1_/**/REALKIND
  contains
...
! **********************************************************************
  ...
  use ol_vamp_1_bbh_bbxh_1_/**/REALKIND, only: vamp_1
...
! M2tree = helicity-summed squared tree matrix element for bottom anti-bottom higgs -> 0
! M2loop0 = helicity-summed squared loop matrix element for bottom anti-bottom higgs -> 0
...
! **********************************************************************
  ...
  use ol_external_bbh_bbxh_1, only: &
    & external_perm_inv_bbh_bbxh_1, &
    & average_factor_bbh_bbxh_1, &
    external_perm_bbh_bbxh_1
  use ol_external_bbh_bbxh_1, only: &
    & hel_not_initialised, hel_init, H, H_HC, POLSEL
  use ol_colourmatrix_bbh_bbxh_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  ...
  real(DREALKIND), intent(in)  :: P_scatt(0:3,3)
  real(REALKIND),  intent(out) :: M2L0, M2L1, IRL1(0:2), M2L2, IRL2(0:4)
  integer, intent(in), optional :: mode
  real(REALKIND)       :: P(0:3,3)
  integer              :: la, i, j, colmatpos, recycle_mode
  complex(REALKIND)    :: M0_col1(1)
  complex(REALKIND)    :: M0(1,4), Mct(1,4), Mcol_loop(1,4)
  real(REALKIND)       :: M2colint(7), M2CC(3,3), M2CC_EW ! colour correlations
  real(REALKIND)       :: M2L2colint(0:7), M2L2CC(3,3)
  integer,        save :: ntry = 1
  real(REALKIND)       :: extmasses2(3)
  real(REALKIND)       :: M2hel, M2ct, M2L2ct, M2L2ct2, vdip, c_dip(0:2)
  real(REALKIND)       :: scalebackfactor
  integer              :: cr, k, m, n, l, hels(2,4)
  real(REALKIND)       :: P_scatt_intern(0:3,3)
  ...
  extmasses2 = [ rMB2, rMB2, rMH2 ]
  call conv_mom_scatt2in(P_scatt, extmasses2, P, &
     & external_perm_inv_bbh_bbxh_1, 3)
  ! internal-propagator momenta in light-cone representation
  call internal_momenta(P, 3)
  ...
  do
...
#if 1 > 0
    do j = 1, 1
      M2L0 = M2L0 + real(sum(conjg(M0(:,la))*K1(1:1,j))*M0(j,la))
    end do
#endif
#if 1 > 0
    do j = 1, 1
      M2ct = M2ct + real(sum(conjg(M0(:,la))*K2(1:1,j))*Mct(j,la))
    end do
#endif
    do j = 1, 1
      M0_col1(j) = sum(conjg(M0(:,la))*KL(1:1,j))
    end do
#if 8 > 1
    if (IR_is_on > 0) then
      do j = 1, 8-1
    ...
    if (IR_is_on > 0) then
#if 1 > 0
        call colint(Mcol_loop(:,la), Mcol_loop(:,la), 1, KL2, [(i, i = 0, 7-1)], M2L2colint)  ! <M1|M1>
        call colint(2*Mcol_loop(:,la), Mct(:,la), 1, KL2ct, [(i, i = 0, 7-1)], M2L2colint)    ! 2*<M1|Mct>
#endif
#if 1 > 0
        call colint(Mct(:,la), Mct(:,la), 1, KL2ct2, [(i, i = 0, 7-1)], M2L2colint)            ! <Mct|Mct>
#endif
    end if
...
! #ifdef LOOPSQUARED
  ...
  if (IR_is_on > 0) then
    ...
    M2CC_EW = M2colint(3*(3+1)/2+1)
    call intdip(ioperator_mode, M2L0, M2CC, M2CC_EW, [2,2,3], [-1,1,0]/3._/**/REALKIND, &
      & 3, extmasses2, QInvariantsMatrix, vdip, c_dip)
    IRL1(0) = c_dip(0) / average_factor_bbh_bbxh_1
    IRL1(1) = c_dip(1) / average_factor_bbh_bbxh_1
    IRL1(2) = c_dip(2) / average_factor_bbh_bbxh_1
  else
    vdip = 0
    IRL1 = 0
  end if
  ...
  if (IR_is_on > 0) then
    ...
    M2CC_EW = M2L2colint(3*(3+1)/2+1)
    call intdip(ioperator_mode, M2L2, M2L2CC, M2CC_EW, [2,2,3], [-1,1,0]/3._/**/REALKIND, &
      & 3, extmasses2, QInvariantsMatrix, vdip, c_dip)
    IRL2(0) = c_dip(0) / average_factor_bbh_bbxh_1
    IRL2(1) = c_dip(1) / average_factor_bbh_bbxh_1
    IRL2(2) = c_dip(2) / average_factor_bbh_bbxh_1
  else
    IRL2 = 0
  end if
#endif
  ...
  M2L0 = M2L0 / average_factor_bbh_bbxh_1
  M2L1 = 2*(M2L1 + M2ct)
  if (IR_is_on > 1) then
    M2L1 = M2L1 + vdip
  end if
  M2L1 = M2L1 / average_factor_bbh_bbxh_1
  ...
  scalebackfactor = scalefactor**(2*3-8)
  ...
  scalebackfactor = scalefactor**(3-4)
...
! P_scatt(0:3,Npart) = external momenta
! M2L0 = helicity-summed squared tree matrix element for bottom anti-bottom higgs -> 0
! M2L2 = helicity-summed tree x loop matrix element for bottom anti-bottom higgs -> 0
! M2L2 = helicity-summed squared loop matrix element for bottom anti-bottom higgs -> 0
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_debug, only: ol_fatal
  use ol_external_bbh_bbxh_1, only: H, H_HC, &
    average_factor_bbh_bbxh_1, &
    external_perm_bbh_bbxh_1, &
    extcomb_perm_bbh_bbxh_1
  use ol_colourmatrix_bbh_bbxh_1_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  use ol_parameters_decl_/**/REALKIND ! parameters_status, scalefactor, <masses>
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_loop_bbh_bbxh_1_/**/REALKIND, only: zerohel, M0, Mct, Mcol_loop
  use ol_wavefunctions_/**/REALKIND, only: wf_V_Std
  use ol_kinematics_/**/REALKIND, only: LC2Std_Rep_cmplx
  ...
  integer              :: la, i, j, colmatpos, recycle_mode
  real(REALKIND)       :: M2colint(7)
  integer,        save :: ntry = 1
  real(REALKIND)       :: M2L1cc(0:7),M2L2cc(0:7)
  integer              :: cr, k, m, n, l, hels(2,4)
  integer              :: extcombs(7)
  ...
  emitter = external_perm_bbh_bbxh_1(corr%emitter)
  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_bbh_bbxh_1(extcombs(k))
  end do
  ...
  if (cr == 1) then
    ...
    do k = 0, 7
      corr%rescc(k) = M2L2cc(extcomb_perm_bbh_bbxh_1(k)) / &
                      average_factor_bbh_bbxh_1
    end do
    M2L2 = corr%rescc(0)
    ...
    do k = 0, 7
      corr%rescc(k) = 0.5_/**/REALKIND*M2L2cc(extcomb_perm_bbh_bbxh_1(k)) / &
                      average_factor_bbh_bbxh_1
    end do
  ...
  ! get wfs
    ...
    if (any(corr%resmunu(:,:) /= corr%resmunu(:,:))) then
      ...
      corr%resmunu = corr%resmunu / average_factor_bbh_bbxh_1
      M2L2 = -corr%resmunu(1,1)+corr%resmunu(2,2)+corr%resmunu(3,3)+corr%resmunu(4,4) ! |M1|^2 = -B^mu_mu
    end if
  else if (cr == 11) then
    do la=1,4
#if 1 > 0
        call colint(2*M0(:,la), Mcol_loop(:,la), 1, KL, extcombs_permuted, M2L1cc)  ! <M0|M1>
#endif
#if 1 > 0
        call colint(2*M0(:,la), Mct(:,la), 1, K2, extcombs_permuted, M2L1cc)    ! <M0|Mct>
#endif
    end do
    do k = 0, 7
      corr%rescc(k) = M2L1cc(extcomb_perm_bbh_bbxh_1(k)) / average_factor_bbh_bbxh_1
    end do
    M2L1 = corr%rescc(0)
...
#if 1 > 0
        call colint(2*M0(:,hels(1,l)), om*Mcol_loop(:,hels(2,l)), 1, KL(1:1,:), extcombs_permuted, M2L2cc)  ! <M0|M1>
        call colint(2*M0(:,hels(1,l)), om*Mct(:,hels(2,l)), 1, K2(1:1,:), extcombs_permuted, M2L2cc)    ! <M0|Mct>
#endif
      end do
    end do
    ...
    do k=1,4,2
      ...
      do l=1,size(hels,2)
#if 1 > 0
        call colint(2*M0(:,hels(1,l)), Mcol_loop(:,hels(2,l)), 1, KL(1:1,:), [0], M2hels(l:l))  ! <M0|M1>
        call colint(2*M0(:,hels(1,l)), Mct(:,hels(2,l)), 1, K2(1:1,:), [0], M2hels(l:l))    ! <M0|Mct>
#endif
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_external_bbh_bbxh_1, only: hel_not_initialised, hel_init, H
  use ol_colourmatrix_bbh_bbxh_1_/**/REALKIND, only: &
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  use ol_external_bbh_bbxh_1, only: &
    & external_perm_inv_bbh_bbxh_1, average_factor_bbh_bbxh_1
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_kinematics_/**/REALKIND, only: conv_mom_scatt2in, internal_momenta
  use ol_forced_parameters_bbh_bbxh_1_/**/REALKIND, only: check_forced_parameters
  use ol_loop_bbh_bbxh_1_/**/REALKIND, only: &
...
!  integer           :: DOI_bak
  ...
  ! Convert 2 -> n-2 PS-point to n -> 0 (so that P(1) + ... + P(n) = 0)
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_bbh_bbxh_1, 3)
  ! internal-propagator momenta in light-cone representation
  call internal_momenta(P, 3)
...
#if 1 > 0
    do j = 1, 1
      M2tree = M2tree + real(sum(conjg(M0(:,la))*K1(1:1,j))*M0(j,la))
    end do
#endif
#if 1 > 0
    do j = 1, 1
      M2ct = M2ct + real(sum(conjg(M0(:,la))*K2(1:1,j))*Mct(j))
    end do
#endif
  end do
  ...
  scalebackfactor = scalefactor**(2*3-8)
  M2tree = scalebackfactor * M2tree / average_factor_bbh_bbxh_1
  M2ct   = scalebackfactor * 2*M2ct / average_factor_bbh_bbxh_1
...
end module ol_vamp_bbh_bbxh_1_/**/REALKIND



#ifdef PRECISION_dp

module ol_vamp_bbh_bbxh_1
  use ol_parameters_decl_/**/DREALKIND, only: procname_length
  use ol_data_types_/**/DREALKIND, only: me_cache
  implicit none
  character(procname_length) :: processname = 'bbh_bbxh_1'
  ...
  end subroutine finish_bbh_bbxh_1


  subroutine vamp2(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_f_vamp2_bbh_bbxh_1")
    use KIND_TYPES, only: DREALKIND
    use ol_init, only: register_cleanup
    use ol_external_bbh_bbxh_1, only: external_perm_bbh_bbxh_1
    use ol_vamp_bbh_bbxh_1_/**/DREALKIND, only: vamp2dp => vamp2pc
#ifdef USE_qp
    use ol_vamp_bbh_bbxh_1_/**/QREALKIND, only: vamp2qp => vamp2pc
#endif
...
#endif
    if (first_call) then
      call register_cleanup(finish_bbh_bbxh_1)
      first_call = .false.
    end if
    call vamp2generic(vamp2dp, vamp2qp, processname, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, &
                    ...
                    & external_perm_bbh_bbxh_1, me_caches)
...
! **********************************************************************
  subroutine vampcr2(P_scatt, M2L0, M2L1, M2L2, crtype, &
      ...
      & bind(c,name="ol_f_vampcr2_bbh_bbxh_1")
...
! **********************************************************************
    use KIND_TYPES, only: DREALKIND
    use ol_external_bbh_bbxh_1, only: external_perm_bbh_bbxh_1
    use ol_vamp_bbh_bbxh_1_/**/DREALKIND, only: vamp2cache
    use ol_data_types_/**/DREALKIND, only: correlator
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,3)
    ...
    real(REALKIND),  intent(out) :: M2cc(0:3*(3+1)/2+1)
    real(DREALKIND), intent(out) :: M2munu(4,4)
    integer :: n_cc = 3*(3+1)/2
    ...
    if (crtype == 0) then
      ...
      allocate(corr%rescc(0:3*(3+1)/2+1))
  ...
  subroutine ctamp2(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_f_ctamp2_bbh_bbxh_1")
    use KIND_TYPES, only: DREALKIND
    use ol_vamp_bbh_bbxh_1_/**/DREALKIND, only: ctamp2base
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,3)
  ...
  subroutine vamp2_c(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_vamp2_bbh_bbxh_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,3)
    real(c_double), intent(out) :: M2L0, M2L1(0:2), IRL1(0:2), M2L2(0:4), IRL2(0:4)
    real(DREALKIND) :: f_p_scatt(0:3,3)
  ...
  subroutine ctamp2_c(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_ctamp2_bbh_bbxh_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,3)
  ...
  subroutine ctamp2_legacy(P_scatt, M2tree, M2ct) &
      & bind(c,name="ctamp2_bbh_bbxh_1_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,3)
