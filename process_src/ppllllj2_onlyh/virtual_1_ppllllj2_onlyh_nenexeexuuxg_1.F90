...
module ol_vamp_1_ppllllj2_onlyh_nenexeexuuxg_1_/**/REALKIND
contains

! **********************************************************************
subroutine vamp_1(M)
! P(0:3,nlegs) = incoming external momenta
! Uses tree structures 'wf', factors 'c', and denominators 'den' from loop_ppllllj2_onlyh_nenexeexuuxg_1.
! Sets colour stripped amplitudes A from the module loop_amplitudes_ppllllj2_onlyh_nenexeexuuxg_1.
! **********************************************************************
...
#endif
  use ol_loop_ppllllj2_onlyh_nenexeexuuxg_1_/**/REALKIND, only: c, f, wf, den
  use ol_vert_interface_/**/REALKIND
  use ol_prop_interface_/**/REALKIND
  use ol_last_step_/**/REALKIND
  use ol_tensor_sum_storage_ppllllj2_onlyh_nenexeexuuxg_1_/**/REALKIND
  ...
  M(1) = M(1) + (f(8)/2._/**/REALKIND * den(7)) * TI2_call(3,momenta_1,masses2_2,G3tensor(:,1),T3sum(1:35,1))
  M(1) = M(1) + (f(8)/2._/**/REALKIND * den(7)) * TI2_call(3,momenta_1,masses2_2,G3tensor(:,3),T3sum(1:35,1))
  M(1) = M(1) + (f(4)/2._/**/REALKIND * den(7)) * TI2_call(3,momenta_1,masses2_1,G3tensor(:,5),T3sum(1:35,2))
  M(1) = M(1) + (f(4)/2._/**/REALKIND * den(7)) * TI2_call(3,momenta_1,masses2_1,G3tensor(:,7),T3sum(1:35,2))
  M(1) = M(1) + (-(f(7)/2._/**/REALKIND) * den(12)) * TI2_call(3,momenta_1,masses2_2,G3tensor(:,2),T3sum(1:35,1))
  M(1) = M(1) + (-(f(7)/2._/**/REALKIND) * den(12)) * TI2_call(3,momenta_1,masses2_2,G3tensor(:,4),T3sum(1:35,1))
  M(1) = M(1) + (-(f(3)/2._/**/REALKIND) * den(12)) * TI2_call(3,momenta_1,masses2_1,G3tensor(:,6),T3sum(1:35,2))
  M(1) = M(1) + (-(f(3)/2._/**/REALKIND) * den(12)) * TI2_call(3,momenta_1,masses2_1,G3tensor(:,8),T3sum(1:35,2))

end subroutine vamp_1

end module ol_vamp_1_ppllllj2_onlyh_nenexeexuuxg_1_/**/REALKIND
