...
module ol_colourmatrix_ppattj_ttxddxagg_1_/**/REALKIND
...
end module ol_colourmatrix_ppattj_ttxddxagg_1_/**/REALKIND



module ol_forced_parameters_ppattj_ttxddxagg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppattj_ttxddxagg_1_/**/REALKIND

module ol_tree_ppattj_ttxddxagg_1_/**/REALKIND
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppattj_ttxddxagg_1")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppattj_ttxddxagg_1_/**/DREALKIND, only: ntry, nhel, Hel, M1helarr
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for top anti-top down anti-down gamma glue glue -> 0
...
! **********************************************************************
  ...
  use ol_external_ppattj_ttxddxagg_1, only: external_perm_ppattj_ttxddxagg_1, &
    & external_perm_inv_ppattj_ttxddxagg_1, extcomb_perm_ppattj_ttxddxagg_1, &
    & average_factor_ppattj_ttxddxagg_1
  use ol_external_ppattj_ttxddxagg_1, only: H, hel_not_initialised, hel_init
  use ol_colourmatrix_ppattj_ttxddxagg_1_/**/REALKIND, only: colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppattj_ttxddxagg_1_/**/REALKIND, only: check_forced_parameters
  use ol_heltables_ppattj_ttxddxagg_1
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_ppattj_ttxddxagg_1,7)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppattj_ttxddxagg_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppattj_ttxddxagg_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_ppattj_ttxddxagg_1(extcombs(k))
  end do
  ...
  call prop_A_Q(ntry, wf4(:,22), Q(:,66), MT, 1_intkind1, wf4(:,23), n2(28))
  call vert_VQ_A(ntry, ex6, wf4(:,8), wf8(:,44), n3(:,74), t3x8(:,:,26))
  call vert_AV_Q(ntry, wf4(:,23), wf4(:,7), wf16(:,42), n3(:,75), t3x16(:,:,36))
  call prop_Q_A(ntry, wf8(:,44), Q(:,49), MT, 1_intkind1, wf8(:,45), n2(29))
  call vert_AV_Q(ntry, wf4(:,23), ex6, wf8(:,46), n3(:,76), t3x8(:,:,27))
  call prop_A_Q(ntry, wf8(:,46), Q(:,98), MT, 1_intkind1, wf8(:,47), n2(30))
  ...
  call prop_A_Q(ntry, wf8(:,49), Q(:,14), MT, 1_intkind1, wf8(:,50), n2(31))
  call vert_AV_Q(ntry, ex2, wf4(:,15), wf8(:,51), n3(:,82), t3x8(:,:,30))
  call prop_A_Q(ntry, wf8(:,51), Q(:,98), MT, 1_intkind1, wf8(:,52), n2(32))
  ...
  call prop_Q_A(ntry, wf8(:,61), Q(:,13), MT, 1_intkind1, wf8(:,62), n2(39))
  call vert_VQ_A(ntry, wf4(:,15), ex1, wf8(:,63), n3(:,102), t3x8(:,:,36))
  call prop_Q_A(ntry, wf8(:,63), Q(:,97), MT, 1_intkind1, wf8(:,64), n2(40))
  ...
  call prop_Q_A(ntry, wf8(:,65), Q(:,49), MT, 1_intkind1, wf8(:,66), n2(41))
  call vert_AV_Q(ntry, wf4(:,23), ex5, wf8(:,67), n3(:,110), t3x8(:,:,38))
  call prop_A_Q(ntry, wf8(:,67), Q(:,82), MT, 1_intkind1, wf8(:,68), n2(42))
  ...
  call prop_Q_A(ntry, wf8(:,69), Q(:,81), MT, 1_intkind1, wf8(:,70), n2(43))
  call vert_AV_Q(ntry, wf4(:,21), ex5, wf8(:,71), n3(:,114), t3x8(:,:,40))
  call prop_A_Q(ntry, wf8(:,71), Q(:,50), MT, 1_intkind1, wf8(:,72), n2(44))
  ...
  M2add = M2 / average_factor_ppattj_ttxddxagg_1

  do k = 0, 30-1
    M2(k) = M2add(extcomb_perm_ppattj_ttxddxagg_1(k))
  end do
...
subroutine colourvector(A, j, M1)
  ...
  M1( 1) = ((A(j,159)%j+A(j,160)%j+A(j,161)%j+A(j,164)%j+A(j,179)%j+A(j,180)%j+A(j,181)%j+A(j,184)%j+A(j,197)%j+A(j,199)%j &
        ...
        -A(j,136)%j-A(j,140)%j-A(j,198)%j-A(j,200)%j-A(j,202)%j-A(j,204)%j)*f(2))/6._/**/REALKIND
  M1( 2) = ((A(j,1)%j-A(j,3)%j+A(j,4)%j-A(j,6)%j-A(j,14)%j-A(j,24)%j-A(j,26)%j-A(j,33)%j-A(j,39)%j-A(j,41)%j-A(j,48)%j-A(j,49)%j &
        ...
        -A(j,7)%j+A(j,9)%j-A(j,10)%j+A(j,12)%j+A(j,64)%j+A(j,70)%j+A(j,71)%j+A(j,77)%j+A(j,85)%j+A(j,86)%j+A(j,91)%j+A(j,92)%j &
        +A(j,116)%j+A(j,117)%j+A(j,123)%j+A(j,129)%j+A(j,137)%j+A(j,140)%j+A(j,190)%j+A(j,204)%j)*f(2))/2._/**/REALKIND+(CI*( &
        -A(j,13)%j+A(j,25)%j+A(j,31)%j-A(j,42)%j-A(j,52)%j+A(j,55)%j-A(j,152)%j-A(j,165)%j-A(j,172)%j &
        -A(j,185)%j)*f(3))/2._/**/REALKIND+(CI*(A(j,66)%j-A(j,73)%j-A(j,78)%j+A(j,90)%j-A(j,94)%j+A(j,106)%j+A(j,119)%j-A(j,122)%j &
  ...
  M1( 3) = ((A(j,1)%j-A(j,3)%j+A(j,4)%j-A(j,6)%j-A(j,17)%j-A(j,24)%j-A(j,26)%j-A(j,30)%j-A(j,39)%j-A(j,41)%j-A(j,43)%j-A(j,44)%j &
        ...
        -A(j,7)%j+A(j,9)%j-A(j,10)%j+A(j,12)%j+A(j,61)%j+A(j,70)%j+A(j,71)%j+A(j,80)%j+A(j,85)%j+A(j,86)%j+A(j,96)%j+A(j,97)%j &
        +A(j,110)%j+A(j,112)%j+A(j,115)%j+A(j,131)%j+A(j,132)%j+A(j,146)%j+A(j,196)%j+A(j,198)%j)*f(2))/2._/**/REALKIND+(CI*( &
        -A(j,16)%j+A(j,27)%j+A(j,28)%j-A(j,40)%j+A(j,47)%j-A(j,58)%j+A(j,157)%j+A(j,162)%j+A(j,177)%j &
        +A(j,182)%j)*f(3))/2._/**/REALKIND+(CI*(A(j,63)%j-A(j,75)%j-A(j,81)%j+A(j,89)%j+A(j,100)%j-A(j,102)%j-A(j,114)%j &
  ...
  M1( 4) = ((A(j,150)%j+A(j,151)%j+A(j,155)%j+A(j,156)%j+A(j,170)%j+A(j,171)%j+A(j,175)%j+A(j,176)%j+A(j,189)%j+A(j,191)%j &
        ...
        -A(j,143)%j-A(j,146)%j-A(j,190)%j-A(j,192)%j-A(j,194)%j-A(j,196)%j)*f(2))/6._/**/REALKIND
  M1( 5) = ((A(j,154)%j+A(j,158)%j+A(j,163)%j+A(j,174)%j+A(j,178)%j+A(j,183)%j)*f(1))/6._/**/REALKIND+((-A(j,61)%j-A(j,62)%j &
        -A(j,74)%j-A(j,79)%j-A(j,80)%j-A(j,88)%j-A(j,96)%j-A(j,97)%j-A(j,98)%j-A(j,99)%j-A(j,101)%j &
        -A(j,103)%j)*f(2))/6._/**/REALKIND+(CI*(A(j,167)%j+A(j,168)%j+A(j,187)%j+A(j,188)%j)*f(3))/6._/**/REALKIND+(CI*(-A(j,68)%j &
        -A(j,69)%j-A(j,83)%j-A(j,84)%j-A(j,107)%j-A(j,108)%j)*f(4))/6._/**/REALKIND
  M1( 6) = ((-A(j,1)%j+A(j,2)%j-A(j,4)%j+A(j,5)%j-A(j,20)%j-A(j,23)%j+A(j,24)%j-A(j,32)%j-A(j,35)%j+A(j,39)%j-A(j,53)%j-A(j,54)%j &
        -A(j,156)%j-A(j,158)%j-A(j,175)%j-A(j,178)%j-A(j,193)%j)*f(1))/2._/**/REALKIND+((A(j,8)%j-A(j,9)%j+A(j,11)%j-A(j,12)%j &
        +A(j,62)%j-A(j,67)%j-A(j,70)%j-A(j,82)%j-A(j,85)%j+A(j,88)%j+A(j,98)%j+A(j,99)%j+A(j,109)%j+A(j,121)%j+A(j,141)%j &
        +A(j,143)%j+A(j,194)%j)*f(2))/2._/**/REALKIND+(CI*(-A(j,19)%j-A(j,25)%j-A(j,31)%j-A(j,36)%j-A(j,55)%j-A(j,60)%j-A(j,157)%j &
        -A(j,167)%j-A(j,177)%j-A(j,187)%j)*f(3))/2._/**/REALKIND+(CI*(-A(j,63)%j+A(j,69)%j+A(j,83)%j-A(j,89)%j-A(j,100)%j &
  ...
  M1( 7) = ((-A(j,2)%j+A(j,3)%j-A(j,5)%j+A(j,6)%j-A(j,15)%j+A(j,20)%j+A(j,26)%j+A(j,35)%j-A(j,38)%j+A(j,41)%j-A(j,50)%j-A(j,51)%j &
        -A(j,159)%j-A(j,163)%j-A(j,181)%j-A(j,183)%j-A(j,201)%j)*f(1))/2._/**/REALKIND+((A(j,7)%j-A(j,8)%j+A(j,10)%j-A(j,11)%j &
        +A(j,67)%j-A(j,71)%j+A(j,74)%j+A(j,79)%j+A(j,82)%j-A(j,86)%j+A(j,101)%j+A(j,103)%j+A(j,118)%j+A(j,120)%j+A(j,136)%j &
        +A(j,138)%j+A(j,202)%j)*f(2))/2._/**/REALKIND+(CI*(A(j,13)%j-A(j,21)%j-A(j,34)%j+A(j,42)%j+A(j,52)%j-A(j,59)%j-A(j,162)%j &
        -A(j,168)%j-A(j,182)%j-A(j,188)%j)*f(3))/2._/**/REALKIND+(CI*(A(j,68)%j+A(j,75)%j+A(j,81)%j+A(j,84)%j+A(j,102)%j &
        +A(j,108)%j-A(j,119)%j+A(j,127)%j-A(j,139)%j+A(j,147)%j)*f(4))/2._/**/REALKIND
  M1( 8) = ((A(j,14)%j+A(j,15)%j+A(j,23)%j+A(j,32)%j+A(j,33)%j+A(j,38)%j+A(j,48)%j+A(j,49)%j+A(j,50)%j+A(j,51)%j+A(j,53)%j &
        +A(j,54)%j)*f(1))/6._/**/REALKIND+((-A(j,117)%j-A(j,118)%j-A(j,121)%j-A(j,137)%j-A(j,138)%j &
        -A(j,141)%j)*f(2))/6._/**/REALKIND+(CI*(A(j,19)%j+A(j,21)%j+A(j,34)%j+A(j,36)%j+A(j,59)%j+A(j,60)%j)*f(3))/6._/**/REALKIND &
        +(CI*(-A(j,127)%j-A(j,128)%j-A(j,147)%j-A(j,148)%j)*f(4))/6._/**/REALKIND
  M1( 9) = ((A(j,149)%j+A(j,153)%j+A(j,166)%j+A(j,169)%j+A(j,173)%j+A(j,186)%j)*f(1))/6._/**/REALKIND+((-A(j,64)%j-A(j,65)%j &
        -A(j,72)%j-A(j,76)%j-A(j,77)%j-A(j,87)%j-A(j,91)%j-A(j,92)%j-A(j,93)%j-A(j,95)%j-A(j,104)%j &
        -A(j,105)%j)*f(2))/6._/**/REALKIND+(CI*(-A(j,167)%j-A(j,168)%j-A(j,187)%j-A(j,188)%j)*f(3))/6._/**/REALKIND+(CI*(A(j,68)%j &
        +A(j,69)%j+A(j,83)%j+A(j,84)%j+A(j,107)%j+A(j,108)%j)*f(4))/6._/**/REALKIND
  M1(10) = ((-A(j,2)%j+A(j,3)%j-A(j,5)%j+A(j,6)%j+A(j,20)%j-A(j,22)%j+A(j,26)%j-A(j,29)%j+A(j,35)%j+A(j,41)%j-A(j,45)%j-A(j,46)%j &
        -A(j,164)%j-A(j,166)%j-A(j,180)%j-A(j,186)%j-A(j,199)%j)*f(1))/2._/**/REALKIND+((A(j,7)%j-A(j,8)%j+A(j,10)%j-A(j,11)%j &
        +A(j,65)%j+A(j,67)%j-A(j,71)%j+A(j,82)%j-A(j,86)%j+A(j,87)%j+A(j,104)%j+A(j,105)%j+A(j,111)%j+A(j,113)%j+A(j,133)%j &
        +A(j,135)%j+A(j,200)%j)*f(2))/2._/**/REALKIND+(CI*(A(j,19)%j-A(j,27)%j-A(j,28)%j+A(j,36)%j-A(j,47)%j+A(j,60)%j+A(j,165)%j &
        +A(j,167)%j+A(j,185)%j+A(j,187)%j)*f(3))/2._/**/REALKIND+(CI*(-A(j,66)%j-A(j,69)%j-A(j,83)%j-A(j,90)%j-A(j,106)%j &
        -A(j,107)%j+A(j,114)%j-A(j,128)%j+A(j,134)%j-A(j,148)%j)*f(4))/2._/**/REALKIND
  M1(11) = ((-A(j,1)%j+A(j,2)%j-A(j,4)%j+A(j,5)%j-A(j,18)%j-A(j,20)%j+A(j,24)%j-A(j,35)%j-A(j,37)%j+A(j,39)%j-A(j,56)%j-A(j,57)%j &
        -A(j,150)%j-A(j,153)%j-A(j,171)%j-A(j,173)%j-A(j,191)%j)*f(1))/2._/**/REALKIND+((A(j,8)%j-A(j,9)%j+A(j,11)%j-A(j,12)%j &
        -A(j,67)%j-A(j,70)%j+A(j,72)%j+A(j,76)%j-A(j,82)%j-A(j,85)%j+A(j,93)%j+A(j,95)%j+A(j,124)%j+A(j,126)%j+A(j,130)%j &
        +A(j,144)%j+A(j,192)%j)*f(2))/2._/**/REALKIND+(CI*(A(j,16)%j+A(j,21)%j+A(j,34)%j+A(j,40)%j+A(j,58)%j+A(j,59)%j+A(j,152)%j &
        +A(j,168)%j+A(j,172)%j+A(j,188)%j)*f(3))/2._/**/REALKIND+(CI*(-A(j,68)%j+A(j,73)%j+A(j,78)%j-A(j,84)%j+A(j,94)%j &
  ...
  M1(12) = ((A(j,17)%j+A(j,18)%j+A(j,22)%j+A(j,29)%j+A(j,30)%j+A(j,37)%j+A(j,43)%j+A(j,44)%j+A(j,45)%j+A(j,46)%j+A(j,56)%j &
        +A(j,57)%j)*f(1))/6._/**/REALKIND+((-A(j,112)%j-A(j,113)%j-A(j,124)%j-A(j,132)%j-A(j,133)%j &
        -A(j,144)%j)*f(2))/6._/**/REALKIND+(CI*(-A(j,19)%j-A(j,21)%j-A(j,34)%j-A(j,36)%j-A(j,59)%j &
        -A(j,60)%j)*f(3))/6._/**/REALKIND+(CI*(A(j,127)%j+A(j,128)%j+A(j,147)%j+A(j,148)%j)*f(4))/6._/**/REALKIND
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppattj_ttxddxagg_1_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppattj_ttxddxagg_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppattj_ttxddxagg_1")
#else
subroutine amp2tree(P, M2)
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppattj_ttxddxagg_1")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppattj_ttxddxagg_1")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppattj_ttxddxagg_1")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppattj_ttxddxagg_1")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppattj_ttxddxagg_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppattj_ttxddxagg_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppattj_ttxddxagg_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppattj_ttxddxagg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppattj_ttxddxagg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2tree_legacy(p, m2) &
    & bind(c,name="amp2tree_ppattj_ttxddxagg_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccone_legacy(p, m2, i, j) &
    & bind(c,name="amp2ccone_ppattj_ttxddxagg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccall_legacy(p, m2) &
    & bind(c,name="amp2ccall_ppattj_ttxddxagg_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(0:30-1)
...
subroutine amp2hcone_legacy(p, m2, i, j, mom) &
    & bind(c,name="amp2hcone_ppattj_ttxddxagg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_legacy(p, m2, i, mom) &
    & bind(c,name="amp2hcall_ppattj_ttxddxagg_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(7)
...
end module ol_tree_ppattj_ttxddxagg_1_/**/REALKIND
