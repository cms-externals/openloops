...
module ol_versiondata_ppwa
  implicit none
  character(4) :: generator_revision = "1248"
  interface
    subroutine version_ppwa(outstring)
      implicit none
      character(60), intent(out), optional :: outstring
    end subroutine version_ppwa
  end interface
end module ol_versiondata_ppwa


subroutine ol_version_ppwa(outstring)
  use ol_version, only: version, revision
  use ol_versiondata_ppwa, only: generator_revision
  implicit none
  character(60), intent(out), optional :: outstring
...
end subroutine ol_version_ppwa
