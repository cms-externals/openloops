...
module ol_external_ppwa_udxawg_1
  implicit none
  integer :: dummy_counter
  ! Permutation and inverse permutation of external particles
  integer, save :: external_perm_ppwa_udxawg_1(5) = &
                     [ (dummy_counter, dummy_counter = 1, 5) ]
  integer, save :: external_perm_inv_ppwa_udxawg_1(5) = &
                     [ (dummy_counter, dummy_counter = 1, 5) ]
  integer, save :: extcomb_perm_ppwa_udxawg_1(0:16) = &
                     [ (dummy_counter, dummy_counter = 0, 16) ]
  ! Particle types (mapping of fields to integers is not fixed!)
  integer, save :: particle_types_ppwa_udxawg_1(5) = &
                     [ 1, 2, 3, 4, 5 ]
  ! Colour and helicity average factors per particle
  integer, save :: average_factors_ppwa_udxawg_1(5) = &
                     [ 6, 6, 2, 3, 16 ]
  ! Average factor; initialised to the identity permutation
  integer, save :: average_factor_ppwa_udxawg_1 = &
                     36
  integer, save :: channel_number_ppwa_udxawg_1 = -1
  ...
  subroutine n_external(n) &
      & bind(c,name="ol_f_n_external_ppwa_udxawg_1")
  ...
  subroutine n_external_c(n) &
      & bind(c,name="ol_n_external_ppwa_udxawg_1")
  ...
  subroutine set_permutation(perm) &
      & bind(c,name="ol_f_set_permutation_ppwa_udxawg_1")
    use ol_parameters_decl_/**/DREALKIND, only: out_symmetry_on
    use ol_generic, only: factorial
    implicit none
    integer, intent(in) :: perm(5)
    integer :: i, j, ii, jj
    integer :: particle_types_perm_ppwa_udxawg_1(5)
    external_perm_ppwa_udxawg_1 = perm
    do i = 1, 5
      external_perm_inv_ppwa_udxawg_1( &
        external_perm_ppwa_udxawg_1(i)) = i
      particle_types_perm_ppwa_udxawg_1(i) = &
        particle_types_ppwa_udxawg_1( &
        external_perm_ppwa_udxawg_1(i))
    end do
    do i = 1, 5
      do j = 1, i
        if (external_perm_ppwa_udxawg_1(i) >= &
          external_perm_ppwa_udxawg_1(j)) then
          ii = external_perm_ppwa_udxawg_1(i)
          jj = external_perm_ppwa_udxawg_1(j)
        else
          ii = external_perm_ppwa_udxawg_1(j)
          jj = external_perm_ppwa_udxawg_1(i)
        end if
        extcomb_perm_ppwa_udxawg_1((i*(i-1))/2 + j) = (ii*(ii-1))/2 + jj
      end do
    end do
    ! Colour and helicity average factor
    average_factor_ppwa_udxawg_1 = &
      average_factors_ppwa_udxawg_1( &
      external_perm_ppwa_udxawg_1(1)) &
      * average_factors_ppwa_udxawg_1( &
      external_perm_ppwa_udxawg_1(2))
    ! Symmetry factor for outgoing particles
    if (out_symmetry_on /= 0) then
      do i = 1, 5
        average_factor_ppwa_udxawg_1 = &
          average_factor_ppwa_udxawg_1 &
          * factorial(count(particle_types_perm_ppwa_udxawg_1(3:5) == i))
      end do
    end if
  ...
  subroutine set_permutation_c(perm) &
      & bind(c,name="ol_set_permutation_ppwa_udxawg_1")
    use, intrinsic :: iso_c_binding, only: c_int
    implicit none
  ...
  subroutine get_masses(m_ex) &
      & bind(c,name="ol_f_get_masses_ppwa_udxawg_1")
    ...
    m_ex_orig = [ rZERO, rZERO, rZERO, rMW_unscaled, rZERO ]
    do i = 1, 5
      m_ex(i) = m_ex_orig(external_perm_ppwa_udxawg_1(i))
    end do
  end subroutine get_masses


  subroutine get_masses_c(m_ex) &
      & bind(c,name="ol_get_masses_ppwa_udxawg_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo(sqrt_s, p_rambo) &
      & bind(c,name="ol_f_rambo_ppwa_udxawg_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo_c(sqrt_s, p_rambo) &
      & bind(c,name="ol_rambo_ppwa_udxawg_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
...
end module ol_external_ppwa_udxawg_1


module colour_basis_ppwa_udxawg_1
  implicit none
  ! tree colour basis
  ...
  pure subroutine tree_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_tree_colbasis_dim_ppwa_udxawg_1")
    implicit none
  ...
  subroutine tree_colbasis(basis, powers) &
    & bind(c, name="ol_tree_colbasis_ppwa_udxawg_1")
    implicit none
...
end module colour_basis_ppwa_udxawg_1


! Only for compatibility with the Sherpa interface

subroutine set_permutation_ppwa_udxawg_1(perm)
  use ol_external_ppwa_udxawg_1, only: set_permutation
  implicit none
  integer, intent(in) :: perm(5)
  call set_permutation(perm)
end subroutine set_permutation_ppwa_udxawg_1

! **********************************************************************
module ol_heltables_ppwa_udxawg_1
! **********************************************************************
  ...
  integer(intkind2), save :: t3x6(2,6,3), t3x4(2,4,4), t3x8(2,8,4), t3x12(2,12,4), t3x48(2,48,8)
...
! **********************************************************************
  ...
  call heltable([3,2,6], n3(:,13), t3x6(:,:,3))
  n2(7) = 6
  call heltable([4,2,8], n3(:,14), t3x8(:,:,3))
  call heltable([2,4,8], n3(:,15), t3x8(:,:,4))
  call heltable([6,8,48], n3(:,16), t3x48(:,:,1))
  call heltable([4,12,48], n3(:,17), t3x48(:,:,2))
  call heltable([4,12,48], n3(:,18), t3x48(:,:,3))
  call heltable([4,12,48], n3(:,19), t3x48(:,:,4))
  call heltable([4,12,48], n3(:,20), t3x48(:,:,5))
  call heltable([6,8,48], n3(:,21), t3x48(:,:,6))
  call heltable([6,8,48], n3(:,22), t3x48(:,:,7))
  call heltable([6,8,48], n3(:,23), t3x48(:,:,8))

  heltables_not_init = .false.

end subroutine init_heltables

end module ol_heltables_ppwa_udxawg_1
