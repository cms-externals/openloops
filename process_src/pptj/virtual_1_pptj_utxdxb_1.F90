...
module ol_vamp_1_pptj_utxdxb_1_/**/REALKIND
contains

! **********************************************************************
subroutine vamp_1(M)
! P(0:3,nlegs) = incoming external momenta
! Uses tree structures 'wf', factors 'c', and denominators 'den' from loop_pptj_utxdxb_1.
! Sets colour stripped amplitudes A from the module loop_amplitudes_pptj_utxdxb_1.
! **********************************************************************
...
#endif
  use ol_loop_pptj_utxdxb_1_/**/REALKIND, only: c, f, wf, den
  use ol_vert_interface_/**/REALKIND
  use ol_prop_interface_/**/REALKIND
  use ol_last_step_/**/REALKIND
  use ol_tensor_sum_storage_pptj_utxdxb_1_/**/REALKIND
  ...
  call loop_Q_A(G1(:,:,:,2),Q(:,13),MT,G2(:,:,:,1))
  call check_last_QA_V(l_switch,G2(:,:,:,1),wf(:,-1),G2tensor(:,1))
  call loop_QA_W(G1(:,:,:,1),wf(:,-1),G1(:,:,:,3))
  call loop_WQ_A(G1(:,:,:,3),wf(:,0),G1(:,:,:,4))
  call loop_Q_A(G1(:,:,:,4),Q(:,11),ZERO,G2(:,:,:,2))
  call check_last_QA_V(l_switch,G2(:,:,:,2),wf(:,-2),G2tensor(:,2))
  call loop_WA_Q(G1(:,:,:,3),wf(:,-2),G1(:,:,:,5))
  call loop_A_Q(G1(:,:,:,5),Q(:,14),ZERO,G2(:,:,:,3))
  call check_last_AQ_V(l_switch,G2(:,:,:,3),wf(:,0),G2tensor(:,3))
  call loop_VA_Q(G0(:,:,:,1),wf(:,-2),G0(:,:,:,3))
  call loop_A_Q(G0(:,:,:,3),Q(:,4),ZERO,G1(:,:,:,6))
  call loop_AW_Q(G1(:,:,:,6),wf(:,2),G1(:,:,:,7))
  call loop_A_Q(G1(:,:,:,7),Q(:,14),ZERO,G2(:,:,:,4))
  call check_last_AQ_V(l_switch,G2(:,:,:,4),wf(:,0),G2tensor(:,4))
  call loop_AQ_W(G1(:,:,:,6),wf(:,0),G1(:,:,:,8))
  call loop_WQ_A(G1(:,:,:,8),wf(:,-3),G1(:,:,:,9))
  call loop_Q_A(G1(:,:,:,9),Q(:,13),MT,G2(:,:,:,5))
  call check_last_QA_V(l_switch,G2(:,:,:,5),wf(:,-1),G2tensor(:,5))
  call loop_VA_Q(G0(:,:,:,1),wf(:,-1),G0(:,:,:,4))
  call loop_A_Q(G0(:,:,:,4),Q(:,2),MT,G1(:,:,:,10))
  call loop_AQ_W(G1(:,:,:,10),wf(:,-3),G1(:,:,:,11))
  call loop_WA_Q(G1(:,:,:,11),wf(:,-2),G1(:,:,:,12))
  call loop_A_Q(G1(:,:,:,12),Q(:,14),ZERO,G2(:,:,:,6))
...
end module ol_vamp_1_pptj_utxdxb_1_/**/REALKIND
