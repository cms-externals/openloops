...
module ol_colourmatrix_ppllllj_nexnmemxbbxg_1_/**/REALKIND
...
end module ol_colourmatrix_ppllllj_nexnmemxbbxg_1_/**/REALKIND



module ol_forced_parameters_ppllllj_nexnmemxbbxg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllllj_nexnmemxbbxg_1_/**/REALKIND

module ol_loop_ppllllj_nexnmemxbbxg_1_/**/REALKIND
...
! **********************************************************************
  ...
  call vert_VV_S(wf(:,4),wf(:,5),wf(:,11))
  call vert_QA_V(wf(:,6),wf(:,-5),wf(:,12))
  call vert_UV_W(wf(:,5),Q(:,10),wf(:,4),Q(:,5),wf(:,13))
  ...
  call prop_Q_A(wf(:,18),Q(:,26),MT,1_intkind1,wf(:,20))
  ...
  call vert_QA_Z(gZl,wf(:,27),wf(:,-3),wf(:,29))
  call vert_AW_Q(wf(:,-3),wf(:,4),wf(:,30))
  call prop_A_Q(wf(:,30),Q(:,13),ZERO,0_intkind1,wf(:,31))
  call vert_QA_Z(gZn,wf(:,-1),wf(:,31),wf(:,32))
  call vert_AW_Q(wf(:,0),wf(:,5),wf(:,33))
  call prop_A_Q(wf(:,33),Q(:,11),ZERO,0_intkind1,wf(:,34))
  call vert_QA_V(wf(:,-2),wf(:,34),wf(:,35))
  call vert_QA_Z(gZl,wf(:,-2),wf(:,34),wf(:,36))
  call vert_WQ_A(wf(:,5),wf(:,-2),wf(:,37))
  call prop_Q_A(wf(:,37),Q(:,14),ZERO,0_intkind1,wf(:,38))
  call vert_QA_Z(gZn,wf(:,38),wf(:,0),wf(:,39))
  call vert_QA_V(wf(:,-4),wf(:,-5),wf(:,40))
  call counter_VVG_G(wf(:,4),wf(:,5),wf(:,-6),wf(:,41))
  ...
  call prop_Q_A(wf(:,8),Q(:,90),MT,1_intkind1,wf(:,52))
  call prop_A_Q(wf(:,51),Q(:,37),MT,1_intkind1,wf(:,53))
  ...
  call prop_A_Q(wf(:,19),Q(:,101),MT,1_intkind1,wf(:,65))
  call prop_Q_A(wf(:,64),Q(:,26),MT,1_intkind1,wf(:,66))
  ...
  call counter_A_Q(cttt,wf(:,9),Q(:,37),wf(:,84))
  ...
  call counter_Q_A(cttt,wf(:,20),Q(:,26),wf(:,90))
  call vert_AW_Q(wf(:,86),wf(:,4),wf(:,91))
  call vert_AV_Q(wf(:,9),wf(:,-6),wf(:,92))
  call prop_A_Q(wf(:,92),Q(:,101),MT,1_intkind1,wf(:,93))
  call prop_Q_A(wf(:,25),Q(:,90),MT,1_intkind1,wf(:,94))
  ...
  call vert_AW_Q(wf(:,9),wf(:,5),wf(:,99))
  call prop_A_Q(wf(:,99),Q(:,47),MB,1_intkind1,wf(:,100))
  call vert_WQ_A(wf(:,4),wf(:,20),wf(:,101))
  call prop_Q_A(wf(:,101),Q(:,31),MB,1_intkind1,wf(:,102))
...
subroutine denominators()
  ...
  den(59) = 1 / (Q(5,15))
  den(63) = 1 / (Q(5,90) - MT2)
  den(69) = 1 / (Q(5,101) - MT2)
  den(114) = 1 / (Q(5,47) - MB2)
  den(117) = 1 / (Q(5,31) - MB2)
  ...
  den(116) = den(1)*den(21)
  den(118) = den(116)*den(117)
  den(119) = den(53)*den(117)
  den(120) = den(60)*den(117)
  den(121) = den(56)*den(117)
  den(122) = den(53)*den(114)
  den(123) = den(60)*den(114)
  den(124) = den(56)*den(114)
  den(125) = den(95)*den(117)
  den(126) = den(97)*den(117)
  den(127) = den(95)*den(114)
  den(128) = den(97)*den(114)
  den(129) = den(99)*den(117)
  den(130) = den(99)*den(114)
  den(131) = den(104)*den(117)
  den(132) = den(106)*den(117)
  den(133) = den(104)*den(114)
  den(134) = den(106)*den(114)
  den(135) = den(108)*den(117)
  den(136) = den(108)*den(114)
...
end module ol_loop_ppllllj_nexnmemxbbxg_1_/**/REALKIND
