...
module ol_external_pphttj_uuxttxddxh_1
  implicit none
  integer :: dummy_counter
  ! Permutation and inverse permutation of external particles
  integer, save :: external_perm_pphttj_uuxttxddxh_1(7) = &
                     [ (dummy_counter, dummy_counter = 1, 7) ]
  integer, save :: external_perm_inv_pphttj_uuxttxddxh_1(7) = &
                     [ (dummy_counter, dummy_counter = 1, 7) ]
  integer, save :: extcomb_perm_pphttj_uuxttxddxh_1(0:29) = &
                     [ (dummy_counter, dummy_counter = 0, 29) ]
  ! Particle types (mapping of fields to integers is not fixed!)
  integer, save :: particle_types_pphttj_uuxttxddxh_1(7) = &
                     [ 1, 2, 3, 4, 5, 6, 7 ]
  ! Colour and helicity average factors per particle
  integer, save :: average_factors_pphttj_uuxttxddxh_1(7) = &
                     [ 6, 6, 6, 6, 6, 6, 1 ]
  ! Average factor; initialised to the identity permutation
  integer, save :: average_factor_pphttj_uuxttxddxh_1 = &
                     36
  integer, save :: channel_number_pphttj_uuxttxddxh_1 = -1
  ...
  subroutine n_external(n) &
      & bind(c,name="ol_f_n_external_pphttj_uuxttxddxh_1")
  ...
  subroutine n_external_c(n) &
      & bind(c,name="ol_n_external_pphttj_uuxttxddxh_1")
  ...
  subroutine set_permutation(perm) &
      & bind(c,name="ol_f_set_permutation_pphttj_uuxttxddxh_1")
    use ol_parameters_decl_/**/DREALKIND, only: out_symmetry_on
    use ol_generic, only: factorial
    implicit none
    integer, intent(in) :: perm(7)
    integer :: i, j, ii, jj
    integer :: particle_types_perm_pphttj_uuxttxddxh_1(7)
    external_perm_pphttj_uuxttxddxh_1 = perm
    do i = 1, 7
      external_perm_inv_pphttj_uuxttxddxh_1( &
        external_perm_pphttj_uuxttxddxh_1(i)) = i
      particle_types_perm_pphttj_uuxttxddxh_1(i) = &
        particle_types_pphttj_uuxttxddxh_1( &
        external_perm_pphttj_uuxttxddxh_1(i))
    end do
    do i = 1, 7
      do j = 1, i
        if (external_perm_pphttj_uuxttxddxh_1(i) >= &
          external_perm_pphttj_uuxttxddxh_1(j)) then
          ii = external_perm_pphttj_uuxttxddxh_1(i)
          jj = external_perm_pphttj_uuxttxddxh_1(j)
        else
          ii = external_perm_pphttj_uuxttxddxh_1(j)
          jj = external_perm_pphttj_uuxttxddxh_1(i)
        end if
        extcomb_perm_pphttj_uuxttxddxh_1((i*(i-1))/2 + j) = (ii*(ii-1))/2 + jj
      end do
    end do
    ! Colour and helicity average factor
    average_factor_pphttj_uuxttxddxh_1 = &
      average_factors_pphttj_uuxttxddxh_1( &
      external_perm_pphttj_uuxttxddxh_1(1)) &
      * average_factors_pphttj_uuxttxddxh_1( &
      external_perm_pphttj_uuxttxddxh_1(2))
    ! Symmetry factor for outgoing particles
    if (out_symmetry_on /= 0) then
      do i = 1, 7
        average_factor_pphttj_uuxttxddxh_1 = &
          average_factor_pphttj_uuxttxddxh_1 &
          * factorial(count(particle_types_perm_pphttj_uuxttxddxh_1(3:7) == i))
      end do
    end if
  ...
  subroutine set_permutation_c(perm) &
      & bind(c,name="ol_set_permutation_pphttj_uuxttxddxh_1")
    use, intrinsic :: iso_c_binding, only: c_int
    implicit none
  ...
  subroutine get_masses(m_ex) &
      & bind(c,name="ol_f_get_masses_pphttj_uuxttxddxh_1")
    ...
    do i = 1, 7
      m_ex(i) = m_ex_orig(external_perm_pphttj_uuxttxddxh_1(i))
    end do
  end subroutine get_masses


  subroutine get_masses_c(m_ex) &
      & bind(c,name="ol_get_masses_pphttj_uuxttxddxh_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo(sqrt_s, p_rambo) &
      & bind(c,name="ol_f_rambo_pphttj_uuxttxddxh_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo_c(sqrt_s, p_rambo) &
      & bind(c,name="ol_rambo_pphttj_uuxttxddxh_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
  ...
  subroutine pol_init(pol) &
      & bind(c,name="ol_f_pol_init_pphttj_uuxttxddxh_1")
    implicit none
    integer, intent(in) :: pol(7)
    POLSEL = pol
  end subroutine pol_init

end module ol_external_pphttj_uuxttxddxh_1


module colour_basis_pphttj_uuxttxddxh_1
  implicit none
  ! tree colour basis
  ...
  pure subroutine tree_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_tree_colbasis_dim_pphttj_uuxttxddxh_1")
    implicit none
  ...
  subroutine tree_colbasis(basis, powers) &
    & bind(c, name="ol_tree_colbasis_pphttj_uuxttxddxh_1")
    implicit none
...
end module colour_basis_pphttj_uuxttxddxh_1


! Only for compatibility with the Sherpa interface

subroutine set_permutation_pphttj_uuxttxddxh_1(perm)
  use ol_external_pphttj_uuxttxddxh_1, only: set_permutation
  implicit none
  integer, intent(in) :: perm(7)
  call set_permutation(perm)
end subroutine set_permutation_pphttj_uuxttxddxh_1

! **********************************************************************
module ol_heltables_pphttj_uuxttxddxh_1
! **********************************************************************
  ...
  integer(intkind2), save :: t3x2(2,2,2), t3x16(2,16,3), t3x4(2,4,4), t3x64(2,64,16), t3x8(2,8,18)
...
! **********************************************************************
  ...
  call heltable([2,4,8], n3(:,20), t3x8(:,:,13))
  n2(8) = 8
  call heltable([4,2,8], n3(:,21), t3x8(:,:,14))
  call heltable([2,4,8], n3(:,22), t3x8(:,:,15))
  call heltable([4,2,8], n3(:,23), t3x8(:,:,16))
  call heltable([2,4,8], n3(:,24), t3x8(:,:,17))
  n2(9) = 8
  call heltable([2,8,16], n3(:,25), t3x16(:,:,2))
  call heltable([4,2,8], n3(:,26), t3x8(:,:,18))
  n2(10) = 8
  call heltable([8,2,16], n3(:,27), t3x16(:,:,3))
  call heltable([8,8,64], n3(:,28), t3x64(:,:,1))
  call heltable([4,16,64], n3(:,29), t3x64(:,:,2))
  call heltable([8,8,64], n3(:,30), t3x64(:,:,3))
  call heltable([8,8,64], n3(:,31), t3x64(:,:,4))
  call heltable([16,4,64], n3(:,32), t3x64(:,:,5))
  ...
  call heltable([4,16,64], n3(:,40), t3x64(:,:,13))
  call heltable([4,16,64], n3(:,41), t3x64(:,:,14))
  call heltable([4,16,64], n3(:,42), t3x64(:,:,15))
  call heltable([4,16,64], n3(:,43), t3x64(:,:,16))

  heltables_not_init = .false.

end subroutine init_heltables

end module ol_heltables_pphttj_uuxttxddxh_1
