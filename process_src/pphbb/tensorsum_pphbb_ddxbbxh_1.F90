...
! **********************************************************************
module ol_tensor_sum_storage_pphbb_ddxbbxh_1_/**/REALKIND
! **********************************************************************
  ...
  complex(REALKIND), save :: T3sum(35,8) = 0

  contains

#ifdef PRECISION_dp
subroutine max_point(r) &
    & bind(c,name="ol_f_max_point_pphbb_ddxbbxh_1")
...
subroutine tensor_rank(r) &
    & bind(c,name="ol_f_tensor_rank_pphbb_ddxbbxh_1")
  ! Return the number maximal tensor rank
  implicit none
  integer, intent(out) :: r
  r = 3
...
! **********************************************************************
  use ol_external_pphbb_ddxbbxh_1, only: channel_number_pphbb_ddxbbxh_1
  ...
  call channel_on(channel_number_pphbb_ddxbbxh_1)
  M2 = 0

  momenta_1(:,1) = Q(1:4,20)
  momenta_2(:,1) = Q(1:4,24)
  momenta_3(:,1) = Q(1:4,28)
  momenta_4(:,1) = Q(1:4,2)
  momenta_4(:,2) = Q(1:4,30)
  momenta_5(:,1) = Q(1:4,4)
  momenta_5(:,2) = Q(1:4,28)
  momenta_6(:,1) = Q(1:4,8)
  ...
  call TI_call(1, momenta_2, masses2_2, T1sum(:,2), M2)

  call TI_call(2, momenta_10, masses2_7, T2sum(:,1), M2)
  call TI_call(2, momenta_7, masses2_8, T2sum(:,2), M2)
  call TI_call(2, momenta_11, masses2_7, T2sum(:,3), M2)
  call TI_call(2, momenta_5, masses2_8, T2sum(:,4), M2)
  call TI_call(2, momenta_6, masses2_7, T2sum(:,5), M2)
  call TI_call(2, momenta_8, masses2_7, T2sum(:,6), M2)
  call TI_call(2, momenta_4, masses2_11, T2sum(:,7), M2)
  call TI_call(2, momenta_12, masses2_11, T2sum(:,8), M2)
  call TI_call(2, momenta_17, masses2_14, T2sum(:,9), M2)
  call TI_call(2, momenta_18, masses2_14, T2sum(:,10), M2)
  call TI_call(2, momenta_14, masses2_14, T2sum(:,11), M2)
  call TI_call(2, momenta_19, masses2_14, T2sum(:,12), M2)
  call TI_call(2, momenta_3, masses2_5, T2sum(:,13), M2)
  call TI_call(2, momenta_3, masses2_3, T2sum(:,14), M2)
  call TI_call(2, momenta_3, masses2_4, T2sum(:,15), M2)
  call TI_call(2, momenta_3, masses2_1, T2sum(:,16), M2)

  call TI_call(3, momenta_9, masses2_9, T3sum(:,1), M2)
  call TI_call(3, momenta_9, masses2_10, T3sum(:,2), M2)
  call TI_call(3, momenta_9, masses2_6, T3sum(:,3), M2)
  call TI_call(3, momenta_15, masses2_12, T3sum(:,4), M2)
  call TI_call(3, momenta_16, masses2_12, T3sum(:,5), M2)
  call TI_call(3, momenta_13, masses2_13, T3sum(:,6), M2)
  call TI_call(3, momenta_20, masses2_15, T3sum(:,7), M2)
  call TI_call(3, momenta_21, masses2_15, T3sum(:,8), M2)

#ifdef LOOPSQUARED
  end if
#endif

  call channel_off(channel_number_pphbb_ddxbbxh_1)
end subroutine integrate_tensor_sum

end module ol_tensor_sum_storage_pphbb_ddxbbxh_1_/**/REALKIND
