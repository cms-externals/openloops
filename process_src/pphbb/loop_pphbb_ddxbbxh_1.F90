...
module ol_colourmatrix_pphbb_ddxbbxh_1_/**/REALKIND
...
end module ol_colourmatrix_pphbb_ddxbbxh_1_/**/REALKIND



module ol_forced_parameters_pphbb_ddxbbxh_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_pphbb_ddxbbxh_1_/**/REALKIND

module ol_loop_pphbb_ddxbbxh_1_/**/REALKIND
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f( 5) = (CI*countertermnorm*ctGqq*eQED*gQCD**4*YB)/(2._/**/REALKIND*MW*sw)
    ...
    f(13) = (eQED*gQCD**4*integralnorm*SwF*YC)/(MW*sw*2._/**/REALKIND)
  ...
  c = [ 9*CI*f(7), 27*CI*f(7), 18*f(8), 54*f(8), f(9), 3*f(9), 6*f(9), 8*f(9), 10*f(9), 18*f(9), 21*f(9), 24*f(9), 54*f(9) &
    , 3*f(10), 9*f(10), 3*f(11), 9*f(11), 3*f(13), 9*f(13), 3*f(15), 9*f(15) ]
  c = (1._/**/REALKIND / 36) * c
...
! **********************************************************************
  ...
  call counter_SG_G(wf(:,-4),wf(:,1),wf(:,9))
  call counter_AV_Q(wf(:,-3),wf(:,1),wf(:,10))
  call counter_SA_Q(gH,wf(:,-4),wf(:,-3),wf(:,11))
  call prop_A_Q(wf(:,11),Q(:,24),MB,1_intkind1,wf(:,12))
  call counter_VQ_A(wf(:,1),wf(:,-2),wf(:,13))
  call counter_QS_A(gH,wf(:,-2),wf(:,-4),wf(:,14))
  call prop_Q_A(wf(:,14),Q(:,20),MB,1_intkind1,wf(:,15))
  call counter_QA_V(wf(:,0),wf(:,-1),wf(:,16))
  call vert_AV_Q(wf(:,-3),wf(:,16),wf(:,17))
  call vert_VQ_A(wf(:,16),wf(:,-2),wf(:,18))
  call vert_QA_V(wf(:,3),wf(:,-3),wf(:,19))
  call counter_V_V(ctGG,wf(:,1),Q(:,3),wf(:,20))
  call counter_Q_A(ctbb,wf(:,3),Q(:,20),wf(:,21))
  call prop_A_Q(wf(:,4),Q(:,11),MB,1_intkind1,wf(:,22))
  call vert_QA_V(wf(:,-2),wf(:,6),wf(:,23))
  call counter_A_Q(ctbb,wf(:,6),Q(:,24),wf(:,24))
  call prop_Q_A(wf(:,7),Q(:,7),MB,1_intkind1,wf(:,25))
...
subroutine diagrams(A)
  ...
  A(4) = cont_VV(wf(:,8),wf(:,9)) * den(7)
  A(5) = cont_VV(wf(:,8),wf(:,9)) * den(7)
  A(6) = cont_QA(wf(:,3),wf(:,10)) * den(3)
  A(7) = cont_QA(wf(:,7),wf(:,12)) * den(5)
  A(8) = cont_QA(wf(:,6),wf(:,13)) * den(5)
  A(9) = cont_QA(wf(:,4),wf(:,15)) * den(3)
  A(10) = cont_QA(wf(:,3),wf(:,17)) * den(3)
  A(11) = cont_QA(wf(:,6),wf(:,18)) * den(5)
  A(12) = cont_VV(wf(:,19),wf(:,20)) * den(10)
  A(13) = cont_QA(wf(:,21),wf(:,22)) * den(13)
  A(14) = cont_VV(wf(:,20),wf(:,23)) * den(15)
  A(15) = cont_QA(wf(:,24),wf(:,25)) * den(18)
...
subroutine colourvectors(A, M1, M2)
  ...
  M2(1) = (A(4)*f(1))/2._/**/REALKIND+((-A(12)-A(13)-A(14)-A(15))*f(3))/2._/**/REALKIND+((A(6)+A(8))*f(4))/2._/**/REALKIND+((A(10) &
       +A(11))*f(5))/2._/**/REALKIND+((A(7)+A(9))*f(6))/2._/**/REALKIND+(A(5)*f(12))/2._/**/REALKIND+(A(3)*f(14))/2._/**/REALKIND
  M2(2) = -(A(4)*f(1))/6._/**/REALKIND+((A(12)+A(13)+A(14)+A(15))*f(3))/6._/**/REALKIND+((-A(6)-A(8))*f(4))/6._/**/REALKIND+(( &
       -A(10)-A(11))*f(5))/6._/**/REALKIND+((-A(7)-A(9))*f(6))/6._/**/REALKIND-(A(5)*f(12))/6._/**/REALKIND &
       -(A(3)*f(14))/6._/**/REALKIND

end subroutine colourvectors

end module ol_loop_pphbb_ddxbbxh_1_/**/REALKIND
