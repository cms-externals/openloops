...
module ol_colourmatrix_pphbb_cccxcxh_1_/**/REALKIND
...
end module ol_colourmatrix_pphbb_cccxcxh_1_/**/REALKIND



module ol_forced_parameters_pphbb_cccxcxh_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_pphbb_cccxcxh_1_/**/REALKIND

module ol_loop_pphbb_cccxcxh_1_/**/REALKIND
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f( 7) = (CI*countertermnorm*ctScc*eQED*gQCD**4*YC)/(2._/**/REALKIND*MW*sw)
    f( 8) = (CI*eQED*gQCD**4*integralnorm*SwB*YC)/(2._/**/REALKIND*MW*sw)
    f( 9) = (eQED*gQCD**4*integralnorm*SwB*YC)/(MW*sw*4._/**/REALKIND)
    f(10) = (eQED*gQCD**4*integralnorm*SwB*YC)/(MW*sw*2._/**/REALKIND)
    f(11) = (eQED*gQCD**4*integralnorm*SwF*YC)/(MW*sw*2._/**/REALKIND)
    f(12) = (eQED*gQCD**4*integralnorm*SwF*YC)/(MW*sw)
    f(13) = (CI*countertermnorm*ctHGG*eQED*gQCD**4*MT*YT)/MQ2sum
    f(14) = (eQED*gQCD**4*integralnorm*SwF*YT)/(MW*sw*2._/**/REALKIND)

  c = [ 3*f(2), 9*f(2), 9*CI*f(8), 27*CI*f(8), 18*f(9), 54*f(9), f(10), 3*f(10), 6*f(10), 8*f(10), 10*f(10), 18*f(10), 21*f(10) &
    , 24*f(10), 54*f(10), 3*f(11), 9*f(11), 3*f(12), 9*f(12), 3*f(14), 9*f(14) ]
  c = (1._/**/REALKIND / 36) * c
...
! **********************************************************************
  ...
  if (present(POLSEL)) then
    ...
    call pol_wf_Q(P(:,2), rMC, H(2), wf(:,-1), POLSEL(2))
    call pol_wf_A(P(:,3), rMC, H(3), wf(:,-2), POLSEL(3))
    call pol_wf_A(P(:,4), rMC, H(4), wf(:,-3), POLSEL(4))
    call pol_wf_S(P(:,5), rMH, H(5), wf(:,-4), POLSEL(5))

  else
    call pol_wf_Q(P(:,1), rMC, H(1), wf(:,0), 0)
    call pol_wf_Q(P(:,2), rMC, H(2), wf(:,-1), 0)
    call pol_wf_A(P(:,3), rMC, H(3), wf(:,-2), 0)
    call pol_wf_A(P(:,4), rMC, H(4), wf(:,-3), 0)
  ...
  call prop_A_Q(wf(:,15),Q(:,20),MC,1_intkind1,wf(:,16))
  ...
  call prop_A_Q(wf(:,23),Q(:,24),MC,1_intkind1,wf(:,24))
  ...
  call prop_Q_A(wf(:,40),Q(:,17),MC,1_intkind1,wf(:,41))
  ...
  call counter_Q_A(ctcc,wf(:,3),Q(:,18),wf(:,53))
  call prop_A_Q(wf(:,4),Q(:,13),MC,1_intkind1,wf(:,54))
  call vert_QA_V(wf(:,-1),wf(:,6),wf(:,55))
  call counter_A_Q(ctcc,wf(:,6),Q(:,24),wf(:,56))
  call prop_Q_A(wf(:,7),Q(:,7),MC,1_intkind1,wf(:,57))
  call vert_AV_Q(wf(:,-3),wf(:,9),wf(:,58))
  call counter_Q_A(ctcc,wf(:,10),Q(:,17),wf(:,59))
  call prop_A_Q(wf(:,58),Q(:,14),MC,1_intkind1,wf(:,60))
  call counter_V_V(ctGG,wf(:,9),Q(:,6),wf(:,61))
  call vert_QA_V(wf(:,0),wf(:,6),wf(:,62))
  call prop_Q_A(wf(:,12),Q(:,7),MC,1_intkind1,wf(:,63))
  call vert_QA_V(wf(:,3),wf(:,-2),wf(:,64))
  call counter_V_V(ctGG,wf(:,13),Q(:,9),wf(:,65))
  call prop_A_Q(wf(:,14),Q(:,13),MC,1_intkind1,wf(:,66))
  call vert_QA_V(wf(:,-1),wf(:,16),wf(:,67))
  call counter_A_Q(ctcc,wf(:,16),Q(:,20),wf(:,68))
  call prop_Q_A(wf(:,17),Q(:,11),MC,1_intkind1,wf(:,69))
  call vert_AV_Q(wf(:,-2),wf(:,18),wf(:,70))
  call prop_A_Q(wf(:,70),Q(:,14),MC,1_intkind1,wf(:,71))
  call counter_V_V(ctGG,wf(:,18),Q(:,10),wf(:,72))
  call vert_QA_V(wf(:,0),wf(:,16),wf(:,73))
  call prop_Q_A(wf(:,20),Q(:,11),MC,1_intkind1,wf(:,74))
...
subroutine colourvectors(A, M1, M2)
  ...
  M1(1) = ((-A(3)-A(4)-A(5)-A(6))*f(4))/6._/**/REALKIND+((-A(1)-A(2)-A(7)-A(8))*f(4))/2._/**/REALKIND
  M1(2) = ((A(3)+A(4)+A(5)+A(6))*f(4))/2._/**/REALKIND+((A(1)+A(2)+A(7)+A(8))*f(4))/6._/**/REALKIND

  M2(1) = -(A(10)*f(1))/2._/**/REALKIND-(A(15)*f(1))/6._/**/REALKIND-(A(11)*f(3))/2._/**/REALKIND-(A(16)*f(3))/6._/**/REALKIND &
       +((A(43)+A(44)+A(45)+A(46)+A(47)+A(48)+A(49)+A(50))*f(5))/6._/**/REALKIND+((A(39)+A(40)+A(41)+A(42)+A(51)+A(52)+A(53) &
       +A(54))*f(5))/2._/**/REALKIND+((-A(17)-A(19)-A(25)-A(29)-A(30)-A(31)-A(35)-A(36))*f(6))/6._/**/REALKIND+((-A(12)-A(21) &
       -A(23)-A(27)-A(28)-A(33)-A(37)-A(38))*f(6))/2._/**/REALKIND+((-A(18)-A(20)-A(26)-A(32))*f(7))/6._/**/REALKIND+((-A(13) &
       -A(22)-A(24)-A(34))*f(7))/2._/**/REALKIND-(A(9)*f(13))/2._/**/REALKIND-(A(14)*f(13))/6._/**/REALKIND
  M2(2) = (A(10)*f(1))/6._/**/REALKIND+(A(15)*f(1))/2._/**/REALKIND+(A(11)*f(3))/6._/**/REALKIND+(A(16)*f(3))/2._/**/REALKIND+(( &
       -A(43)-A(44)-A(45)-A(46)-A(47)-A(48)-A(49)-A(50))*f(5))/2._/**/REALKIND+((-A(39)-A(40)-A(41)-A(42)-A(51)-A(52)-A(53) &
       -A(54))*f(5))/6._/**/REALKIND+((A(17)+A(19)+A(25)+A(29)+A(30)+A(31)+A(35)+A(36))*f(6))/2._/**/REALKIND+((A(12)+A(21)+A(23) &
       +A(27)+A(28)+A(33)+A(37)+A(38))*f(6))/6._/**/REALKIND+((A(18)+A(20)+A(26)+A(32))*f(7))/2._/**/REALKIND+((A(13)+A(22)+A(24) &
       +A(34))*f(7))/6._/**/REALKIND+(A(9)*f(13))/6._/**/REALKIND+(A(14)*f(13))/2._/**/REALKIND

end subroutine colourvectors

end module ol_loop_pphbb_cccxcxh_1_/**/REALKIND
