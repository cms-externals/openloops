...
module ol_colourmatrix_ppllj2_eexggg_1_/**/REALKIND
...
end module ol_colourmatrix_ppllj2_eexggg_1_/**/REALKIND



module ol_forced_parameters_ppllj2_eexggg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllj2_eexggg_1_/**/REALKIND

module ol_loop_ppllj2_eexggg_1_/**/REALKIND
  ...
  ! tree wavefunctions
  complex(REALKIND), save :: wf(4,-5+1:13)
  ! denominators
...
! **********************************************************************
  ...
  ! factors of the diagrams
    ...
    f( 2) = CI*countertermnorm*ctZGG*eQED**2*gQCD**3
    f( 3) = (CI*eQED**2*gQCD**3*integralnorm*SwF)/3._/**/REALKIND
    f( 4) = (2*CI*eQED**2*gQCD**3*integralnorm*SwF)/3._/**/REALKIND
    f( 5) = CI*eQED**2*gQCD**3*integralnorm*SwF
    f( 6) = (4*CI*eQED**2*gQCD**3*integralnorm*SwF)/3._/**/REALKIND
    f( 7) = 2*CI*eQED**2*gQCD**3*integralnorm*SwF
    ...
    f(12) = 2*eQED**2*gQCD**3*integralnorm*SwF
...
! **********************************************************************
  ...
  call counter_GGG_V(ctZGGG,wf(:,-2),wf(:,-3),wf(:,-4),wf(:,5))
  call prop_W_W(wf(:,4),Q(:,3),MZ,1_intkind1,wf(:,6))
  call counter_GGG_V(ctZGGG,wf(:,-2),wf(:,-4),wf(:,-3),wf(:,7))
  call vert_UV_W(wf(:,-2),Q(:,4),wf(:,-3),Q(:,8),wf(:,8))
  call counter_VG_G(wf(:,6),wf(:,-4),Q(:,16),wf(:,9),Q(:,19))
  call vert_UV_W(wf(:,-2),Q(:,4),wf(:,-4),Q(:,16),wf(:,10))
  call counter_VG_G(wf(:,6),wf(:,-3),Q(:,8),wf(:,11),Q(:,11))
  call vert_UV_W(wf(:,-3),Q(:,8),wf(:,-4),Q(:,16),wf(:,12))
  call counter_VG_G(wf(:,6),wf(:,-2),Q(:,4),wf(:,13),Q(:,7))
...
subroutine diagrams(A)
  ...
  A(3) = cont_VV(wf(:,5),wf(:,6)) * den(2)
  A(4) = cont_VV(wf(:,6),wf(:,7)) * den(2)
  A(5) = cont_VV(wf(:,8),wf(:,9)) * den(4)
  A(6) = cont_VV(wf(:,10),wf(:,11)) * den(6)
  A(7) = cont_VV(wf(:,12),wf(:,13)) * den(8)
...
subroutine colourvectors(A, M1, M2)
  ...
  M2(1) = (A(1)+A(3))*f(1)+2*CI*(A(5)-A(6)+A(7))*f(2)
  M2(2) = (A(2)+A(4))*f(1)+2*CI*(-A(5)+A(6)-A(7))*f(2)

end subroutine colourvectors

end module ol_loop_ppllj2_eexggg_1_/**/REALKIND
