...
module ol_colourmatrix_ppvvvj_ddxbbxawwx_1_/**/REALKIND
...
end module ol_colourmatrix_ppvvvj_ddxbbxawwx_1_/**/REALKIND



module ol_forced_parameters_ppvvvj_ddxbbxawwx_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppvvvj_ddxbbxawwx_1_/**/REALKIND

module ol_tree_ppvvvj_ddxbbxawwx_1_/**/REALKIND
...
subroutine born_denominators_init()
  ...
  den(163) = den(78)*den(120)
  den(164) = den(15)*den(163)
  den(165) = den(78)*den(122)
  den(166) = den(121)*den(165)
  den(167) = den(15)*den(127)
  den(168) = den(15)*den(161)
  den(169) = den(85)*den(140)
  den(170) = den(131)*den(169)
  den(171) = den(85)*den(147)
  den(172) = den(146)*den(171)
  den(173) = den(15)*den(93)
  den(174) = den(15)*den(95)
  den(175) = den(93)*den(131)
  den(176) = den(95)*den(131)
  den(177) = den(15)*den(171)
  den(178) = den(131)*den(159)
  den(179) = den(131)*den(144)
  den(180) = den(15)*den(105)
  den(181) = den(15)*den(107)
  den(182) = den(105)*den(131)
  den(183) = den(107)*den(131)
  den(184) = den(131)*den(165)
  ...
  den(215) = den(127)*den(213)
  den(216) = den(144)*den(213)
  ...
  den(223) = den(127)*den(221)
  den(224) = den(144)*den(221)
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppvvvj_ddxbbxawwx_1")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppvvvj_ddxbbxawwx_1_/**/DREALKIND, only: ntry, nhel, Hel, M1helarr
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for down anti-down bottom anti-bottom gamma W- W+ -> 0
...
! **********************************************************************
  ...
  use ol_external_ppvvvj_ddxbbxawwx_1, only: external_perm_ppvvvj_ddxbbxawwx_1, &
    & external_perm_inv_ppvvvj_ddxbbxawwx_1, extcomb_perm_ppvvvj_ddxbbxawwx_1, &
    & average_factor_ppvvvj_ddxbbxawwx_1
  use ol_external_ppvvvj_ddxbbxawwx_1, only: H, hel_not_initialised, hel_init
  use ol_colourmatrix_ppvvvj_ddxbbxawwx_1_/**/REALKIND, only: colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppvvvj_ddxbbxawwx_1_/**/REALKIND, only: check_forced_parameters
  use ol_heltables_ppvvvj_ddxbbxawwx_1
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_ppvvvj_ddxbbxawwx_1,7)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppvvvj_ddxbbxawwx_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppvvvj_ddxbbxawwx_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_ppvvvj_ddxbbxawwx_1(extcombs(k))
  end do
  ...
  call vert_AW_Q(ntry, ex2, ex6, wf6(:,9), n3(:,68), t3x6(:,:,5))
  call prop_Q_A(ntry, wf4(:,7), Q(:,17), ZERO, 0_intkind1, wf4(:,8), n2(32))
  call prop_A_Q(ntry, wf6(:,9), Q(:,34), ZERO, 0_intkind1, wf6(:,10), n2(33))
  call vert_WQ_A(ntry, ex7, wf4(:,8), wf12(:,13), n3(:,69), t3x12(:,:,7))
  call vert_AV_Q(ntry, wf6(:,10), wf4(:,2), wf24(:,5), n3(:,70), t3x24(:,:,5))
  call prop_Q_A(ntry, wf12(:,13), Q(:,81), ZERO, 0_intkind1, wf12(:,14), n2(34))
  call vert_AW_Q(ntry, wf6(:,10), ex7, wf18(:,25), n3(:,71), t3x18(:,:,13))
  ...
  call prop_Q_A(ntry, wf12(:,17), Q(:,81), ZERO, 0_intkind1, wf12(:,18), n2(45))
  call vert_AV_Q(ntry, wf6(:,10), ex5, wf12(:,19), n3(:,90), t3x12(:,:,10))
  call prop_A_Q(ntry, wf12(:,19), Q(:,50), ZERO, 0_intkind1, wf12(:,20), n2(46))
  call vert_AW_Q(ntry, wf6(:,10), wf6(:,6), wf36(:,29), n3(:,91), t3x36(:,:,25))
  call vert_WQ_A(ntry, wf6(:,6), ex1, wf12(:,21), n3(:,92), t3x12(:,:,11))
  call prop_Q_A(ntry, wf12(:,21), Q(:,81), ZERO, 0_intkind1, wf12(:,22), n2(47))
  call vert_VQ_A(ntry, ex5, wf8(:,5), wf16(:,13), n3(:,93), t3x16(:,:,13))
  call vert_WQ_A(ntry, wf6(:,8), wf6(:,12), wf36(:,30), n3(:,94), t3x36(:,:,26))
  call vert_AW_Q(ntry, ex2, wf6(:,8), wf12(:,23), n3(:,95), t3x12(:,:,12))
  call prop_A_Q(ntry, wf12(:,23), Q(:,50), ZERO, 0_intkind1, wf12(:,24), n2(48))
  call vert_QA_V(ntry, wf8(:,5), ex2, wf16(:,14), n3(:,96), t3x16(:,:,14))
  call vert_QA_Z(gZd,ntry, wf8(:,5), ex2, wf16(:,15), n3(:,97), t3x16(:,:,15))
  call vert_QA_V(ntry, ex1, wf8(:,8), wf16(:,16), n3(:,98), t3x16(:,:,16))
  call vert_QA_Z(gZd,ntry, ex1, wf8(:,8), wf16(:,17), n3(:,99), t3x16(:,:,17))
  call vert_AV_Q(ntry, wf8(:,8), ex5, wf16(:,18), n3(:,100), t3x16(:,:,18))
  call vert_WQ_A(ntry, ex6, wf12(:,22), wf36(:,31), n3(:,101), t3x36(:,:,27))
  ...
  call vert_VQ_A(ntry, wf8(:,11), ex1, wf16(:,23), n3(:,113), t3x16(:,:,23))
  call vert_AV_Q(ntry, ex2, wf8(:,11), wf16(:,24), n3(:,114), t3x16(:,:,24))
  ...
  call vert_VQ_A(ntry, wf8(:,12), ex1, wf16(:,25), n3(:,120), t3x16(:,:,25))
  call vert_AV_Q(ntry, ex2, wf8(:,12), wf16(:,26), n3(:,121), t3x16(:,:,26))
  ...
  M2add = M2 / average_factor_ppvvvj_ddxbbxawwx_1

  do k = 0, 30-1
    M2(k) = M2add(extcomb_perm_ppvvvj_ddxbbxawwx_1(k))
  end do
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call cont_QA(nsync, wf8(:,5), wf36(:,29), A(:,73), n3(:,194), t3x288(:,:,73), nhel, den(164))
    call cont_QA(nsync, wf24(:,5), wf12(:,22), A(:,74), n3(:,195), t3x288(:,:,74), nhel, den(166))
    call cont_QA(nsync, wf18(:,26), wf16(:,13), A(:,75), n3(:,196), t3x288(:,:,75), nhel, den(167))
    call cont_QA(nsync, wf24(:,8), wf12(:,20), A(:,76), n3(:,197), t3x288(:,:,76), nhel, den(168))
    call cont_QA(nsync, wf8(:,8), wf36(:,30), A(:,77), n3(:,198), t3x288(:,:,77), nhel, den(170))
    call cont_QA(nsync, wf24(:,7), wf12(:,24), A(:,78), n3(:,199), t3x288(:,:,78), nhel, den(172))
    ...
    call cont_QA(nsync, wf24(:,8), wf12(:,24), A(:,83), n3(:,204), t3x288(:,:,83), nhel, den(177))
    call cont_QA(nsync, wf24(:,6), wf12(:,18), A(:,84), n3(:,205), t3x288(:,:,84), nhel, den(178))
    call cont_QA(nsync, wf18(:,32), wf16(:,18), A(:,85), n3(:,206), t3x288(:,:,85), nhel, den(179))
    ...
    call cont_QA(nsync, wf8(:,8), wf36(:,31), A(:,90), n3(:,211), t3x288(:,:,90), nhel, den(184))
    call cont_QA(nsync, wf18(:,28), wf16(:,13), A(:,91), n3(:,212), t3x288(:,:,91), nhel, den(185))
    call cont_QA(nsync, wf18(:,30), wf16(:,13), A(:,92), n3(:,213), t3x288(:,:,92), nhel, den(186))
    ...
    call cont_QA(nsync, wf18(:,26), wf16(:,23), A(:,114), n3(:,235), t3x288(:,:,114), nhel, den(215))
    call cont_QA(nsync, wf18(:,32), wf16(:,24), A(:,115), n3(:,236), t3x288(:,:,115), nhel, den(216))
    ...
    call cont_QA(nsync, wf18(:,26), wf16(:,25), A(:,121), n3(:,242), t3x288(:,:,121), nhel, den(223))
    call cont_QA(nsync, wf18(:,32), wf16(:,26), A(:,122), n3(:,243), t3x288(:,:,122), nhel, den(224))
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = ((A(j,12)%j+A(j,15)%j+A(j,21)%j+A(j,24)%j+A(j,52)%j+A(j,55)%j+A(j,59)%j+A(j,61)%j+A(j,66)%j+A(j,68)%j+A(j,91)%j &
       +A(j,93)%j+A(j,99)%j+A(j,102)%j+A(j,108)%j+A(j,111)%j+A(j,116)%j+A(j,118)%j+A(j,123)%j+A(j,125)%j)*f(1))/2._/**/REALKIND &
       +((A(j,1)%j+A(j,3)%j+A(j,5)%j+A(j,7)%j-A(j,36)%j-A(j,40)%j-A(j,45)%j-A(j,48)%j-A(j,79)%j-A(j,81)%j-A(j,86)%j &
       -A(j,88)%j)*f(2))/2._/**/REALKIND+((-A(j,9)%j-A(j,10)%j-A(j,17)%j-A(j,18)%j-A(j,19)%j-A(j,26)%j-A(j,31)%j-A(j,43)%j &
       -A(j,57)%j-A(j,58)%j-A(j,63)%j-A(j,64)%j-A(j,65)%j-A(j,70)%j-A(j,75)%j-A(j,85)%j-A(j,95)%j-A(j,96)%j-A(j,97)%j-A(j,104)%j &
       -A(j,105)%j-A(j,106)%j-A(j,113)%j-A(j,114)%j-A(j,115)%j-A(j,120)%j-A(j,121)%j-A(j,122)%j)*f(3))/2._/**/REALKIND+((A(j,27)%j &
       +A(j,28)%j+A(j,32)%j+A(j,42)%j+A(j,71)%j+A(j,72)%j+A(j,76)%j+A(j,84)%j)*f(4))/2._/**/REALKIND+((A(j,29)%j+A(j,30)%j &
       +A(j,33)%j+A(j,34)%j+A(j,38)%j+A(j,50)%j+A(j,73)%j+A(j,74)%j+A(j,77)%j+A(j,78)%j+A(j,83)%j+A(j,90)%j)*f(5))/2._/**/REALKIND &
       +((A(j,11)%j+A(j,14)%j+A(j,20)%j+A(j,23)%j+A(j,51)%j+A(j,54)%j+A(j,98)%j+A(j,101)%j+A(j,107)%j &
       +A(j,110)%j)*f(6))/2._/**/REALKIND+((-A(j,35)%j-A(j,39)%j-A(j,44)%j-A(j,47)%j)*f(7))/2._/**/REALKIND+((-A(j,13)%j-A(j,16)%j &
       -A(j,22)%j-A(j,25)%j-A(j,53)%j-A(j,56)%j-A(j,60)%j-A(j,62)%j-A(j,67)%j-A(j,69)%j-A(j,92)%j-A(j,94)%j-A(j,100)%j-A(j,103)%j &
       -A(j,109)%j-A(j,112)%j-A(j,117)%j-A(j,119)%j-A(j,124)%j-A(j,126)%j)*f(8))/2._/**/REALKIND+((-A(j,2)%j-A(j,4)%j-A(j,6)%j &
       -A(j,8)%j+A(j,37)%j+A(j,41)%j+A(j,46)%j+A(j,49)%j+A(j,80)%j+A(j,82)%j+A(j,87)%j+A(j,89)%j)*f(9))/2._/**/REALKIND
  M1(2) = ((-A(j,12)%j-A(j,15)%j-A(j,21)%j-A(j,24)%j-A(j,52)%j-A(j,55)%j-A(j,59)%j-A(j,61)%j-A(j,66)%j-A(j,68)%j-A(j,91)%j &
       -A(j,93)%j-A(j,99)%j-A(j,102)%j-A(j,108)%j-A(j,111)%j-A(j,116)%j-A(j,118)%j-A(j,123)%j-A(j,125)%j)*f(1))/6._/**/REALKIND+(( &
       -A(j,1)%j-A(j,3)%j-A(j,5)%j-A(j,7)%j+A(j,36)%j+A(j,40)%j+A(j,45)%j+A(j,48)%j+A(j,79)%j+A(j,81)%j+A(j,86)%j &
       +A(j,88)%j)*f(2))/6._/**/REALKIND+((A(j,9)%j+A(j,10)%j+A(j,17)%j+A(j,18)%j+A(j,19)%j+A(j,26)%j+A(j,31)%j+A(j,43)%j &
       +A(j,57)%j+A(j,58)%j+A(j,63)%j+A(j,64)%j+A(j,65)%j+A(j,70)%j+A(j,75)%j+A(j,85)%j+A(j,95)%j+A(j,96)%j+A(j,97)%j+A(j,104)%j &
       +A(j,105)%j+A(j,106)%j+A(j,113)%j+A(j,114)%j+A(j,115)%j+A(j,120)%j+A(j,121)%j+A(j,122)%j)*f(3))/6._/**/REALKIND+(( &
       -A(j,27)%j-A(j,28)%j-A(j,32)%j-A(j,42)%j-A(j,71)%j-A(j,72)%j-A(j,76)%j-A(j,84)%j)*f(4))/6._/**/REALKIND+((-A(j,29)%j &
       -A(j,30)%j-A(j,33)%j-A(j,34)%j-A(j,38)%j-A(j,50)%j-A(j,73)%j-A(j,74)%j-A(j,77)%j-A(j,78)%j-A(j,83)%j &
       -A(j,90)%j)*f(5))/6._/**/REALKIND+((-A(j,11)%j-A(j,14)%j-A(j,20)%j-A(j,23)%j-A(j,51)%j-A(j,54)%j-A(j,98)%j-A(j,101)%j &
       -A(j,107)%j-A(j,110)%j)*f(6))/6._/**/REALKIND+((A(j,35)%j+A(j,39)%j+A(j,44)%j+A(j,47)%j)*f(7))/6._/**/REALKIND+((A(j,13)%j &
       +A(j,16)%j+A(j,22)%j+A(j,25)%j+A(j,53)%j+A(j,56)%j+A(j,60)%j+A(j,62)%j+A(j,67)%j+A(j,69)%j+A(j,92)%j+A(j,94)%j+A(j,100)%j &
       +A(j,103)%j+A(j,109)%j+A(j,112)%j+A(j,117)%j+A(j,119)%j+A(j,124)%j+A(j,126)%j)*f(8))/6._/**/REALKIND+((A(j,2)%j+A(j,4)%j &
       +A(j,6)%j+A(j,8)%j-A(j,37)%j-A(j,41)%j-A(j,46)%j-A(j,49)%j-A(j,80)%j-A(j,82)%j-A(j,87)%j-A(j,89)%j)*f(9))/6._/**/REALKIND
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppvvvj_ddxbbxawwx_1_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppvvvj_ddxbbxawwx_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppvvvj_ddxbbxawwx_1")
#else
subroutine amp2tree(P, M2)
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppvvvj_ddxbbxawwx_1")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppvvvj_ddxbbxawwx_1")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppvvvj_ddxbbxawwx_1")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppvvvj_ddxbbxawwx_1")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppvvvj_ddxbbxawwx_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppvvvj_ddxbbxawwx_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppvvvj_ddxbbxawwx_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppvvvj_ddxbbxawwx_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppvvvj_ddxbbxawwx_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2tree_legacy(p, m2) &
    & bind(c,name="amp2tree_ppvvvj_ddxbbxawwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccone_legacy(p, m2, i, j) &
    & bind(c,name="amp2ccone_ppvvvj_ddxbbxawwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccall_legacy(p, m2) &
    & bind(c,name="amp2ccall_ppvvvj_ddxbbxawwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(0:30-1)
...
subroutine amp2hcone_legacy(p, m2, i, j, mom) &
    & bind(c,name="amp2hcone_ppvvvj_ddxbbxawwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_legacy(p, m2, i, mom) &
    & bind(c,name="amp2hcall_ppvvvj_ddxbbxawwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(7)
...
end module ol_tree_ppvvvj_ddxbbxawwx_1_/**/REALKIND
