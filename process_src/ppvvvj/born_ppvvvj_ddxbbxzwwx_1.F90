...
module ol_colourmatrix_ppvvvj_ddxbbxzwwx_1_/**/REALKIND
...
end module ol_colourmatrix_ppvvvj_ddxbbxzwwx_1_/**/REALKIND



module ol_forced_parameters_ppvvvj_ddxbbxzwwx_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppvvvj_ddxbbxzwwx_1_/**/REALKIND

module ol_tree_ppvvvj_ddxbbxzwwx_1_/**/REALKIND
...
subroutine born_denominators_init()
  ...
  den(169) = den(78)*den(126)
  den(170) = den(15)*den(169)
  den(171) = den(78)*den(128)
  den(172) = den(127)*den(171)
  den(173) = den(15)*den(133)
  den(174) = den(15)*den(167)
  den(175) = den(85)*den(146)
  den(176) = den(137)*den(175)
  den(177) = den(85)*den(153)
  den(178) = den(152)*den(177)
  den(179) = den(15)*den(93)
  den(180) = den(15)*den(95)
  den(181) = den(93)*den(137)
  den(182) = den(95)*den(137)
  den(183) = den(15)*den(177)
  den(184) = den(137)*den(165)
  den(185) = den(137)*den(150)
  den(186) = den(15)*den(105)
  den(187) = den(15)*den(107)
  den(188) = den(105)*den(137)
  den(189) = den(107)*den(137)
  den(190) = den(137)*den(171)
  ...
  den(223) = den(133)*den(221)
  den(224) = den(150)*den(221)
  ...
  den(231) = den(133)*den(229)
  den(232) = den(150)*den(229)
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppvvvj_ddxbbxzwwx_1")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppvvvj_ddxbbxzwwx_1_/**/DREALKIND, only: ntry, nhel, Hel, M1helarr
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for down anti-down bottom anti-bottom Z W- W+ -> 0
...
! **********************************************************************
  ...
  use ol_external_ppvvvj_ddxbbxzwwx_1, only: external_perm_ppvvvj_ddxbbxzwwx_1, &
    & external_perm_inv_ppvvvj_ddxbbxzwwx_1, extcomb_perm_ppvvvj_ddxbbxzwwx_1, &
    & average_factor_ppvvvj_ddxbbxzwwx_1
  use ol_external_ppvvvj_ddxbbxzwwx_1, only: H, hel_not_initialised, hel_init
  use ol_colourmatrix_ppvvvj_ddxbbxzwwx_1_/**/REALKIND, only: colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppvvvj_ddxbbxzwwx_1_/**/REALKIND, only: check_forced_parameters
  use ol_heltables_ppvvvj_ddxbbxzwwx_1
  ...
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_ppvvvj_ddxbbxzwwx_1,7)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppvvvj_ddxbbxzwwx_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppvvvj_ddxbbxzwwx_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = extcomb_perm_ppvvvj_ddxbbxzwwx_1(extcombs(k))
  end do
  ...
  call vert_ZQ_A(gZd,ntry, ex5, ex1, wf6(:,9), n3(:,69), t3x6(:,:,5))
  call vert_AW_Q(ntry, ex2, ex6, wf6(:,10), n3(:,70), t3x6(:,:,6))
  call prop_Q_A(ntry, wf6(:,9), Q(:,17), ZERO, 0_intkind1, wf6(:,11), n2(33))
  call prop_A_Q(ntry, wf6(:,10), Q(:,34), ZERO, 0_intkind1, wf6(:,12), n2(34))
  call vert_WQ_A(ntry, ex7, wf6(:,11), wf18(:,29), n3(:,71), t3x18(:,:,15))
  call vert_AV_Q(ntry, wf6(:,12), wf4(:,2), wf24(:,9), n3(:,72), t3x24(:,:,9))
  call prop_Q_A(ntry, wf18(:,29), Q(:,81), ZERO, 0_intkind1, wf18(:,30), n2(35))
  call vert_AW_Q(ntry, wf6(:,12), ex7, wf18(:,31), n3(:,73), t3x18(:,:,16))
  ...
  call vert_ZQ_A(gZd,ntry, wf9(:,3), wf6(:,11), wf54(:,26), n3(:,76), t3x54(:,:,22))
  call vert_AV_Q(ntry, ex2, wf9(:,2), wf18(:,33), n3(:,77), t3x18(:,:,17))
  call prop_A_Q(ntry, wf18(:,33), Q(:,98), ZERO, 0_intkind1, wf18(:,34), n2(38))
  call vert_AZ_Q(gZd,ntry, ex2, wf9(:,3), wf18(:,35), n3(:,78), t3x18(:,:,18))
  call prop_A_Q(ntry, wf18(:,35), Q(:,98), ZERO, 0_intkind1, wf18(:,36), n2(39))
  call vert_AW_Q(ntry, wf8(:,8), ex6, wf24(:,11), n3(:,79), t3x24(:,:,11))
  call vert_WQ_A(ntry, ex7, ex1, wf6(:,13), n3(:,80), t3x6(:,:,7))
  call vert_AZ_Q(gZd,ntry, ex2, ex5, wf6(:,14), n3(:,81), t3x6(:,:,8))
  call prop_Q_A(ntry, wf6(:,13), Q(:,65), ZERO, 0_intkind1, wf6(:,15), n2(40))
  call prop_A_Q(ntry, wf6(:,14), Q(:,18), ZERO, 0_intkind1, wf6(:,16), n2(41))
  call vert_WQ_A(ntry, ex6, wf6(:,15), wf18(:,37), n3(:,82), t3x18(:,:,19))
  call vert_AV_Q(ntry, wf6(:,16), wf4(:,2), wf24(:,12), n3(:,83), t3x24(:,:,12))
  call prop_Q_A(ntry, wf18(:,37), Q(:,97), ZERO, 0_intkind1, wf18(:,38), n2(42))
  call vert_AW_Q(ntry, wf6(:,16), ex6, wf18(:,39), n3(:,84), t3x18(:,:,20))
  call vert_VQ_A(ntry, wf4(:,2), wf6(:,15), wf24(:,13), n3(:,85), t3x24(:,:,13))
  call prop_A_Q(ntry, wf18(:,39), Q(:,50), ZERO, 0_intkind1, wf18(:,40), n2(43))
  call vert_AV_Q(ntry, wf6(:,16), wf9(:,2), wf54(:,27), n3(:,86), t3x54(:,:,23))
  call vert_AZ_Q(gZd,ntry, wf6(:,16), wf9(:,3), wf54(:,28), n3(:,87), t3x54(:,:,24))
  call vert_VQ_A(ntry, wf9(:,2), ex1, wf18(:,41), n3(:,88), t3x18(:,:,21))
  call prop_Q_A(ntry, wf18(:,41), Q(:,97), ZERO, 0_intkind1, wf18(:,42), n2(44))
  call vert_ZQ_A(gZd,ntry, wf9(:,3), ex1, wf18(:,43), n3(:,89), t3x18(:,:,22))
  call prop_Q_A(ntry, wf18(:,43), Q(:,97), ZERO, 0_intkind1, wf18(:,44), n2(45))
  call vert_WQ_A(ntry, ex7, wf8(:,5), wf24(:,14), n3(:,90), t3x24(:,:,14))
  call vert_ZQ_A(gZu,ntry, ex5, wf6(:,15), wf18(:,45), n3(:,91), t3x18(:,:,23))
  call prop_Q_A(ntry, wf18(:,45), Q(:,81), ZERO, 0_intkind1, wf18(:,46), n2(46))
  call vert_AZ_Q(gZu,ntry, wf6(:,12), ex5, wf18(:,47), n3(:,92), t3x18(:,:,24))
  call prop_A_Q(ntry, wf18(:,47), Q(:,50), ZERO, 0_intkind1, wf18(:,48), n2(47))
  call vert_AW_Q(ntry, wf6(:,12), wf9(:,5), wf54(:,29), n3(:,93), t3x54(:,:,25))
  call vert_WQ_A(ntry, wf9(:,5), ex1, wf18(:,49), n3(:,94), t3x18(:,:,25))
  call prop_Q_A(ntry, wf18(:,49), Q(:,81), ZERO, 0_intkind1, wf18(:,50), n2(48))
  call vert_ZQ_A(gZd,ntry, ex5, wf8(:,5), wf24(:,15), n3(:,95), t3x24(:,:,15))
  call vert_WQ_A(ntry, wf9(:,7), wf6(:,15), wf54(:,30), n3(:,96), t3x54(:,:,26))
  call vert_AW_Q(ntry, ex2, wf9(:,7), wf18(:,51), n3(:,97), t3x18(:,:,26))
  call prop_A_Q(ntry, wf18(:,51), Q(:,50), ZERO, 0_intkind1, wf18(:,52), n2(49))
  call vert_QA_V(ntry, wf8(:,5), ex2, wf16(:,7), n3(:,98), t3x16(:,:,7))
  call vert_QA_Z(gZd,ntry, wf8(:,5), ex2, wf16(:,8), n3(:,99), t3x16(:,:,8))
  call vert_QA_V(ntry, ex1, wf8(:,8), wf16(:,9), n3(:,100), t3x16(:,:,9))
  call vert_QA_Z(gZd,ntry, ex1, wf8(:,8), wf16(:,10), n3(:,101), t3x16(:,:,10))
  call vert_AZ_Q(gZd,ntry, wf8(:,8), ex5, wf24(:,16), n3(:,102), t3x24(:,:,16))
  call vert_WQ_A(ntry, ex6, wf18(:,50), wf54(:,31), n3(:,103), t3x54(:,:,27))
  call vert_ZQ_A(gZd,ntry, ex5, wf18(:,42), wf54(:,32), n3(:,104), t3x54(:,:,28))
  call vert_ZQ_A(gZd,ntry, ex5, wf18(:,44), wf54(:,33), n3(:,105), t3x54(:,:,29))
  ...
  call vert_VQ_A(ntry, wf12(:,3), ex1, wf24(:,21), n3(:,115), t3x24(:,:,21))
  call vert_AV_Q(ntry, ex2, wf12(:,3), wf24(:,22), n3(:,116), t3x24(:,:,22))
  ...
  call vert_VQ_A(ntry, wf12(:,4), ex1, wf24(:,23), n3(:,122), t3x24(:,:,23))
  call vert_AV_Q(ntry, ex2, wf12(:,4), wf24(:,24), n3(:,123), t3x24(:,:,24))
  ...
  M2add = M2 / average_factor_ppvvvj_ddxbbxzwwx_1

  do k = 0, 30-1
    M2(k) = M2add(extcomb_perm_ppvvvj_ddxbbxzwwx_1(k))
  end do
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call cont_QA(nsync, wf8(:,5), wf54(:,29), A(:,77), n3(:,200), t3x432(:,:,77), nhel, den(170))
    call cont_QA(nsync, wf24(:,9), wf18(:,50), A(:,78), n3(:,201), t3x432(:,:,78), nhel, den(172))
    call cont_QA(nsync, wf18(:,32), wf24(:,15), A(:,79), n3(:,202), t3x432(:,:,79), nhel, den(173))
    call cont_QA(nsync, wf24(:,14), wf18(:,48), A(:,80), n3(:,203), t3x432(:,:,80), nhel, den(174))
    call cont_QA(nsync, wf8(:,8), wf54(:,30), A(:,81), n3(:,204), t3x432(:,:,81), nhel, den(176))
    call cont_QA(nsync, wf24(:,13), wf18(:,52), A(:,82), n3(:,205), t3x432(:,:,82), nhel, den(178))
    ...
    call cont_QA(nsync, wf24(:,14), wf18(:,52), A(:,87), n3(:,210), t3x432(:,:,87), nhel, den(183))
    call cont_QA(nsync, wf24(:,11), wf18(:,46), A(:,88), n3(:,211), t3x432(:,:,88), nhel, den(184))
    call cont_QA(nsync, wf18(:,38), wf24(:,16), A(:,89), n3(:,212), t3x432(:,:,89), nhel, den(185))
    ...
    call cont_QA(nsync, wf8(:,8), wf54(:,31), A(:,94), n3(:,217), t3x432(:,:,94), nhel, den(190))
    ...
    call cont_QA(nsync, wf18(:,34), wf24(:,15), A(:,97), n3(:,220), t3x432(:,:,97), nhel, den(193))
    call cont_QA(nsync, wf18(:,36), wf24(:,15), A(:,98), n3(:,221), t3x432(:,:,98), nhel, den(194))
    ...
    call cont_QA(nsync, wf18(:,32), wf24(:,21), A(:,120), n3(:,243), t3x432(:,:,120), nhel, den(223))
    call cont_QA(nsync, wf18(:,38), wf24(:,22), A(:,121), n3(:,244), t3x432(:,:,121), nhel, den(224))
    ...
    call cont_QA(nsync, wf18(:,32), wf24(:,23), A(:,127), n3(:,250), t3x432(:,:,127), nhel, den(231))
    call cont_QA(nsync, wf18(:,38), wf24(:,24), A(:,128), n3(:,251), t3x432(:,:,128), nhel, den(232))
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = ((A(j,12)%j+A(j,15)%j+A(j,21)%j+A(j,24)%j+A(j,54)%j+A(j,59)%j+A(j,63)%j+A(j,65)%j+A(j,70)%j+A(j,72)%j+A(j,97)%j &
       +A(j,99)%j+A(j,105)%j+A(j,108)%j+A(j,114)%j+A(j,117)%j+A(j,122)%j+A(j,124)%j+A(j,129)%j+A(j,131)%j)*f(1))/2._/**/REALKIND &
       +((-A(j,29)%j-A(j,30)%j-A(j,33)%j-A(j,34)%j-A(j,38)%j-A(j,50)%j-A(j,77)%j-A(j,78)%j-A(j,81)%j-A(j,82)%j-A(j,87)%j &
       -A(j,94)%j)*f(2))/2._/**/REALKIND+((A(j,51)%j+A(j,56)%j)*f(3))/2._/**/REALKIND+((A(j,35)%j+A(j,39)%j+A(j,44)%j &
       +A(j,47)%j)*f(4))/2._/**/REALKIND+((-A(j,9)%j-A(j,10)%j-A(j,17)%j-A(j,18)%j-A(j,19)%j-A(j,26)%j-A(j,27)%j-A(j,28)%j &
       -A(j,31)%j-A(j,32)%j-A(j,42)%j-A(j,43)%j-A(j,61)%j-A(j,62)%j-A(j,67)%j-A(j,68)%j-A(j,69)%j-A(j,74)%j-A(j,75)%j-A(j,76)%j &
       -A(j,79)%j-A(j,80)%j-A(j,88)%j-A(j,89)%j-A(j,101)%j-A(j,102)%j-A(j,103)%j-A(j,110)%j-A(j,111)%j-A(j,112)%j-A(j,119)%j &
       -A(j,120)%j-A(j,121)%j-A(j,126)%j-A(j,127)%j-A(j,128)%j)*f(5))/2._/**/REALKIND+((A(j,2)%j+A(j,4)%j+A(j,6)%j+A(j,8)%j &
       -A(j,37)%j-A(j,41)%j-A(j,46)%j-A(j,49)%j-A(j,84)%j-A(j,86)%j-A(j,91)%j-A(j,93)%j)*f(6))/2._/**/REALKIND+((A(j,11)%j &
       +A(j,14)%j+A(j,20)%j+A(j,23)%j+A(j,53)%j+A(j,58)%j+A(j,104)%j+A(j,107)%j+A(j,113)%j+A(j,116)%j)*f(7))/2._/**/REALKIND &
       +((A(j,52)%j+A(j,57)%j+A(j,95)%j+A(j,96)%j)*f(8))/2._/**/REALKIND+((-A(j,1)%j-A(j,3)%j-A(j,5)%j-A(j,7)%j+A(j,36)%j &
       +A(j,40)%j+A(j,45)%j+A(j,48)%j+A(j,83)%j+A(j,85)%j+A(j,90)%j+A(j,92)%j)*f(9))/2._/**/REALKIND+((-A(j,13)%j-A(j,16)%j &
       -A(j,22)%j-A(j,25)%j-A(j,55)%j-A(j,60)%j-A(j,64)%j-A(j,66)%j-A(j,71)%j-A(j,73)%j-A(j,98)%j-A(j,100)%j-A(j,106)%j-A(j,109)%j &
       -A(j,115)%j-A(j,118)%j-A(j,123)%j-A(j,125)%j-A(j,130)%j-A(j,132)%j)*f(10))/2._/**/REALKIND
  M1(2) = ((-A(j,12)%j-A(j,15)%j-A(j,21)%j-A(j,24)%j-A(j,54)%j-A(j,59)%j-A(j,63)%j-A(j,65)%j-A(j,70)%j-A(j,72)%j-A(j,97)%j &
       -A(j,99)%j-A(j,105)%j-A(j,108)%j-A(j,114)%j-A(j,117)%j-A(j,122)%j-A(j,124)%j-A(j,129)%j-A(j,131)%j)*f(1))/6._/**/REALKIND &
       +((A(j,29)%j+A(j,30)%j+A(j,33)%j+A(j,34)%j+A(j,38)%j+A(j,50)%j+A(j,77)%j+A(j,78)%j+A(j,81)%j+A(j,82)%j+A(j,87)%j &
       +A(j,94)%j)*f(2))/6._/**/REALKIND+((-A(j,51)%j-A(j,56)%j)*f(3))/6._/**/REALKIND+((-A(j,35)%j-A(j,39)%j-A(j,44)%j &
       -A(j,47)%j)*f(4))/6._/**/REALKIND+((A(j,9)%j+A(j,10)%j+A(j,17)%j+A(j,18)%j+A(j,19)%j+A(j,26)%j+A(j,27)%j+A(j,28)%j &
       +A(j,31)%j+A(j,32)%j+A(j,42)%j+A(j,43)%j+A(j,61)%j+A(j,62)%j+A(j,67)%j+A(j,68)%j+A(j,69)%j+A(j,74)%j+A(j,75)%j+A(j,76)%j &
       +A(j,79)%j+A(j,80)%j+A(j,88)%j+A(j,89)%j+A(j,101)%j+A(j,102)%j+A(j,103)%j+A(j,110)%j+A(j,111)%j+A(j,112)%j+A(j,119)%j &
       +A(j,120)%j+A(j,121)%j+A(j,126)%j+A(j,127)%j+A(j,128)%j)*f(5))/6._/**/REALKIND+((-A(j,2)%j-A(j,4)%j-A(j,6)%j-A(j,8)%j &
       +A(j,37)%j+A(j,41)%j+A(j,46)%j+A(j,49)%j+A(j,84)%j+A(j,86)%j+A(j,91)%j+A(j,93)%j)*f(6))/6._/**/REALKIND+((-A(j,11)%j &
       -A(j,14)%j-A(j,20)%j-A(j,23)%j-A(j,53)%j-A(j,58)%j-A(j,104)%j-A(j,107)%j-A(j,113)%j-A(j,116)%j)*f(7))/6._/**/REALKIND+(( &
       -A(j,52)%j-A(j,57)%j-A(j,95)%j-A(j,96)%j)*f(8))/6._/**/REALKIND+((A(j,1)%j+A(j,3)%j+A(j,5)%j+A(j,7)%j-A(j,36)%j-A(j,40)%j &
       -A(j,45)%j-A(j,48)%j-A(j,83)%j-A(j,85)%j-A(j,90)%j-A(j,92)%j)*f(9))/6._/**/REALKIND+((A(j,13)%j+A(j,16)%j+A(j,22)%j &
       +A(j,25)%j+A(j,55)%j+A(j,60)%j+A(j,64)%j+A(j,66)%j+A(j,71)%j+A(j,73)%j+A(j,98)%j+A(j,100)%j+A(j,106)%j+A(j,109)%j &
       +A(j,115)%j+A(j,118)%j+A(j,123)%j+A(j,125)%j+A(j,130)%j+A(j,132)%j)*f(10))/6._/**/REALKIND
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppvvvj_ddxbbxzwwx_1_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppvvvj_ddxbbxzwwx_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppvvvj_ddxbbxzwwx_1")
#else
subroutine amp2tree(P, M2)
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppvvvj_ddxbbxzwwx_1")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppvvvj_ddxbbxzwwx_1")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppvvvj_ddxbbxzwwx_1")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppvvvj_ddxbbxzwwx_1")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppvvvj_ddxbbxzwwx_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppvvvj_ddxbbxzwwx_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppvvvj_ddxbbxzwwx_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppvvvj_ddxbbxzwwx_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppvvvj_ddxbbxzwwx_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
...
subroutine amp2tree_legacy(p, m2) &
    & bind(c,name="amp2tree_ppvvvj_ddxbbxzwwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccone_legacy(p, m2, i, j) &
    & bind(c,name="amp2ccone_ppvvvj_ddxbbxzwwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2ccall_legacy(p, m2) &
    & bind(c,name="amp2ccall_ppvvvj_ddxbbxzwwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(0:30-1)
...
subroutine amp2hcone_legacy(p, m2, i, j, mom) &
    & bind(c,name="amp2hcone_ppvvvj_ddxbbxzwwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_legacy(p, m2, i, mom) &
    & bind(c,name="amp2hcall_ppvvvj_ddxbbxzwwx_1_")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,7)
  real(c_double), intent(out) :: m2(7)
...
end module ol_tree_ppvvvj_ddxbbxzwwx_1_/**/REALKIND
