...
module ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND
  contains
...
! **********************************************************************
  ...
  use ol_vamp_1_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND, only: vamp_1
  use ol_vamp_2_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND, only: vamp_2
...
! M2tree = helicity-summed squared tree matrix element for nu_e nu_mu e+ mu+ anti-up anti-charm down strange -> 0
! M2loop0 = helicity-summed squared loop matrix element for nu_e nu_mu e+ mu+ anti-up anti-charm down strange -> 0
...
! **********************************************************************
  ...
  use ol_colourmatrix_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  ...
  use ol_tensor_sum_storage_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND, only: &
    & reset_tensor_sum, integrate_tensor_sum, scale_tensor_sum
  use ol_loop_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND, only: &
...
!  complex(REALKIND)    :: Mcol_loop(1), Mct(1)
  real(REALKIND)       :: M2colint(37), M2CC(8,8), M2CC_EW ! colour correlations
  integer,        save :: ntry = 1
...
! #ifdef LOOPSQUARED
...
#ifdef LOOPSQUARED
  M2L2 = (M2L2 + 2*M2L2ct + M2L2ct2) / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
#else
  ...
  if (IR_is_on > 0) then
    ...
    M2CC_EW = M2colint(8*(8+1)/2+1)
    call intdip(ioperator_mode, M2L0, M2CC, M2CC_EW, [3,3,3,3,2,2,2,2], [0,0,3,3,-2,-2,-1,-1]/3._/**/REALKIND, &
      & 8, extmasses2, QInvariantsMatrix, vdip, c_dip)
    IRL1(0) = c_dip(0) / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
    IRL1(1) = c_dip(1) / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
    IRL1(2) = c_dip(2) / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
  else
    vdip = 0
    IRL1 = 0
  end if
  ...
  M2L0 = M2L0 / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
  M2L1 = 2*(M2L1 + M2ct)
  if (IR_is_on > 1) then
    M2L1 = M2L1 + vdip
  end if
  M2L1 = M2L1 / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
...
#ifdef LOOPSQUAREDcorr
! **********************************************************************
subroutine vamp2cache(P_scatt, M2L0, M2L1, M2L2, corr)
! P_scatt(0:3,Npart) = external momenta
! M2L0 = helicity-summed squared tree matrix element for nu_e nu_mu e+ mu+ anti-up anti-charm down strange -> 0
! M2L2 = helicity-summed tree x loop matrix element for nu_e nu_mu e+ mu+ anti-up anti-charm down strange -> 0
! M2L2 = helicity-summed squared loop matrix element for nu_e nu_mu e+ mu+ anti-up anti-charm down strange -> 0
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_debug, only: ol_fatal
  use ol_external_ppllnnjj_vbs_nenmexmxuxcxds_2, only: average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2, H, H_HC, &
   external_perm_ppllnnjj_vbs_nenmexmxuxcxds_2, extcomb_perm_ppllnnjj_vbs_nenmexmxuxcxds_2
  use ol_colourmatrix_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND ! colmat_not_initialised, colourmatrix_init, K1, K2, KL, KL2, KL2ct, KL2ct2
  use ol_parameters_decl_/**/REALKIND ! parameters_status, scalefactor, <masses>
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_loop_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND, only: zerohel, M0, Mct, Mcol_loop
  use ol_wavefunctions_/**/REALKIND, only: wf_V_Std
  use ol_kinematics_/**/REALKIND, only: LC2Std_Rep_cmplx
  ...
  integer              :: la, i, j, colmatpos, recycle_mode
  real(REALKIND)       :: M2colint(37)
  integer,        save :: ntry = 1
  real(REALKIND)       :: M2hel, M2ct
  real(REALKIND)       :: M2L2cc(0:37)
  integer              :: cr, k, m, n, l, hels(2,4)
  integer              :: extcombs(37), extcombs_permuted(37)
  ...
  emitter = external_perm_ppllnnjj_vbs_nenmexmxuxcxds_2(corr%emitter)
  do k = 1, nextcombs
  ...
  if (cr == 1) then
    ...
    do la=1,256
#if 1 > 0
        call colint(Mcol_loop(:,la), Mcol_loop(:,la), 1, KL2, extcombs_permuted(1:nextcombs), M2L2cc)  ! <M1|M1>
        call colint(2*Mcol_loop(:,la), Mct(:,la), 1, KL2ct, extcombs_permuted(1:nextcombs), M2L2cc)    ! 2*<M1|Mct>
#endif
#if 1 > 0
        call colint(Mct(:,la), Mct(:,la), 1, KL2ct2, extcombs_permuted(1:nextcombs), M2L2cc)            ! <Mct|Mct>
#endif
    end do
    do k = 0, 37
      corr%rescc(k) = M2L2cc(extcomb_perm_ppllnnjj_vbs_nenmexmxuxcxds_2(k)) &
      & / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
    end do
    M2L2 = corr%rescc(0)
...
#if 1 > 0
          call colint(Mcol_loop(:,hels(1,l)), om*Mcol_loop(:,hels(2,l)), 1, KL2, extcombs_permuted(1:nextcombs), M2L2cc) ! <M1|M1>
          call colint(Mcol_loop(:,hels(1,l)), om*Mct(:,hels(2,l)), 1, KL2ct, extcombs_permuted(1:nextcombs), M2L2cc)     ! <M1|Mct>
          call colint(om*Mcol_loop(:,hels(1,l)), Mct(:,hels(2,l)), 1, KL2ct, extcombs_permuted(1:nextcombs), M2L2cc)     ! <Mct|M1>
#endif
#if 1 > 0
          call colint(Mct(:,hels(1,l)), om*Mct(:,hels(2,l)), 1, KL2ct2, extcombs_permuted(1:nextcombs), M2L2cc)           ! <Mct|Mct>
#endif
      end do
    end do

    do k = 0, 37
      corr%rescc(k) = 0.5_/**/REALKIND*M2L2cc(extcomb_perm_ppllnnjj_vbs_nenmexmxuxcxds_2(k)) &
      & / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
    end do
  ...
  ! get wfs
    ...
    if (any(corr%resmunu(:,:) /= corr%resmunu(:,:))) then
      ...
      corr%resmunu = corr%resmunu / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
...
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND
  use ol_external_ppllnnjj_vbs_nenmexmxuxcxds_2, only: hel_not_initialised, hel_init, H
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  use ol_external_ppllnnjj_vbs_nenmexmxuxcxds_2, only: &
    & external_perm_inv_ppllnnjj_vbs_nenmexmxuxcxds_2, average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_kinematics_/**/REALKIND, only: conv_mom_scatt2in, internal_momenta
  use ol_forced_parameters_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND, only: check_forced_parameters
  use ol_loop_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND, only: &
...
!  DOI      = 0
  ...
  ! Convert 2 -> n-2 PS-point to n -> 0 (so that P(1) + ... + P(n) = 0)
  call conv_mom_scatt2in(P_scatt, extmasses2, P, external_perm_inv_ppllnnjj_vbs_nenmexmxuxcxds_2, 8)
  ! internal-propagator momenta in light-cone representation
  call internal_momenta(P, 8)
...
!  DOI      = DOI_bak
  ...
  scalebackfactor = scalefactor**(2*8-8)
  M2tree = scalebackfactor * M2tree / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
  M2ct   = scalebackfactor * 2*M2ct / average_factor_ppllnnjj_vbs_nenmexmxuxcxds_2
...
end module ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/REALKIND



#ifdef PRECISION_dp

module ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2
  use ol_parameters_decl_/**/DREALKIND, only: procname_length
  use ol_data_types_/**/DREALKIND, only: me_cache
  implicit none
  character(procname_length) :: processname = 'ppllnnjj_vbs_nenmexmxuxcxds_2'
  ...
  subroutine finish_ppllnnjj_vbs_nenmexmxuxcxds_2()
    ! final update of the stability histogram
    use KIND_TYPES, only: DREALKIND
    use ol_stability, only: finish_histograms
    use ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/DREALKIND, only: vamp
    implicit none
  ...
  end subroutine finish_ppllnnjj_vbs_nenmexmxuxcxds_2


  subroutine vamp2(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_f_vamp2_ppllnnjj_vbs_nenmexmxuxcxds_2")
    use KIND_TYPES, only: DREALKIND
    use ol_init, only: register_cleanup
    use ol_external_ppllnnjj_vbs_nenmexmxuxcxds_2, only: external_perm_ppllnnjj_vbs_nenmexmxuxcxds_2
    use ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/DREALKIND, only: vamp2dp => vamp2pc
#ifdef USE_qp
    use ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/QREALKIND, only: vamp2qp => vamp2pc
#endif
...
#endif
    if (first_call) then
      call register_cleanup(finish_ppllnnjj_vbs_nenmexmxuxcxds_2)
      first_call = .false.
    end if
    call vamp2generic(vamp2dp, vamp2qp, processname, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, &
                    ...
                    & external_perm_ppllnnjj_vbs_nenmexmxuxcxds_2, me_caches)
...
! **********************************************************************
  subroutine vampcr2(P_scatt, M2L0, M2L1, M2L2, crtype, &
      ...
      & bind(c,name="ol_f_vampcr2_ppllnnjj_vbs_nenmexmxuxcxds_2")
...
! **********************************************************************
    use KIND_TYPES, only: DREALKIND
    use ol_external_ppllnnjj_vbs_nenmexmxuxcxds_2, only: external_perm_ppllnnjj_vbs_nenmexmxuxcxds_2
    use ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/DREALKIND, only: vamp2cache
    use ol_data_types_/**/DREALKIND, only: correlator
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,8)
    ...
    real(REALKIND),  intent(out) :: M2cc(0:8*(8+1)/2+1)
    real(DREALKIND), intent(out) :: M2munu(4,4)
    integer :: n_cc = 8*(8+1)/2
    ...
    if (crtype == 0) then
      ...
      allocate(corr%rescc(0:8*(8+1)/2+1))
  ...
  subroutine ctamp2(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_f_ctamp2_ppllnnjj_vbs_nenmexmxuxcxds_2")
    use KIND_TYPES, only: DREALKIND
    use ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2_/**/DREALKIND, only: ctamp2base
    implicit none
    real(DREALKIND), intent(in)  :: P_scatt(0:3,8)
  ...
  subroutine vamp2_c(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="ol_vamp2_ppllnnjj_vbs_nenmexmxuxcxds_2")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,8)
    real(c_double), intent(out) :: M2L0, M2L1(0:2), IRL1(0:2), M2L2(0:4), IRL2(0:4)
    real(DREALKIND) :: f_p_scatt(0:3,8)
  ...
  subroutine ctamp2_c(P_scatt, M2tree, M2ct) &
      & bind(c,name="ol_ctamp2_ppllnnjj_vbs_nenmexmxuxcxds_2")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,8)
  ...
  subroutine vamp2_legacy(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
      & bind(c,name="vamp2_ppllnnjj_vbs_nenmexmxuxcxds_2_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,8)
  ...
  subroutine ctamp2_legacy(P_scatt, M2tree, M2ct) &
      & bind(c,name="ctamp2_ppllnnjj_vbs_nenmexmxuxcxds_2_")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
    real(c_double), intent(in)  :: P_scatt(0:3,8)
...
end module ol_vamp_ppllnnjj_vbs_nenmexmxuxcxds_2
