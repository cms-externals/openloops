...
module ol_colourmatrix_eettj_eexttxg_1_/**/REALKIND
...
end module ol_colourmatrix_eettj_eexttxg_1_/**/REALKIND



module ol_forced_parameters_eettj_eexttxg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_eettj_eexttxg_1_/**/REALKIND

module ol_loop_eettj_eexttxg_1_/**/REALKIND
...
! **********************************************************************
  ...
  ! factors of the diagrams
    ...
    f( 5) = (2*CI*countertermnorm*ctGtt*eQED**2*gQCD**3)/3._/**/REALKIND
    f( 6) = CI*countertermnorm*ctGtt*eQED**2*gQCD**3
    f( 7) = (2*CI*countertermnorm*ctVtt*eQED**2*gQCD**3)/3._/**/REALKIND
    f( 8) = CI*countertermnorm*ctVtt*eQED**2*gQCD**3
...
! **********************************************************************
  ...
  call prop_A_Q(wf(:,8),Q(:,24),MT,1_intkind1,wf(:,9))
  ...
  call counter_Q_A(cttt,wf(:,3),Q(:,20),wf(:,22))
  call prop_A_Q(wf(:,4),Q(:,11),MT,1_intkind1,wf(:,23))
  call prop_A_Q(wf(:,7),Q(:,11),MT,1_intkind1,wf(:,24))
  call counter_A_Q(cttt,wf(:,9),Q(:,24),wf(:,25))
  call prop_Q_A(wf(:,10),Q(:,7),MT,1_intkind1,wf(:,26))
  call prop_Q_A(wf(:,11),Q(:,7),MT,1_intkind1,wf(:,27))
...
subroutine diagrams(A)
  ...
  A(5) = cont_VV(wf(:,12),wf(:,13)) * den(10)
  A(6) = cont_QA(wf(:,3),wf(:,14)) * den(3)
  A(7) = cont_QA(wf(:,3),wf(:,15)) * den(5)
  A(8) = cont_QA(wf(:,10),wf(:,17)) * den(7)
  A(9) = cont_QA(wf(:,11),wf(:,17)) * den(8)
  A(10) = cont_QA(wf(:,9),wf(:,18)) * den(7)
  A(11) = cont_QA(wf(:,9),wf(:,19)) * den(8)
  A(12) = cont_QA(wf(:,4),wf(:,21)) * den(3)
  A(13) = cont_QA(wf(:,7),wf(:,21)) * den(5)
  A(14) = cont_QA(wf(:,22),wf(:,23)) * den(13)
  A(15) = cont_QA(wf(:,22),wf(:,24)) * den(15)
  A(16) = cont_QA(wf(:,25),wf(:,26)) * den(18)
  A(17) = cont_QA(wf(:,25),wf(:,27)) * den(20)
...
end module ol_loop_eettj_eexttxg_1_/**/REALKIND
