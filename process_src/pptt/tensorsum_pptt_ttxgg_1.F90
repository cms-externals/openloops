...
! **********************************************************************
module ol_tensor_sum_storage_pptt_ttxgg_1_/**/REALKIND
! **********************************************************************
...
subroutine max_point(r) &
    & bind(c,name="ol_f_max_point_pptt_ttxgg_1")
...
subroutine tensor_rank(r) &
    & bind(c,name="ol_f_tensor_rank_pptt_ttxgg_1")
  ! Return the number maximal tensor rank
  implicit none
  integer, intent(out) :: r
  r = 3
...
! **********************************************************************
  use ol_external_pptt_ttxgg_1, only: channel_number_pptt_ttxgg_1
  ...
  call channel_on(channel_number_pptt_ttxgg_1)
  M2 = 0
  ...
  momenta_14(:,1) = Q(1:4,9)
  momenta_14(:,2) = Q(1:4,13)
  ...
  call TI_call(1, momenta_3, masses2_3, T1sum(:,3), M2)
  call TI_call(1, momenta_2, masses2_3, T1sum(:,4), M2)

  call TI_call(2, momenta_16, masses2_8, T2sum(:,1), M2)
  call TI_call(2, momenta_10, masses2_7, T2sum(:,2), M2)
  call TI_call(2, momenta_12, masses2_8, T2sum(:,3), M2)
  call TI_call(2, momenta_11, masses2_7, T2sum(:,4), M2)
  call TI_call(2, momenta_13, masses2_8, T2sum(:,5), M2)
  call TI_call(2, momenta_14, masses2_7, T2sum(:,6), M2)
  call TI_call(2, momenta_8, masses2_8, T2sum(:,7), M2)
  call TI_call(2, momenta_15, masses2_7, T2sum(:,8), M2)
  call TI_call(2, momenta_9, masses2_8, T2sum(:,9), M2)
  call TI_call(2, momenta_6, masses2_7, T2sum(:,10), M2)
  call TI_call(2, momenta_5, masses2_4, T2sum(:,11), M2)
  call TI_call(2, momenta_5, masses2_2, T2sum(:,12), M2)
  call TI_call(2, momenta_5, masses2_1, T2sum(:,13), M2)

  call TI_call(3, momenta_7, masses2_9, T3sum(:,1), M2)
  call TI_call(3, momenta_7, masses2_6, T3sum(:,2), M2)
  call TI_call(3, momenta_7, masses2_5, T3sum(:,3), M2)
  call TI_call(3, momenta_18, masses2_10, T3sum(:,4), M2)
  call TI_call(3, momenta_20, masses2_12, T3sum(:,5), M2)
  call TI_call(3, momenta_17, masses2_10, T3sum(:,6), M2)
  call TI_call(3, momenta_22, masses2_12, T3sum(:,7), M2)
  call TI_call(3, momenta_21, masses2_11, T3sum(:,8), M2)
  call TI_call(3, momenta_19, masses2_11, T3sum(:,9), M2)

#ifdef LOOPSQUARED
  end if
#endif

  call channel_off(channel_number_pptt_ttxgg_1)
end subroutine integrate_tensor_sum

end module ol_tensor_sum_storage_pptt_ttxgg_1_/**/REALKIND
