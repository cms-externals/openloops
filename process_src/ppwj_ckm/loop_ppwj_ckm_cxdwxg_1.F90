...
module ol_colourmatrix_ppwj_ckm_cxdwxg_1_/**/REALKIND
...
end module ol_colourmatrix_ppwj_ckm_cxdwxg_1_/**/REALKIND



module ol_forced_parameters_ppwj_ckm_cxdwxg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppwj_ckm_cxdwxg_1_/**/REALKIND

module ol_loop_ppwj_ckm_cxdwxg_1_/**/REALKIND
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f(2) = (CI*countertermnorm*eQED*gQCD**3*VCKMdc)/(sqrt2*sw)
    f(3) = (CI*countertermnorm*ctGcc*eQED*gQCD**3*VCKMdc)/(sqrt2*sw)
    f(4) = (CI*countertermnorm*ctGqq*eQED*gQCD**3*VCKMdc)/(sqrt2*sw)
    f(5) = (CI*countertermnorm*ctVqq*eQED*gQCD**3*VCKMdc)/(sqrt2*sw)
    f(6) = (CI*eQED*gQCD**3*integralnorm*SwB*VCKMdc)/(sqrt2*sw)
    f(7) = (eQED*gQCD**3*integralnorm*SwB*VCKMdc)/(sqrt2*sw)
...
! **********************************************************************
  ...
  call vert_AV_Q(wf(:,0),wf(:,-3),wf(:,4))
  call vert_WQ_A(wf(:,-2),wf(:,-1),wf(:,5))
  call prop_A_Q(wf(:,4),Q(:,9),ZERO,0_intkind1,wf(:,6))
  call counter_VQ_A(wf(:,-3),wf(:,-1),wf(:,7))
  call counter_WQ_A(wf(:,-2),wf(:,-1),wf(:,8))
  call counter_AV_Q(wf(:,0),wf(:,-3),wf(:,9))
  call prop_Q_A(wf(:,5),Q(:,6),ZERO,0_intkind1,wf(:,10))
  call counter_AW_Q(wf(:,0),wf(:,-2),wf(:,11))
  call prop_Q_A(wf(:,2),Q(:,10),ZERO,0_intkind1,wf(:,12))
  call counter_A_Q(ctqq,wf(:,3),Q(:,5),wf(:,13))
  call counter_A_Q(ctcc,wf(:,6),Q(:,9),wf(:,14))
...
subroutine colourvectors(A, M1, M2)
  ...
  M2(1) = (-A(7)-A(8))*f(2)+A(5)*f(3)+A(3)*f(4)+(A(4)+A(6))*f(5)

end subroutine colourvectors

end module ol_loop_ppwj_ckm_cxdwxg_1_/**/REALKIND
