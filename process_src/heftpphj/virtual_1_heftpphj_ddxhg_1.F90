...
module ol_vamp_1_heftpphj_ddxhg_1_/**/REALKIND
contains

! **********************************************************************
subroutine vamp_1(M)
! P(0:3,nlegs) = incoming external momenta
! Uses tree structures 'wf', factors 'c', and denominators 'den' from loop_heftpphj_ddxhg_1.
! Sets colour stripped amplitudes A from the module loop_amplitudes_heftpphj_ddxhg_1.
! **********************************************************************
...
#endif
  use ol_loop_heftpphj_ddxhg_1_/**/REALKIND, only: c, f, wf, den
  use ol_vert_interface_/**/REALKIND
  use ol_prop_interface_/**/REALKIND
  use ol_last_step_/**/REALKIND
  use ol_tensor_sum_storage_heftpphj_ddxhg_1_/**/REALKIND
  ...
  call loop_GHG_G(G0(:,:,:,1),Q(:,0),wf(:,-2),wf(:,-3),Q(:,8),G1(:,:,:,1),Q(:,12))
  call loop_VA_Q(G1(:,:,:,1),wf(:,-1),G1(:,:,:,2))
  call loop_A_Q(G1(:,:,:,2),Q(:,14),ZERO,G2(:,:,:,1))
  call check_last_AQ_V(l_switch,G2(:,:,:,1),wf(:,0),G2tensor(:,1))
  call check_last_UV_W(l_switch,G1(:,:,:,1),Q(:,12),wf(:,1),Q(:,3),G2tensor(:,2))
  call loop_AS_Q(G0(:,:,:,1),wf(:,-2),G0(:,:,:,2),gH)
  call loop_A_Q(G0(:,:,:,2),Q(:,4),MB,G1(:,:,:,3))
  call loop_AV_Q(G1(:,:,:,3),wf(:,-3),G1(:,:,:,4))
  call loop_A_Q(G1(:,:,:,4),Q(:,12),MB,G2(:,:,:,2))
  call loop_AV_Q(G2(:,:,:,2),wf(:,1),G2(:,:,:,3))
  call check_last_A_Q(l_switch,G2(:,:,:,3),Q(:,15),MB,G3tensor(:,1))
  call loop_QS_A(G0(:,:,:,1),wf(:,-2),G0(:,:,:,3),gH)
  call loop_Q_A(G0(:,:,:,3),Q(:,4),MB,G1(:,:,:,5))
  call loop_QV_A(G1(:,:,:,5),wf(:,-3),G1(:,:,:,6))
  call loop_Q_A(G1(:,:,:,6),Q(:,12),MB,G2(:,:,:,4))
  call loop_QV_A(G2(:,:,:,4),wf(:,1),G2(:,:,:,5))
  call check_last_Q_A(l_switch,G2(:,:,:,5),Q(:,15),MB,G3tensor(:,2))
  call loop_GH_G(G0(:,:,:,1),Q(:,0),wf(:,-2),G2(:,:,:,6),Q(:,4))
  call loop_UV_W(G2(:,:,:,6),Q(:,4),wf(:,-3),Q(:,8),G3(:,:,:,1))
  call check_last_UV_W(l_switch,G3(:,:,:,1),Q(:,12),wf(:,1),Q(:,3),G4tensor(:,1))
  call loop_VA_Q(G3(:,:,:,1),wf(:,-1),G3(:,:,:,2))
  call loop_A_Q(G3(:,:,:,2),Q(:,14),ZERO,G4(:,:,:,1))
  call check_last_AQ_V(l_switch,G4(:,:,:,1),wf(:,0),G4tensor(:,2))
  call loop_VQ_A(G2(:,:,:,6),wf(:,7),G2(:,:,:,7))
  call loop_Q_A(G2(:,:,:,7),Q(:,13),ZERO,G3(:,:,:,3))
  call check_last_QA_V(l_switch,G3(:,:,:,3),wf(:,-1),G3tensor(:,3))
  call loop_VA_Q(G2(:,:,:,6),wf(:,10),G2(:,:,:,8))
  call loop_A_Q(G2(:,:,:,8),Q(:,14),ZERO,G3(:,:,:,4))
  call check_last_AQ_V(l_switch,G3(:,:,:,4),wf(:,0),G3tensor(:,4))
  call loop_VA_Q(G2(:,:,:,6),wf(:,-1),G2(:,:,:,9))
  call loop_A_Q(G2(:,:,:,9),Q(:,6),ZERO,G3(:,:,:,5))
  call loop_AV_Q(G3(:,:,:,5),wf(:,-3),G3(:,:,:,6))
  call loop_A_Q(G3(:,:,:,6),Q(:,14),ZERO,G4(:,:,:,2))
  call check_last_AQ_V(l_switch,G4(:,:,:,2),wf(:,0),G4tensor(:,3))
  call loop_VA_Q(G0(:,:,:,1),wf(:,-1),G0(:,:,:,4))
  call loop_A_Q(G0(:,:,:,4),Q(:,2),ZERO,G1(:,:,:,7))
  call loop_AV_Q(G1(:,:,:,7),wf(:,2),G1(:,:,:,8))
  call loop_A_Q(G1(:,:,:,8),Q(:,14),ZERO,G2(:,:,:,10))
  call check_last_AQ_V(l_switch,G2(:,:,:,10),wf(:,0),G2tensor(:,3))
  call loop_UV_W(G0(:,:,:,1),Q(:,0),wf(:,2),Q(:,12),G1(:,:,:,9))
  call loop_VA_Q(G1(:,:,:,9),wf(:,-1),G1(:,:,:,10))
  call loop_A_Q(G1(:,:,:,10),Q(:,14),ZERO,G2(:,:,:,11))
  call check_last_AQ_V(l_switch,G2(:,:,:,11),wf(:,0),G2tensor(:,4))
  call check_last_UV_W(l_switch,G1(:,:,:,9),Q(:,12),wf(:,1),Q(:,3),G2tensor(:,5))
  call loop_UV_W(G0(:,:,:,1),Q(:,0),wf(:,-3),Q(:,8),G1(:,:,:,11))
  call loop_GH_G(G1(:,:,:,11),Q(:,8),wf(:,-2),G3(:,:,:,7),Q(:,12))
  call loop_VA_Q(G3(:,:,:,7),wf(:,-1),G3(:,:,:,8))
  call loop_A_Q(G3(:,:,:,8),Q(:,14),ZERO,G4(:,:,:,3))
  call check_last_AQ_V(l_switch,G4(:,:,:,3),wf(:,0),G4tensor(:,4))
  call check_last_GHG_G(l_switch,G1(:,:,:,11),Q(:,8),wf(:,-2),wf(:,1),Q(:,3),G2tensor(:,6),Q(:,15))
  call loop_GGG_G_12(G0(:,:,:,1),wf(:,-3),wf(:,1),G0(:,:,:,5))
  call check_last_GH_G(l_switch,G0(:,:,:,5),Q(:,11),wf(:,-2),G2tensor(:,7),Q(:,15))
  call loop_GGG_G_12(G0(:,:,:,1),wf(:,1),wf(:,-3),G0(:,:,:,6))
  call check_last_GH_G(l_switch,G0(:,:,:,6),Q(:,11),wf(:,-2),G2tensor(:,8),Q(:,15))
  call loop_GGG_G_23(G0(:,:,:,1),wf(:,-3),wf(:,1),G0(:,:,:,7))
  call check_last_GH_G(l_switch,G0(:,:,:,7),Q(:,11),wf(:,-2),G2tensor(:,9),Q(:,15))
  call loop_QV_A(G0(:,:,:,1),wf(:,2),G0(:,:,:,8))
  call loop_Q_A(G0(:,:,:,8),Q(:,12),ZERO,G1(:,:,:,12))
  call loop_QV_A(G1(:,:,:,12),wf(:,1),G1(:,:,:,13))
  call check_last_Q_A(l_switch,G1(:,:,:,13),Q(:,15),ZERO,G2tensor(:,10))
  call loop_QV_A(G0(:,:,:,1),wf(:,2),G0(:,:,:,9))
  call loop_Q_A(G0(:,:,:,9),Q(:,12),MB,G1(:,:,:,14))
  call loop_QV_A(G1(:,:,:,14),wf(:,1),G1(:,:,:,15))
  call check_last_Q_A(l_switch,G1(:,:,:,15),Q(:,15),MB,G2tensor(:,11))
  call loop_CV_D(G0(:,:,:,1),Q(:,0),wf(:,2),Q(:,12),G1(:,:,:,16))
  call check_last_CV_D(l_switch,G1(:,:,:,16),Q(:,12),wf(:,1),Q(:,3),G2tensor(:,12))
...
end module ol_vamp_1_heftpphj_ddxhg_1_/**/REALKIND
