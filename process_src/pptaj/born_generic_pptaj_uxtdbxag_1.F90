...
module ol_external_pptaj_uxtdbxag_1
  implicit none
  integer :: dummy_counter
  ! Permutation and inverse permutation of external particles
  integer, save :: external_perm_pptaj_uxtdbxag_1(6) = &
                     [ (dummy_counter, dummy_counter = 1, 6) ]
  integer, save :: external_perm_inv_pptaj_uxtdbxag_1(6) = &
                     [ (dummy_counter, dummy_counter = 1, 6) ]
  integer, save :: extcomb_perm_pptaj_uxtdbxag_1(0:22) = &
                     [ (dummy_counter, dummy_counter = 0, 22) ]
  ! Particle types (mapping of fields to integers is not fixed!)
  integer, save :: particle_types_pptaj_uxtdbxag_1(6) = &
                     [ 1, 2, 3, 4, 5, 6 ]
  ! Colour and helicity average factors per particle
  integer, save :: average_factors_pptaj_uxtdbxag_1(6) = &
                     [ 6, 6, 6, 6, 2, 16 ]
  ! Average factor; initialised to the identity permutation
  integer, save :: average_factor_pptaj_uxtdbxag_1 = &
                     36
  integer, save :: channel_number_pptaj_uxtdbxag_1 = -1
  ...
  subroutine n_external(n) &
      & bind(c,name="ol_f_n_external_pptaj_uxtdbxag_1")
  ...
  subroutine n_external_c(n) &
      & bind(c,name="ol_n_external_pptaj_uxtdbxag_1")
  ...
  subroutine set_permutation(perm) &
      & bind(c,name="ol_f_set_permutation_pptaj_uxtdbxag_1")
    use ol_parameters_decl_/**/DREALKIND, only: out_symmetry_on
    use ol_generic, only: factorial
    implicit none
    integer, intent(in) :: perm(6)
    integer :: i, j, ii, jj
    integer :: particle_types_perm_pptaj_uxtdbxag_1(6)
    external_perm_pptaj_uxtdbxag_1 = perm
    do i = 1, 6
      external_perm_inv_pptaj_uxtdbxag_1( &
        external_perm_pptaj_uxtdbxag_1(i)) = i
      particle_types_perm_pptaj_uxtdbxag_1(i) = &
        particle_types_pptaj_uxtdbxag_1( &
        external_perm_pptaj_uxtdbxag_1(i))
    end do
    do i = 1, 6
      do j = 1, i
        if (external_perm_pptaj_uxtdbxag_1(i) >= &
          external_perm_pptaj_uxtdbxag_1(j)) then
          ii = external_perm_pptaj_uxtdbxag_1(i)
          jj = external_perm_pptaj_uxtdbxag_1(j)
        else
          ii = external_perm_pptaj_uxtdbxag_1(j)
          jj = external_perm_pptaj_uxtdbxag_1(i)
        end if
        extcomb_perm_pptaj_uxtdbxag_1((i*(i-1))/2 + j) = (ii*(ii-1))/2 + jj
      end do
    end do
    ! Colour and helicity average factor
    average_factor_pptaj_uxtdbxag_1 = &
      average_factors_pptaj_uxtdbxag_1( &
      external_perm_pptaj_uxtdbxag_1(1)) &
      * average_factors_pptaj_uxtdbxag_1( &
      external_perm_pptaj_uxtdbxag_1(2))
    ! Symmetry factor for outgoing particles
    if (out_symmetry_on /= 0) then
      do i = 1, 6
        average_factor_pptaj_uxtdbxag_1 = &
          average_factor_pptaj_uxtdbxag_1 &
          * factorial(count(particle_types_perm_pptaj_uxtdbxag_1(3:6) == i))
      end do
    end if
  ...
  subroutine set_permutation_c(perm) &
      & bind(c,name="ol_set_permutation_pptaj_uxtdbxag_1")
    use, intrinsic :: iso_c_binding, only: c_int
    implicit none
  ...
  subroutine get_masses(m_ex) &
      & bind(c,name="ol_f_get_masses_pptaj_uxtdbxag_1")
    ...
    m_ex_orig = [ rZERO, rMT_unscaled, rZERO, rMB_unscaled, rZERO, rZERO ]
    do i = 1, 6
      m_ex(i) = m_ex_orig(external_perm_pptaj_uxtdbxag_1(i))
    end do
  end subroutine get_masses


  subroutine get_masses_c(m_ex) &
      & bind(c,name="ol_get_masses_pptaj_uxtdbxag_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo(sqrt_s, p_rambo) &
      & bind(c,name="ol_f_rambo_pptaj_uxtdbxag_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo_c(sqrt_s, p_rambo) &
      & bind(c,name="ol_rambo_pptaj_uxtdbxag_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
...
end module ol_external_pptaj_uxtdbxag_1


module colour_basis_pptaj_uxtdbxag_1
  implicit none
  ! tree colour basis
  ...
  pure subroutine tree_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_tree_colbasis_dim_pptaj_uxtdbxag_1")
    implicit none
  ...
  subroutine tree_colbasis(basis, powers) &
    & bind(c, name="ol_tree_colbasis_pptaj_uxtdbxag_1")
    implicit none
...
end module colour_basis_pptaj_uxtdbxag_1


! Only for compatibility with the Sherpa interface

subroutine set_permutation_pptaj_uxtdbxag_1(perm)
  use ol_external_pptaj_uxtdbxag_1, only: set_permutation
  implicit none
  integer, intent(in) :: perm(6)
  call set_permutation(perm)
end subroutine set_permutation_pptaj_uxtdbxag_1

! **********************************************************************
module ol_heltables_pptaj_uxtdbxag_1
! **********************************************************************
  ...
  integer(intkind2), save :: t3x16(2,16,4), t3x4(2,4,11), t3x8(2,8,26), t3x64(2,64,28)
...
! **********************************************************************
  ...
  call heltable([2,2,4], n3(:,27), t3x4(:,:,10))
  n2(10) = 4
  n2(11) = 4
  call heltable([4,4,16], n3(:,28), t3x16(:,:,4))
  call heltable([2,4,8], n3(:,29), t3x8(:,:,15))
  call heltable([2,4,8], n3(:,30), t3x8(:,:,16))
  n2(12) = 8
  call heltable([2,2,4], n3(:,31), t3x4(:,:,11))
  call heltable([2,4,8], n3(:,32), t3x8(:,:,17))
  call heltable([4,2,8], n3(:,33), t3x8(:,:,18))
  call heltable([2,4,8], n3(:,34), t3x8(:,:,19))
  call heltable([4,2,8], n3(:,35), t3x8(:,:,20))
  call heltable([2,4,8], n3(:,36), t3x8(:,:,21))
  call heltable([4,2,8], n3(:,37), t3x8(:,:,22))
  call heltable([2,4,8], n3(:,38), t3x8(:,:,23))
  call heltable([4,2,8], n3(:,39), t3x8(:,:,24))
  call heltable([4,2,8], n3(:,40), t3x8(:,:,25))
  call heltable([2,4,8], n3(:,41), t3x8(:,:,26))
  call heltable([4,16,64], n3(:,42), t3x64(:,:,1))
  call heltable([8,8,64], n3(:,43), t3x64(:,:,2))
  call heltable([4,16,64], n3(:,44), t3x64(:,:,3))
  call heltable([8,8,64], n3(:,45), t3x64(:,:,4))
  call heltable([8,8,64], n3(:,46), t3x64(:,:,5))
  call heltable([8,8,64], n3(:,47), t3x64(:,:,6))
  call heltable([8,8,64], n3(:,48), t3x64(:,:,7))
  call heltable([8,8,64], n3(:,49), t3x64(:,:,8))
  call heltable([8,8,64], n3(:,50), t3x64(:,:,9))
  call heltable([8,8,64], n3(:,51), t3x64(:,:,10))
  call heltable([4,16,64], n3(:,52), t3x64(:,:,11))
  call heltable([8,8,64], n3(:,53), t3x64(:,:,12))
  call heltable([4,16,64], n3(:,54), t3x64(:,:,13))
  call heltable([8,8,64], n3(:,55), t3x64(:,:,14))
  call heltable([8,8,64], n3(:,56), t3x64(:,:,15))
  call heltable([8,8,64], n3(:,57), t3x64(:,:,16))
  call heltable([8,8,64], n3(:,58), t3x64(:,:,17))
  call heltable([8,8,64], n3(:,59), t3x64(:,:,18))
  call heltable([8,8,64], n3(:,60), t3x64(:,:,19))
  call heltable([8,8,64], n3(:,61), t3x64(:,:,20))
  call heltable([8,8,64], n3(:,62), t3x64(:,:,21))
  call heltable([8,8,64], n3(:,63), t3x64(:,:,22))
  call heltable([8,8,64], n3(:,64), t3x64(:,:,23))
  call heltable([8,8,64], n3(:,65), t3x64(:,:,24))
  call heltable([8,8,64], n3(:,66), t3x64(:,:,25))
  call heltable([8,8,64], n3(:,67), t3x64(:,:,26))
  call heltable([8,8,64], n3(:,68), t3x64(:,:,27))
  call heltable([8,8,64], n3(:,69), t3x64(:,:,28))

  heltables_not_init = .false.

end subroutine init_heltables

end module ol_heltables_pptaj_uxtdbxag_1
